...
mod tests {
    ...
    use hot_trimmer_domain::{
        LogicalGridSpec, NormalizedPoint, PartitionFamily, PartitionRecipe, RectificationSettings,
        generate_partition,
    };
    ...
    fn intentional_source_partition() {
        for (grid, target) in [
            ...
            assert_eq!(
                first,
                generate_partition(&recipe).expect("deterministic partition")
            );
            ...
            for region in &first {
                for y in region.grid_rect.y..region.grid_rect.y + region.grid_rect.height {
                    for x in region.grid_rect.x..region.grid_rect.x + region.grid_rect.width {
                        cells[(y * grid.width + x) as usize] += 1;
                    }
                }
            }
            ...
            assert_eq!(
                first
                    .iter()
                    .filter(|region| region.family == PartitionFamily::SmallDetail)
                    .count(),
                2
            );
            assert_eq!(
            ...
            assert!(
                first
                    ...
                    .all(|region| region.grid_rect.height
                        <= recipe.composition.horizontal_strips.maximum_thickness)
            );
        }

        let grid = LogicalGridSpec {
            schema_version: 1,
            width: 64,
            height: 64,
        };
        ...
        assert_ne!(
            generate_partition(&panel_heavy).unwrap(),
            generate_partition(&strip_heavy).unwrap(),
