...
@group(0) @binding(2) var out_tex: texture_storage_2d<r32uint, write>;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var i = 0u; i < header.command_count; i = i + 1u) {
        ...
        if (pixel.x >= cmd.dst_x && pixel.x < cmd.dst_x + cmd.dst_width &&
            ...
            compact = cmd.region_index;
        }
    }
    textureStore(out_tex, vec2<i32>(i32(id.x), i32(id.y)), vec4<u32>(compact, 0u, 0u, 0u));
}
