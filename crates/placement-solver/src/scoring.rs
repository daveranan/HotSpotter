...
use crate::{CandidateFamily, CandidateRoute, CandidateSet, CropCandidate, SlotDemandView};
...
impl Default for ScoringSettings {
    fn default() -> Self {
        Self {
            top_k: DEFAULT_SCORING_TOP_K,
            scale_normalization_ratio: 4.0,
...
impl Default for CandidateScoringMeasurements {
    fn default() -> Self {
        Self {
            source_pixels_per_output_pixel_milli: 1000,
            resolution_confidence_milli: 0,
            lattice_completion_milli: 0,
            structure_confidence_milli: 0,
            dominant_direction_degrees: None,
            orientation_confidence_milli: 0,
            seam_quality_milli: 0,
            seam_confidence_milli: 0,
            boundary_cut_milli: 0,
            boundary_confidence_milli: 0,
            visual_quality_milli: 1000,
            quality_confidence_milli: 0,
            role_compatibility_milli: None,
...
fn score_candidates_with_guard<S: SlotDemandView>(
    ...
    for (term, value) in &settings.weight_overrides {
        set_weight(&mut weights, *term, *value);
    }
    ...
    for candidate in candidates {
        ...
        if !reasons.is_empty() {
            rejections.push(ApplicabilityRejection {
    ...
    scored.sort_by(|a, b| {
        a.breakdown
            .total_cost
            .total_cmp(&b.breakdown.total_cost)
    ...
    let settings_hash =
        ContentDigest::sha256(format!("{context:?}|{settings:?}|{weights:?}").as_bytes());
    Ok(ScoredCandidateSet {
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance {
                algorithm_id: STAGE_12_ALGORITHM_ID.into(),
                version: STAGE_12_ALGORITHM_VERSION.into(),
            },
            settings_hash,
            diagnostics: Vec::<CompilationDiagnostic>::new(),
        },
        top_candidates: scored,
        applicability_rejections: rejections,
        legal_candidate_count: u32::try_from(legal_count).unwrap_or(u32::MAX),
        truncated_candidates: u32::try_from(legal_count.saturating_sub(settings.top_k))
            .unwrap_or(u32::MAX),
        weights,
        qa_views: vec![
            ScoringQaView::RankedCandidates,
            ScoringQaView::CostBreakdown,
            ScoringQaView::Applicability,
            ScoringQaView::TermContribution,
            ScoringQaView::RoleMaterialPolicy,
        ],
    })
}

fn validate<S: SlotDemandView>(
    slot: &S,
    context: &ScoringContext,
    settings: &ScoringSettings,
) -> Result<(), ScoringError> {
    if settings.top_k == 0
        || !settings.scale_normalization_ratio.is_finite()
        || settings.scale_normalization_ratio <= 1.0
        || !context.requested_physical_scale.is_finite()
        || context.requested_physical_scale <= 0.0
        || settings
            .weight_overrides
            .values()
            .any(|v| !v.is_finite() || *v < 0.0)
        || slot.destination_pixels().0 == 0
        || slot.destination_pixels().1 == 0
    {
    ...
    if context
        .measurements
        .values()
        .any(|m| m.dominant_direction_degrees.is_some_and(|v| !v.is_finite()))
    {
...
fn legality_rejections<S: SlotDemandView>(slot: &S, c: &CropCandidate) -> Vec<String> {
    ...
    if c.slot_id != slot.slot_id() {
        reasons.push("candidate belongs to a different slot".into());
    }
    if !c.eligibility.mapping_permitted {
        reasons.push("mapping mode was not permitted by Stage 11".into());
    }
    if !c.eligibility.transform_permitted {
        reasons.push("transform was not permitted by Stage 11".into());
    }
    ...
    if !c.eligibility.exact_aspect {
        reasons.push("candidate does not preserve the required aspect".into());
    }
    ...
    if !typed_synthesis {
        if c.crop.is_none() {
            reasons.push("direct candidate has no source crop".into());
        }
        ...
        match c.eligibility.entire_crop_usable {
            ...
            Some(false) => {}
            None => {
                reasons.push("direct candidate lacks positive whole-crop usability evidence".into())
            }
        }
    }
    if is_repeat_family(c.family) {
        match c.eligibility.cross_axis_preserved {
            ...
            None => reasons
                .push("repeat candidate lacks positive cross-axis preservation evidence".into()),
        }
    } else if c.eligibility.cross_axis_preserved == Some(false) {
        reasons.push("strip cross-axis thickness is not preserved".into());
    }
    if c.eligibility.lattice_aligned == Some(false) {
        reasons.push("required lattice alignment failed".into());
    }
    if matches!(
        c.mapping_mode,
...
fn breakdown<S: SlotDemandView>(
    slot: &S,
    c: &CropCandidate,
    context: &ScoringContext,
    m: &CandidateScoringMeasurements,
    weights: &UnaryWeights,
    settings: &ScoringSettings,
    ...
    let structured = matches!(
        context.material_behavior,
        MaterialBehaviorClass::PeriodicLatticeStructured
            | MaterialBehaviorClass::ManufacturedPattern
            | MaterialBehaviorClass::LayeredBanded
    );
    let repeatable =
        matches!(c.family, CandidateFamily::PanelSeamlessTile) || is_repeat_family(c.family);
    ...
    let orientation = orientation_cost(
        slot.orientation(),
    ...
    let values = [
        (
            ...
            1.0,
            true,
            "absolute log scale error; one at a 4x default ratio",
        ),
        (
            UnaryCostTerm::Resolution,
            resolution,
            unit(m.resolution_confidence_milli),
            true,
        ...
        (
            ...
            behavior_confidence,
            true,
            if slot.role() == TemplateSlotRole::UniqueDetail {
                "distance from controlled unique-detail stationarity"
            } else {
                "inverse measured stationarity for a generic/repeating surface"
            },
        ),
        (
            UnaryCostTerm::Saliency,
            ...
            if unique_compatible {
                "distance from controlled saliency target 0.70 for a compatible unique role"
            } else {
                "salient uniqueness penalty for a generic or incompatible role"
            },
        ),
        (
            UnaryCostTerm::Structure,
            structure_cost,
            unit(m.structure_confidence_milli) * behavior_confidence,
            structured,
            "inverse lattice/period completion",
        ),
        (
            UnaryCostTerm::Orientation,
            orientation,
            unit(m.orientation_confidence_milli) * behavior_confidence,
            is_directional(context.material_behavior)
                && slot.orientation() != RegionOrientation::Unspecified,
            "1 - abs(cos(delta)); source direction includes candidate rotation",
        ),
        (
            UnaryCostTerm::Seam,
            seam_cost,
            unit(m.seam_confidence_milli),
            repeatable,
        ...
        (
            UnaryCostTerm::BoundaryCut,
            boundary_cost,
            unit(m.boundary_confidence_milli),
        ...
        (
            UnaryCostTerm::Quality,
            quality_cost,
            unit(m.quality_confidence_milli),
            true,
            "inverse bounded visual quality; absent quality evidence has zero confidence",
        ),
        (
            UnaryCostTerm::Role,
            1.0 - role_compatibility,
            m.role_compatibility_milli
                .map(|_| unit(m.role_confidence_milli))
                .unwrap_or(1.0),
            true,
    ...
    let terms = ALL_TERMS
        ...
        .map(|(expected, value)| {
            ...
            CostTermExplanation {
                ...
                confidence,
                weight,
                weighted_cost: if applicable {
                    normalized_cost * confidence * weight
                } else {
                    0.0
                },
                applicable,
...
fn is_typed_synthesis(candidate: &CropCandidate) -> bool {
    matches!(
        (candidate.route, candidate.family),
        (
...
fn orientation_cost(
    slot: RegionOrientation,
    rotation: QuarterTurn,
    source_degrees: Option<f64>,
) -> f64 {
    let Some(source) = source_degrees else {
        return 0.0;
    };
    let target = match slot {
        RegionOrientation::Horizontal => 0.0,
        RegionOrientation::Vertical => 90.0,
        RegionOrientation::Unspecified => return 0.0,
    };
...
fn role_compatibility(
    ...
    let family_match = match role {
        TemplateSlotRole::Planar => matches!(
            family,
        ...
        TemplateSlotRole::UniqueDetail => matches!(
            ...
            CandidateFamily::UniqueContain
                | CandidateFamily::UniqueCover
                | CandidateFamily::UniquePatchBase
                | CandidateFamily::UniqueSynthesisExtension
        ),
        TemplateSlotRole::TrimCap => matches!(
            family,
            CandidateFamily::ThreeSliceCap | CandidateFamily::NineSlicePanel
        ),
    ...
    match role {
        TemplateSlotRole::UniqueDetail
            if matches!(
                behavior,
                MaterialBehaviorClass::UniqueDetail | MaterialBehaviorClass::ManufacturedPattern
            ) =>
        {
            1.0
        }
...
fn synthesis_complexity(family: CandidateFamily) -> f64 {
    match family {
        ...
        | CandidateFamily::UniqueContain
        | CandidateFamily::UniqueCover
        | CandidateFamily::ThreeSliceCap
        | CandidateFamily::NineSlicePanel
        | CandidateFamily::PlanarRadialSquare
        | CandidateFamily::PlanarRadialDetail
        | CandidateFamily::PlanarRadialAnnularProfile => 0.0,
        CandidateFamily::PanelSeamlessTile
        | CandidateFamily::RepeatXGraphCut
        | CandidateFamily::RepeatYGraphCut
        | CandidateFamily::UniquePatchBase => 0.2,
        CandidateFamily::PanelQuiltedExpansion
        | CandidateFamily::RepeatXQuilted
        | CandidateFamily::RepeatYQuilted
        | CandidateFamily::UniqueSynthesisExtension => 0.55,
...
mod tests {
    ...
    use super::*;
    use crate::{
        CandidateDescriptors, CandidateTransform, EligibilityEvidence, MirrorTransform,
    ...
    fn candidate(
        ...
        CropCandidate {
            ...
            source_id: ContentDigest::sha256(b"source"),
            domain_id: ContentDigest::sha256(b"domain"),
            slot_id: slot.id,
            ...
            correspondence_reference: ContentDigest::sha256(b"domain"),
            descriptors: CandidateDescriptors {
                saliency_milli: saliency,
                stationarity_milli: stationarity,
    ...
    fn measured(quality: u16) -> CandidateScoringMeasurements {
        CandidateScoringMeasurements {
            visual_quality_milli: quality,
            quality_confidence_milli: 1000,
            ..CandidateScoringMeasurements::default()
        }
    }

    fn context(
        behavior: MaterialBehaviorClass,
        candidates: &[(&CropCandidate, CandidateScoringMeasurements)],
    ) -> ScoringContext {
        ScoringContext {
            material_behavior: behavior,
            material_confidence_milli: 1000,
            requested_physical_scale: 1.0,
            measurements: candidates
                .iter()
                .map(|(c, m)| (c.candidate_id.clone(), m.clone()))
                .collect(),
        }
    }
    ...
    fn algorithm_stage_12_scoring_explains_role_material_rankings_and_legality() {
        ...
        let generic_result = score_candidates(
            &generic,
            &[loud.clone(), calm.clone()],
            &context(
                MaterialBehaviorClass::StochasticIsotropic,
                &[(&calm, measured(900)), (&loud, measured(900))],
            ),
            &ScoringSettings::default(),
        )
        .unwrap();
        assert_eq!(
            generic_result.top_candidates[0].candidate.candidate_id,
            calm.candidate_id
        );
        ...
        assert!(
            calm_cost
                .term(UnaryCostTerm::Stationarity)
                .unwrap()
                .normalized_cost
                < 0.1
        );
        ...
        let structured = Slot {
            id: RegionId::from_bytes([13; 16]),
            role: TemplateSlotRole::RepeatingStrip,
            orientation: RegionOrientation::Horizontal,
        };
        let mut aligned = candidate(
            &structured,
            b"aligned",
            200,
            800,
        ...
        let aligned_m = CandidateScoringMeasurements {
            lattice_completion_milli: 1000,
            structure_confidence_milli: 1000,
            dominant_direction_degrees: Some(0.0),
            orientation_confidence_milli: 1000,
            boundary_cut_milli: 0,
            boundary_confidence_milli: 1000,
            visual_quality_milli: 900,
            quality_confidence_milli: 1000,
            ..CandidateScoringMeasurements::default()
        };
        let crossed_m = CandidateScoringMeasurements {
            lattice_completion_milli: 0,
            boundary_cut_milli: 800,
            ..aligned_m.clone()
        };
        let structured_result = score_candidates(
            &structured,
            &[crossed.clone(), aligned.clone()],
            &context(
                MaterialBehaviorClass::PeriodicLatticeStructured,
                &[(&aligned, aligned_m), (&crossed, crossed_m)],
            ),
            &ScoringSettings::default(),
        )
        .unwrap();
        assert_eq!(
            structured_result.top_candidates[0].candidate.candidate_id,
            aligned.candidate_id
        );
        let crossed_cost = structured_result
            .top_candidates
            .iter()
            .find(|s| s.candidate.candidate_id == crossed.candidate_id)
            .unwrap();
        assert!(
            crossed_cost
                .breakdown
                .term(UnaryCostTerm::Orientation)
                .unwrap()
                .weighted_cost
                > 1.9
        );
        ...
        assert!(
            crossed_cost
                .breakdown
                .term(UnaryCostTerm::BoundaryCut)
                .unwrap()
                .weighted_cost
                > 1.4
        );
        ...
        let unique = Slot {
            id: RegionId::from_bytes([14; 16]),
            role: TemplateSlotRole::UniqueDetail,
        ...
        let unique_result = score_candidates(
            &unique,
            &[empty.clone(), controlled.clone()],
            &context(
                MaterialBehaviorClass::UniqueDetail,
                &[(&controlled, measured(900)), (&empty, measured(900))],
            ),
            &ScoringSettings::default(),
        )
        .unwrap();
        assert_eq!(
            unique_result.top_candidates[0].candidate.candidate_id,
            controlled.candidate_id
        );
        assert_eq!(
            unique_result.top_candidates[0]
                .breakdown
                .term(UnaryCostTerm::Saliency)
                .unwrap()
                .normalized_cost,
        ...
        let mut legal_synthesis = candidate(
            &generic,
            b"legal-synthesis",
            80,
            920,
        ...
        let gated = score_candidates(
            &generic,
            &[
                illegal_mapping.clone(),
                missing_route.clone(),
                missing_usability.clone(),
                unusable_synthesis.clone(),
                legal_synthesis.clone(),
                calm.clone(),
            ],
            &context(
                MaterialBehaviorClass::StochasticIsotropic,
        ...
        assert!(
            gated
                .top_candidates
                .iter()
                .any(|s| s.candidate.candidate_id == legal_synthesis.candidate_id)
        );
        assert_eq!(gated.applicability_rejections.len(), 4);
        let route_rejection = gated
            .applicability_rejections
            .iter()
            .find(|r| r.candidate_id == missing_route.candidate_id)
            .unwrap();
        assert!(
            route_rejection
                .reasons
                .iter()
                .any(|reason| reason.contains("direct-crop applicability"))
        );
        let usability_rejection = gated
            .applicability_rejections
            .iter()
            .find(|r| r.candidate_id == missing_usability.candidate_id)
            .unwrap();
        assert!(
            usability_rejection
                .reasons
                .iter()
                .any(|reason| reason.contains("positive whole-crop"))
        );
        let synthesis_rejection = gated
            .applicability_rejections
            .iter()
            .find(|r| r.candidate_id == unusable_synthesis.candidate_id)
            .unwrap();
        assert!(
            synthesis_rejection
                .reasons
                .iter()
                .any(|reason| reason.contains("explicit unusable"))
        );
    }
    ...
    fn algorithm_stage_12_scoring_weight_changes_and_ties_are_explainable() {
        let slot = Slot {
            id: RegionId::from_bytes([15; 16]),
            role: TemplateSlotRole::Planar,
            orientation: RegionOrientation::Unspecified,
        };
        let low_saliency = candidate(
            &slot,
            b"low-saliency",
            100,
            700,
        ...
        let ctx = context(
            MaterialBehaviorClass::StochasticIsotropic,
            &[
                (&low_saliency, measured(600)),
                (&high_quality, measured(1000)),
            ],
        );
        let base = score_candidates(
            &slot,
            &[high_quality.clone(), low_saliency.clone()],
            &ctx,
            &ScoringSettings::default(),
        )
        .unwrap();
        assert_eq!(
            base.top_candidates[0].candidate.candidate_id,
            low_saliency.candidate_id
        );
        let mut changed_settings = ScoringSettings::default();
        changed_settings
            .weight_overrides
            .insert(UnaryCostTerm::Saliency, 0.0);
        let changed = score_candidates(
            &slot,
            &[high_quality.clone(), low_saliency.clone()],
            &ctx,
            &changed_settings,
        ...
        .unwrap();
        assert_eq!(
            changed.top_candidates[0].candidate.candidate_id,
            high_quality.candidate_id
        );
        let before_saliency = base
            .top_candidates
            .iter()
            .find(|s| s.candidate.candidate_id == high_quality.candidate_id)
            .unwrap()
            .breakdown
            .term(UnaryCostTerm::Saliency)
            .unwrap();
        let after_saliency = changed.top_candidates[0]
            .breakdown
            .term(UnaryCostTerm::Saliency)
            .unwrap();
        ...
        let tie_context = context(
            MaterialBehaviorClass::StochasticIsotropic,
            &[(&tie_a, measured(900)), (&tie_b, measured(900))],
        );
        let forward = score_candidates(
            &slot,
        ...
        assert_eq!(
            forward.top_candidates[0].breakdown.total_cost,
    ...
    fn algorithm_stage_12_scoring_complexity_is_only_a_comparable_quality_preference() {
        ...
        let result = score_candidates(
            &slot,
            &[synthesis.clone(), direct.clone()],
            &equal,
            &ScoringSettings::default(),
        )
        .unwrap();
        assert_eq!(
            result.top_candidates[0].candidate.candidate_id,
            direct.candidate_id
        );
        assert_eq!(
            result.top_candidates[0]
                .breakdown
                .term(UnaryCostTerm::SynthesisComplexity)
                .unwrap()
                .normalized_cost,
            0.0
        );
        let synthesis_complexity = result
            .top_candidates
            .iter()
            .find(|s| s.candidate.candidate_id == synthesis.candidate_id)
            .unwrap()
            .breakdown
            .term(UnaryCostTerm::SynthesisComplexity)
            .unwrap();
        ...
        let missing_quality_context = context(
            MaterialBehaviorClass::StochasticIsotropic,
            &[(&direct, CandidateScoringMeasurements::default())],
        );
        let missing_quality = score_candidates(
            &slot,
            &[direct.clone()],
        ...
        .unwrap();
        let quality_term = missing_quality.top_candidates[0]
            .breakdown
            .term(UnaryCostTerm::Quality)
            .unwrap();
        assert_eq!(quality_term.confidence, 0.0);
        assert_eq!(quality_term.weighted_cost, 0.0);

        let repeat_slot = Slot {
            id: RegionId::from_bytes([17; 16]),
            role: TemplateSlotRole::RepeatingStrip,
            orientation: RegionOrientation::Horizontal,
        };
        let mut quilted_repeat = candidate(
            &repeat_slot,
            b"quilted-repeat",
            200,
            800,
        ...
        let repeat_measurement = CandidateScoringMeasurements {
            seam_quality_milli: 0,
            seam_confidence_milli: 1000,
            ..measured(900)
        };
        let repeat_context = context(
            MaterialBehaviorClass::StochasticIsotropic,
            &[(&quilted_repeat, repeat_measurement)],
        );
        let repeat_result = score_candidates(
            &repeat_slot,
            &[missing_cross_axis.clone(), quilted_repeat],
            &repeat_context,
            &ScoringSettings::default(),
        )
        .unwrap();
        ...
        assert_eq!(
            repeat_result.applicability_rejections[0].candidate_id,
            missing_cross_axis.candidate_id
        );
        assert!(
            repeat_result.applicability_rejections[0]
                .reasons
                .iter()
                .any(|reason| reason.contains("positive cross-axis"))
        );
        let seam_term = repeat_result.top_candidates[0]
            .breakdown
            .term(UnaryCostTerm::Seam)
            .unwrap();
        assert!(seam_term.applicable);
        assert!(seam_term.weighted_cost > 0.0);

        let wood_measurement = CandidateScoringMeasurements {
            boundary_cut_milli: 1000,
            boundary_confidence_milli: 1000,
            ..measured(900)
        };
        let wood_context = context(
            MaterialBehaviorClass::OrganicDirectional,
            &[(&direct, wood_measurement)],
        );
        let wood_result =
            score_candidates(&slot, &[direct], &wood_context, &ScoringSettings::default()).unwrap();
        let boundary_term = wood_result.top_candidates[0]
            .breakdown
            .term(UnaryCostTerm::BoundaryCut)
            .unwrap();
