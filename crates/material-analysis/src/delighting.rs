...
use hot_trimmer_render_core::{PreparedExemplar, PreparedExemplarChannel, RenderCancellationToken};
use thiserror::Error;
...
pub enum DelightingError {
    ...
    Cancelled,
    #[error(
        "Stage 4 requires {required_operations} operations and {required_bytes} working bytes, exceeding its declared limits"
    )]
    ...
    match &intent.route {
        ...
        DelightingRouteIntent::ClassicalLowFrequency => {
            ...
            classical(
                ...
                intent,
                cancellation,
                RouteExecution::ClassicalLowFrequency,
                Vec::new(),
            )
        }
        DelightingRouteIntent::LocalIntrinsicProvider {
            provider_id,
            fallback,
        } => {
...
fn provider_unavailable(
    ...
    let execution = RouteExecution::ProviderUnavailable {
    ...
    match fallback {
        ...
        IntrinsicProviderFallback::PassThrough => Ok(pass_through(
            exemplar,
            original.clone(),
            DelightingPassThroughReason::UserDisabled,
            Some((execution, diagnostic)),
        )),
...
fn execute_provider(
    ...
    let result = provider
        .infer(
            IntrinsicProviderRequest {
                base_color: &original,
                analyze_masks: intent.classical.analyze_masks,
            },
            cancellation,
        )
        ...
        || {
            (
                ...
                StageResult::PassThrough {
                    reason: pass_through_reason(reason).into(),
                },
            )
        },
...
fn classical(
    ...
    let count = usize::try_from(u64::from(width) * u64::from(height))
        .map_err(|_| DelightingError::InvalidSettings)?;
    let mut log_luminance = Vec::with_capacity(count);
    for y in 0..height {
        if cancellation.is_cancelled() {
            return Err(DelightingError::Cancelled);
        }
        for x in 0..width {
    ...
    let illumination = bilateral_axis(
        &horizontal,
        width,
        height,
        radius,
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            let recovery = if low < mean_illumination {
                shadow_recovery
            } else {
                highlight_recovery
            };
            ...
            let edge_attenuation =
                structural_edge_attenuation(&log_luminance, width, height, x, y, edge);
            let correction =
                ((mean_illumination - low) * strength * (1.0 + recovery) * edge_attenuation)
            ...
            let preserved = source
                .rgb
                .map(|component| (component * correction).clamp(0.0, 1.0));
            let rgb = std::array::from_fn(|component| {
                neutral[component] * (1.0 - color_preservation)
                    + preserved[component] * color_preservation
            });
            corrected.push(LinearColor {
                rgb,
                alpha: source.alpha,
            });
        }
    ...
    let masks = intent
        .classical
        .analyze_masks
        .then(|| build_masks(&original, &illumination, mean_illumination))
        .transpose()?;
    ...
    Ok(DelitPreparedExemplar {
        ...
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance {
                algorithm_id: STAGE_04_ALGORITHM_ID.into(),
                version: STAGE_04_ALGORITHM_VERSION.into(),
            },
...
fn preflight_classical(
    ...
    let bytes_per_pixel = if intent.classical.analyze_masks {
        128_u64
    } else {
        88_u64
    };
...
fn structural_edge_attenuation(
    ...
    if x > 0 {
        maximum_difference =
            maximum_difference.max((center - log_luminance[index(width, x - 1, y)]).abs());
    }
    if x + 1 < width {
        maximum_difference =
            maximum_difference.max((center - log_luminance[index(width, x + 1, y)]).abs());
    ...
    if y > 0 {
        maximum_difference =
            maximum_difference.max((center - log_luminance[index(width, x, y - 1)]).abs());
    }
    if y + 1 < height {
        maximum_difference =
            maximum_difference.max((center - log_luminance[index(width, x, y + 1)]).abs());
    ...
    .all(|plane| {
        plane.width() == width
            ...
            && plane
                ...
                .iter()
                .flat_map(|tile| &tile.pixels)
                .all(|value| value.0.is_finite() && (0.0..=1.0).contains(&value.0))
    })
}
...
fn build_masks(
    ...
    for y in 0..original.height() {
        for x in 0..original.width() {
            ...
            let shadow_value =
                smoothstep(0.35, 0.02, lum) * smoothstep(mean - 0.05, mean - 0.7, low);
            let clipped = if color
                .rgb
                .iter()
                .any(|value| *value <= EPSILON || *value >= 1.0 - EPSILON)
            {
                1.0
            } else {
                0.0
...
fn replace_base_color(
    channels: &[PreparedExemplarChannel],
    plane: ImagePlane<LinearColor>,
) -> Vec<PreparedExemplarChannel> {
    let mut replacement = Some(plane);
    channels
        .iter()
        .map(|channel| match channel {
            PreparedExemplarChannel::BaseColor { alpha_mode, .. } => {
                PreparedExemplarChannel::BaseColor {
                    plane: replacement.take().expect("one registered Base Color"),
...
fn validate_settings(intent: &DelightingIntent) -> Result<(), DelightingError> {
    ...
    if settings.strength_milli > 1000
        || settings.shadow_recovery_milli > 1000
        || settings.highlight_recovery_milli > 1000
        || settings.color_preservation_milli > 1000
...
fn resolve_radius(
    ...
    width: u32,
    height: u32,
) -> Result<u32, DelightingError> {
    let pixels = match radius {
        DelightingRadius::Pixels(value) => u32::from(value),
        DelightingRadius::RelativeBasisPoints(value) => u32::from(width.min(height))
            .saturating_mul(u32::from(value))
            .div_ceil(10_000),
        DelightingRadius::PhysicalMillimeters {
            millimeters_milli,
            pixels_per_meter_milli,
        } => u64::from(millimeters_milli)
            .saturating_mul(u64::from(pixels_per_meter_milli))
            .div_ceil(1_000_000_000) as u32,
    };
    if pixels == 0 || pixels > MAX_FILTER_RADIUS_PIXELS {
        return Err(DelightingError::InvalidSettings);
    }
    Ok(pixels)
}
...
fn classical_settings_hash(intent: &DelightingIntent, radius: u32) -> ContentDigest {
    ...
    ContentDigest::sha256(
        format!(
            ...
            settings.strength_milli,
            settings.shadow_recovery_milli,
            settings.highlight_recovery_milli,
            settings.color_preservation_milli,
            settings.edge_preservation_milli,
            settings.analyze_masks,
        )
        .as_bytes(),
    )
}

fn provider_settings_hash(
    descriptor: &IntrinsicProviderDescriptor,
    intent: &DelightingIntent,
) -> ContentDigest {
    ContentDigest::sha256(
...
fn unavailable_diagnostic(provider_id: &str) -> CompilationDiagnostic {
    CompilationDiagnostic {
        ...
        stage: Some(4),
        message: format!(
            "Local intrinsic provider '{provider_id}' is unavailable; learned inference was not executed."
        ),
...
const fn pass_through_reason(reason: DelightingPassThroughReason) -> &'static str {
    match reason {
        DelightingPassThroughReason::DefaultNewOrUnclassified => {
            "default PassThrough for new or unclassified source"
        }
        DelightingPassThroughReason::AuthoredTextureOrPbrSet => {
            "authored texture or PBR set is already de-lit"
        }
        DelightingPassThroughReason::UserDisabled => {
            "user disabled de-lighting; original prepared Base Color restored"
        }
    }
}
...
#[cfg(test)]
mod tests {
    use hot_trimmer_domain::{ClassicalDelightingSettings, MaterialChannelRole, SourceSetId};
    use hot_trimmer_image_io::{CategoryId, LinearScalar, ResolvedAlphaMode};
    use hot_trimmer_render_core::{PreparedExemplarCacheKey, PreparedExemplarScope};
    ...
    fn fixture() -> PreparedExemplar {
            ...
            for x in 0..width {
                ...
                color.push(LinearColor {
                    rgb: [
                        ...
                        structure * 0.8 * illumination,
                        structure * 0.6 * illumination,
                    ],
                    alpha: 1.0,
                });
        ...
        let plane =
            |pixels: &[LinearColor]| ImagePlane::from_row_major(width, height, 8, pixels).unwrap();
        PreparedExemplar {
            ...
            scope: PreparedExemplarScope {
                source_set_id: SourceSetId::from_bytes([4; 16]),
                source_revision: 1,
                patch_id: None,
                patch_revision: 0,
            },
            ...
            stage_result: StageResult::PassThrough {
                reason: "fixture already planar".into(),
            },
        }
    }
    ...
    fn scalar_and_ids(
        exemplar: &DelitPreparedExemplar,
    ) -> (&ImagePlane<LinearScalar>, &ImagePlane<CategoryId>) {
        let scalar = exemplar
            .channels
            .iter()
            .find_map(|channel| match channel {
        ...
        let ids = exemplar
            ...
            .iter()
            .find_map(|channel| match channel {
                PreparedExemplarChannel::MaterialId { plane } => Some(plane),
                _ => None,
            })
            .unwrap();
        (scalar, ids)
    ...
    impl LocalIntrinsicProvider for MalformedProvider {
        ...
        fn infer(
            ...
            let mask_width = if self.invalid_mask_dimensions {
                1
            } else {
                width
            };
            let mask_height = if self.invalid_mask_dimensions {
                1
            } else {
                height
            };
    ...
    fn algorithm_stage_04_delighting() {
        ...
        let default_result =
            prepare_delit_exemplar(&exemplar, &DelightingIntent::default(), None, &cancellation)
                .unwrap();
        assert_eq!(default_result.base_color(), base_color(&exemplar).unwrap());
        assert_eq!(
            default_result.original_prepared_base_color,
            *base_color(&exemplar).unwrap()
        );
        assert!(default_result.masks.is_none());
        assert!(matches!(
            default_result.route_execution,
            RouteExecution::PassThrough(DelightingPassThroughReason::DefaultNewOrUnclassified)
        ));
        let authored = DelightingIntent {
            route: DelightingRouteIntent::PassThrough {
                reason: DelightingPassThroughReason::AuthoredTextureOrPbrSet,
            },
            classical: ClassicalDelightingSettings {
                radius: DelightingRadius::Pixels(0),
                ..ClassicalDelightingSettings::default()
            },
        };
        let authored_result =
            prepare_delit_exemplar(&exemplar, &authored, None, &cancellation).unwrap();
        ...
        let first =
            prepare_delit_exemplar(&exemplar, &classical_intent(), None, &cancellation).unwrap();
        let second =
            prepare_delit_exemplar(&exemplar, &classical_intent(), None, &cancellation).unwrap();
        assert_eq!(first, second);
        assert_eq!(
            first.reflectance_provenance,
            ReflectanceProvenance::Estimated
        );
        let before_gradient = (region_mean(base_color(&exemplar).unwrap(), 56, 64)
            - region_mean(base_color(&exemplar).unwrap(), 0, 8))
        .abs();
        let after_gradient =
            (region_mean(first.base_color(), 56, 64) - region_mean(first.base_color(), 0, 8)).abs();
        assert!(
            after_gradient < before_gradient * 0.65,
            "{after_gradient} !< {before_gradient}"
        );
        let before_edge = (luminance(base_color(&exemplar).unwrap().pixel(8, 16))
            - luminance(base_color(&exemplar).unwrap().pixel(7, 16)))
        .abs();
        let after_edge = (luminance(first.base_color().pixel(8, 16))
            - luminance(first.base_color().pixel(7, 16)))
        .abs();
        assert!(
            after_edge >= before_edge * 0.7,
            "structural edge was over-smoothed"
        );
        ...
        assert!(
            masks
                ...
                .to_row_major()
                .iter()
                .any(|value| value.0 < 0.9)
        );
        let (source_scalar, source_ids) = {
            let source = DelitPreparedExemplar {
                exemplar_id: String::new(),
                original_prepared_base_color: base_color(&exemplar).unwrap().clone(),
                prepared_source_digest: exemplar.cache_key.0.clone(),
                perspective_confidence_milli: exemplar.perspective_confidence_milli,
                channels: exemplar.channels.clone(),
                coverage: None,
                ...
                upstream_stage_result: exemplar.stage_result.clone(),
                stage_result: exemplar.stage_result.clone(),
            };
        ...
        let disabled = DelightingIntent {
            route: DelightingRouteIntent::PassThrough {
                reason: DelightingPassThroughReason::UserDisabled,
            },
        ...
        assert!(
            matches!(restored.stage_result, StageResult::PassThrough { ref reason } if reason.contains("user disabled"))
        );
        ...
        let unavailable = DelightingIntent {
            route: DelightingRouteIntent::LocalIntrinsicProvider {
                provider_id: "not-installed".into(),
                fallback: IntrinsicProviderFallback::None,
            },
            classical: classical_intent().classical,
        };
        let unavailable_result =
            prepare_delit_exemplar(&exemplar, &unavailable, None, &cancellation).unwrap();
        assert_eq!(
            unavailable_result.base_color(),
            base_color(&exemplar).unwrap()
        );
        assert!(matches!(
            unavailable_result.route_execution,
            RouteExecution::ProviderUnavailable {
                fallback: IntrinsicProviderFallback::None,
                ..
            }
        ));
        assert!(matches!(
            unavailable_result.stage_result,
            StageResult::FailedWithRecovery { .. }
        ));
        ...
        assert!(matches!(
            bounded,
            Err(DelightingError::ResourceLimitExceeded { .. })
        ));
        ...
        let invalid_color = MalformedProvider {
            invalid_color: true,
            invalid_mask_dimensions: false,
        };
        assert!(matches!(
            prepare_delit_exemplar(
                &exemplar,
                &provider_intent,
                Some(&invalid_color),
                &cancellation
        ...
        let invalid_masks = MalformedProvider {
            invalid_color: false,
            invalid_mask_dimensions: true,
        };
        assert!(matches!(
            prepare_delit_exemplar(
                &exemplar,
        ...
        let valid_provider = MalformedProvider {
            invalid_color: false,
            invalid_mask_dimensions: false,
        };
        ...
        .unwrap();
        assert!(matches!(
            provider_result.route_execution,
            RouteExecution::IntrinsicProvider { .. }
        ));
        assert!(matches!(
            prepare_delit_exemplar(
                &exemplar,
