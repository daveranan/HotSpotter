...
use hot_trimmer_domain::{
    AlgorithmProvenance, CompilationDiagnostic, ContentDigest, DiagnosticCode, RecoveryChoice,
    StageResult, MaterialClassificationCommand, MaterialClassificationIntent,
};
...
impl Default for ClassificationTolerance {
    fn default() -> Self {
        Self { minimum_hard_class_confidence_milli: 350, minimum_top_margin_milli: 35 }
    }
}
...
impl MaterialClassification {
    ...
    pub const fn routed_class(&self) -> MaterialBehaviorClass {
        match self.routing_intent.override_class { Some(class) => class, None => self.analyzed_class }
    }
...
impl SourceAnalysisReport {
    ...
    pub fn inspector_evidence(&self) -> SourceInspectorEvidence {
        ...
        SourceInspectorEvidence {
            ...
            confidence_percent: u8::try_from(self.classification.confidence_milli / 10).unwrap_or(100),
            evidence_summary: format!(
                "orientation {}%, periods {}%/{}%, bands {}%, saliency {}%, radial {}%",
                m.orientation_coherence_milli / 10, m.periodic_x_milli / 10,
                m.periodic_y_milli / 10, m.bandedness_milli / 10,
                m.localized_saliency_milli / 10, m.radial_symmetry_milli / 10,
            ),
...
impl SourceAnalysisCache {
    #[must_use]
    pub fn get(&self, key: &SourceAnalysisCacheKey) -> Option<&SourceAnalysisReport> { self.entries.get(key) }
...
pub fn analyze_source(
    ...
    if width == 0 || height == 0 { return Err(SourceAnalysisError::EmptyInput); }
    let step = width.max(height).div_ceil(settings.max_analysis_edge).max(1);
    ...
    let (distribution, authority) = provider_distribution(
        provider, &quality, &measurements, &heuristic, cancellation,
    )?;
    ...
    Ok(SourceAnalysisReport {
        ...
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance {
                algorithm_id: STAGE_05_ALGORITHM_ID.into(), version: STAGE_05_ALGORITHM_VERSION.into(),
            },
...
fn validate_settings(settings: &AnalysisSettings) -> Result<(), SourceAnalysisError> {
    ...
    let all_scores = [t.minimum_sharpness_milli, t.maximum_noise_milli,
        t.maximum_compression_milli, t.minimum_dynamic_range_milli,
        t.minimum_perspective_confidence_milli, t.minimum_registration_quality_milli,
        settings.tolerance.minimum_hard_class_confidence_milli,
        settings.tolerance.minimum_top_margin_milli];
    if settings.max_analysis_edge < 16 || settings.max_analysis_edge > 1024
        || settings.max_registration_shift_px > 16
        || all_scores.into_iter().any(|value| value > SCORE_MAX)
        || t.maximum_clipped_ppm > 1_000_000 || t.minimum_usable_area_ppm > 1_000_000
    { return Err(SourceAnalysisError::InvalidSettings); }
    Ok(())
}

#[derive(Clone, Debug)]
struct SampledPlane { width: u32, height: u32, values: Vec<f32> }
...
fn sample_luminance(source: &DelitPreparedExemplar, step: u32) -> SampledPlane {
    ...
    let mut values = Vec::with_capacity(usize::try_from(u64::from(width) * u64::from(height)).unwrap_or(0));
    for sy in 0..height {
        for sx in 0..width {
            let color = base.pixel((sx * step).min(base.width() - 1), (sy * step).min(base.height() - 1));
            values.push((color.rgb[0] * 0.2126 + color.rgb[1] * 0.7152 + color.rgb[2] * 0.0722).clamp(0.0, 1.0));
        }
    }
    SampledPlane { width, height, values }
}

fn measure_quality(
    source: &DelitPreparedExemplar, sampled: &SampledPlane, step: u32, settings: &AnalysisSettings,
) -> SourceQualityReport {
    ...
    for sy in 0..sampled.height {
        for sx in 0..sampled.width {
            ...
            if color.rgb.iter().any(|value| *value <= 1.0e-5 || *value >= 1.0 - 1.0e-5) { clipped += 1; }
            let covered = source.coverage.as_ref().map_or(1.0, |mask| mask.pixel(x, y).0);
            if color.alpha > 1.0 / 255.0 && covered > 0.5 { usable += 1; }
        }
    }
    ...
    let estimated_material_resolution_px = base.width().min(base.height())
        .saturating_mul(usable_area_ppm) / 1_000_000
        * detail_factor / 1000;
    SourceQualityReport {
        sharpness_milli, noise_milli, compression_milli, dynamic_range_milli,
        clipped_pixels_ppm: fraction_ppm(clipped, total),
        perspective_confidence_milli: source.perspective_confidence_milli.min(1000),
        usable_area_ppm, registration_quality_milli, registration_worst_offset_px,
        estimated_material_resolution_px,
    }
}

fn registration_quality(source: &DelitPreparedExemplar, step: u32, max_shift: u8) -> (u16, (i8, i8)) {
    ...
    for channel in &source.channels {
        match channel {
            PreparedExemplarChannel::BaseColor { .. } | PreparedExemplarChannel::MaterialId { .. } => continue,
        ...
        for dy in -shift..=shift {
            for dx in -shift..=shift {
                ...
                if value > best + 1.0e-6 || ((value - best).abs() <= 1.0e-6 && (dx, dy) < best_offset) {
                    best = value;
                    best_offset = (dx, dy);
                }
            }
        }
        let shift_distance = u16::from(best_offset.0.unsigned_abs()) + u16::from(best_offset.1.unsigned_abs());
        let alignment = if best <= 1.0e-5 { 0.5 } else { (zero.max(0.0) / best).clamp(0.0, 1.0) };
        let channel_score = score(alignment) .saturating_sub(shift_distance.saturating_mul(90));
        if channel_score < worst_score { worst_score = channel_score; worst_offset = best_offset; }
    }
    if companion_count == 0 { (SCORE_MAX, (0, 0)) } else { (worst_score, worst_offset) }
}

fn sample_channel(channel: &PreparedExemplarChannel, step: u32, offset_x: i8, offset_y: i8) -> SampledPlane {
    ...
    for sy in 0..height { for sx in 0..width {
...
fn measure_behavior(plane: &SampledPlane) -> BehaviorMeasurements {
    ...
    BehaviorMeasurements {
        ...
        periodic_x_milli: score(px), periodic_y_milli: score(py),
...
fn heuristic_distribution(m: BehaviorMeasurements) -> Vec<RankedClassEvidence> {
    ...
    let scores = [
        ...
        (MaterialBehaviorClass::PeriodicLatticeStructured, both_periods * 0.8 + f(m.regularity_milli) * 0.2),
        (MaterialBehaviorClass::LayeredBanded, f(m.bandedness_milli) * (1.0 - both_periods * 0.5)),
        (MaterialBehaviorClass::OrganicDirectional,
            variation * (0.5 + orientation * 0.5) * (1.0 - periods * 0.5)),
        (MaterialBehaviorClass::ManufacturedPattern, periods * (0.5 + 0.5 * f(m.regularity_milli)) * (1.0 - variation * 0.4)),
        (MaterialBehaviorClass::UniqueDetail, saliency * (1.0 - periods * 0.6)),
        (MaterialBehaviorClass::RadialDetail, radial * (0.65 + saliency * 0.35)),
...
fn normalize_scores(scores: &[(MaterialBehaviorClass, f32)]) -> Vec<RankedClassEvidence> {
    ...
    let mut ranked: Vec<_> = scores.iter().map(|(class, value)| RankedClassEvidence {
        class: *class,
        probability_milli: score(value.max(0.001) / total),
        support_milli: score(*value),
    }).collect();
    let sum: i32 = ranked.iter().map(|item| i32::from(item.probability_milli)).sum();
    if let Some(first) = ranked.first_mut() {
        first.probability_milli = u16::try_from((i32::from(first.probability_milli) + 1000 - sum).clamp(0, 1000)).unwrap_or(0);
    }
    ranked.sort_by(|a, b| b.probability_milli.cmp(&a.probability_milli).then(a.class.cmp(&b.class)));
    ranked
}

fn provider_distribution(
    provider: Option<&dyn LocalClassifierProvider>, quality: &SourceQualityReport,
    measurements: &BehaviorMeasurements, heuristic: &[RankedClassEvidence],
    cancellation: &RenderCancellationToken,
) -> Result<(Vec<RankedClassEvidence>, ClassificationAuthority), SourceAnalysisError> {
    let Some(provider) = provider else { return Ok((heuristic.to_vec(), ClassificationAuthority::DeterministicHeuristics)); };
    let descriptor = provider.descriptor();
    if descriptor.interface_version != CLASSIFIER_PROVIDER_INTERFACE_VERSION {
        return Ok((heuristic.to_vec(), ClassificationAuthority::DeterministicHeuristics));
    }
    let request = ClassifierProviderRequest { quality, measurements, heuristic_distribution: heuristic };
    ...
    if output.output_version != CLASSIFIER_PROVIDER_INTERFACE_VERSION || !valid_distribution(&output.distribution) {
        return Ok((heuristic.to_vec(), ClassificationAuthority::DeterministicHeuristics));
    }
...
fn valid_distribution(distribution: &[RankedClassEvidence]) -> bool {
    ...
    distribution.len() == MaterialBehaviorClass::ALL.len()
        ...
        && distribution.iter().map(|item| u32::from(item.probability_milli)).sum::<u32>() == 1000
        && distribution.iter().all(|item| item.probability_milli <= 1000 && item.support_milli <= 1000)
        && distribution.windows(2).all(|pair| pair[0].probability_milli >= pair[1].probability_milli)
}

fn resolve_class(distribution: &[RankedClassEvidence], tolerance: ClassificationTolerance) -> (MaterialBehaviorClass, u16) {
    let Some(top) = distribution.first() else { return (MaterialBehaviorClass::MixedUnknown, 0); };
    ...
    else { (top.class, top.probability_milli) }
}

fn quality_warnings(quality: &SourceQualityReport, t: QualityThresholds) -> Vec<QualityWarning> {
    let mut warnings = Vec::new();
    let mut add = |condition, code, message: &str, recoveries| if condition { warnings.push(QualityWarning {
        code, message: message.into(), recoveries,
    }); };
    add(quality.sharpness_milli < t.minimum_sharpness_milli, QualityWarningCode::Soft,
        "Source is soft; retain physical scale and use a lower-fidelity texel-density route.",
        vec![RecoveryChoice::LowerTexelDensity, RecoveryChoice::ChooseAnotherSource]);
    add(quality.noise_milli > t.maximum_noise_milli, QualityWarningCode::Noisy,
        "Noise may be reproduced as material structure.", vec![RecoveryChoice::AdjustSettings, RecoveryChoice::UseSynthesis]);
    add(quality.compression_milli > t.maximum_compression_milli, QualityWarningCode::Compressed,
        "Block compression is visible in measurable boundary discontinuities.", vec![RecoveryChoice::ChooseAnotherSource, RecoveryChoice::LowerTexelDensity]);
    add(quality.dynamic_range_milli < t.minimum_dynamic_range_milli, QualityWarningCode::NarrowDynamicRange,
        "Source has narrow luminance range; classification evidence is weaker.", vec![RecoveryChoice::AdjustSettings]);
    add(quality.clipped_pixels_ppm > t.maximum_clipped_ppm, QualityWarningCode::Clipped,
        "Clipped samples cannot provide reliable reflectance detail.", vec![RecoveryChoice::ChooseAnotherSource, RecoveryChoice::UseSynthesis]);
    add(quality.perspective_confidence_milli < t.minimum_perspective_confidence_milli, QualityWarningCode::LowPerspectiveConfidence,
        "Planar rectification confidence is low.", vec![RecoveryChoice::AdjustSettings, RecoveryChoice::ChooseAnotherSource]);
    add(quality.usable_area_ppm < t.minimum_usable_area_ppm, QualityWarningCode::LimitedUsableArea,
        "Only a limited source area is usable; synthesis or lower density remains available.", vec![RecoveryChoice::UseSynthesis, RecoveryChoice::LowerTexelDensity]);
    add(quality.registration_quality_milli < t.minimum_registration_quality_milli, QualityWarningCode::MisregisteredChannels,
        "PBR companion maps are measurably offset from Base Color.", vec![RecoveryChoice::AdjustSettings, RecoveryChoice::ChooseAnotherSource]);
    add(quality.estimated_material_resolution_px < t.minimum_material_resolution_px, QualityWarningCode::LowMaterialResolution,
        "Estimated material resolution is below the preferred route threshold.", vec![RecoveryChoice::LowerTexelDensity, RecoveryChoice::UseSynthesis, RecoveryChoice::IncreaseOutputResolution]);
    warnings
}
...
fn warning_diagnostic(warning: &QualityWarning) -> CompilationDiagnostic {
    CompilationDiagnostic {
        ...
        stage: Some(5), message: warning.message.clone(),
...
pub fn source_analysis_cache_key(
    ...
    bytes.extend_from_slice(source.original_prepared_base_color.width().to_le_bytes().as_slice());
    bytes.extend_from_slice(source.original_prepared_base_color.height().to_le_bytes().as_slice());
    for color in source.base_color().to_row_major() {
        for component in color.rgb { bytes.extend_from_slice(&component.to_bits().to_le_bytes()); }
    ...
    match &source.coverage {
        Some(coverage) => {
            bytes.push(1);
            for value in coverage.to_row_major() { bytes.extend_from_slice(&value.0.to_bits().to_le_bytes()); }
        }
        None => bytes.push(0),
    }
    for channel in &source.channels {
        ...
        match channel {
            PreparedExemplarChannel::BaseColor { plane, .. } => for value in plane.to_row_major() {
                for component in value.rgb { bytes.extend_from_slice(&component.to_bits().to_le_bytes()); }
                bytes.extend_from_slice(&value.alpha.to_bits().to_le_bytes());
            },
            PreparedExemplarChannel::Scalar { plane, .. } => for value in plane.to_row_major() {
                bytes.extend_from_slice(&value.0.to_bits().to_le_bytes());
            },
            PreparedExemplarChannel::Normal { plane, .. } => for value in plane.to_row_major() {
                for component in value.xyz { bytes.extend_from_slice(&component.to_bits().to_le_bytes()); }
                bytes.extend_from_slice(&value.alpha.to_bits().to_le_bytes());
            },
            PreparedExemplarChannel::MaterialId { plane } => for value in plane.to_row_major() {
                bytes.extend_from_slice(&value.0.to_le_bytes());
            },
            PreparedExemplarChannel::Mask { plane, .. } => for value in plane.to_row_major() {
                bytes.extend_from_slice(&value.0.to_bits().to_le_bytes());
            },
        }
    }
...
fn gradients(plane: &SampledPlane) -> Vec<f32> {
    ...
    if plane.width < 2 || plane.height < 2 { return out; }
    for y in 0..plane.height - 1 { for x in 0..plane.width - 1 {
...
fn structure_tensor(
    plane: &SampledPlane, x0: u32, x1: u32, y0: u32, y1: u32,
) -> (f32, f32, f32) {
    let mut jxx = 0.0;
    let mut jxy = 0.0;
    let mut jyy = 0.0;
    ...
    for y in start_y..end_y { for x in start_x..end_x {
...
fn tensor_eigenvalues(jxx: f32, jxy: f32, jyy: f32) -> (f32, f32) {
    ...
    ((trace + discriminant) * 0.5, (trace - discriminant).max(0.0) * 0.5)
}

fn noise_residual(plane: &SampledPlane) -> f32 {
    if plane.width < 3 || plane.height < 3 { return 0.0; }
    let mut residual = 0.0; let mut count = 0_u32;
    for y in 1..plane.height - 1 { for x in 1..plane.width - 1 {
        let neighbors = plane.values[idx(plane.width, x - 1, y)] + plane.values[idx(plane.width, x + 1, y)]
            + plane.values[idx(plane.width, x, y - 1)] + plane.values[idx(plane.width, x, y + 1)];
        residual += (plane.values[idx(plane.width, x, y)] - neighbors * 0.25).abs(); count += 1;
    }}
    residual / count.max(1) as f32
}

fn block_discontinuity(plane: &SampledPlane) -> f32 {
    if plane.width < 9 || plane.height < 9 { return 0.0; }
    let mut boundary = 0.0; let mut boundary_count = 0_u32;
    let mut interior = 0.0; let mut interior_count = 0_u32;
    for y in 0..plane.height { for x in 1..plane.width {
        let difference = (plane.values[idx(plane.width, x, y)] - plane.values[idx(plane.width, x - 1, y)]).abs();
        if x % 8 == 0 { boundary += difference; boundary_count += 1; } else { interior += difference; interior_count += 1; }
    }}
...
fn boundary_agreement(plane: &SampledPlane) -> f32 {
    if plane.width < 2 || plane.height < 2 { return 0.0; }
    let mut seam = 0.0; let mut adjacent = 0.0; let mut count = 0_u32;
    for y in 0..plane.height {
        seam += (plane.values[idx(plane.width, 0, y)] - plane.values[idx(plane.width, plane.width - 1, y)]).abs();
        adjacent += (plane.values[idx(plane.width, 0, y)] - plane.values[idx(plane.width, 1, y)]).abs();
        adjacent += (plane.values[idx(plane.width, plane.width - 1, y)] - plane.values[idx(plane.width, plane.width - 2, y)]).abs();
        count += 1;
    }
    for x in 0..plane.width {
        seam += (plane.values[idx(plane.width, x, 0)] - plane.values[idx(plane.width, x, plane.height - 1)]).abs();
        adjacent += (plane.values[idx(plane.width, x, 0)] - plane.values[idx(plane.width, x, 1)]).abs();
        adjacent += (plane.values[idx(plane.width, x, plane.height - 1)] - plane.values[idx(plane.width, x, plane.height - 2)]).abs();
        count += 1;
    }
...
fn best_period(plane: &SampledPlane, x_axis: bool) -> f32 {
    ...
    if limit < 6 { return 0.0; }
    let max_lag = (limit / 2).min(32);
    let mut correlations = Vec::new();
    for lag in 1..=max_lag {
        let (dx, dy) = if x_axis { (lag, 0) } else { (0, lag) };
        let mut a = Vec::new(); let mut b = Vec::new();
        for y in 0..plane.height - dy { for x in 0..plane.width - dx {
            a.push(plane.values[idx(plane.width, x, y)]);
            b.push(plane.values[idx(plane.width, x + dx, y + dy)]);
        }}
        let mean_a = mean(&a); let mean_b = mean(&b);
        let mut covariance = 0.0; let mut aa = 0.0; let mut bb = 0.0;
        for (av, bv) in a.iter().zip(&b) {
            let ac = *av - mean_a; let bc = *bv - mean_b;
            covariance += ac * bc; aa += ac * ac; bb += bc * bc;
        }
        let value = if aa <= 1.0e-8 || bb <= 1.0e-8 { 0.0 } else { covariance / (aa * bb).sqrt() };
        correlations.push(value);
    }
    let mut best: f32 = 0.0;
    for index in 1..correlations.len().saturating_sub(1) {
        let value = correlations[index].clamp(0.0, 1.0);
        let adjacent = ((correlations[index - 1] + correlations[index + 1]) * 0.5).clamp(-1.0, 0.999);
    ...
    if x_axis {
        for y in 0..plane.height { for x in 1..plane.width {
            axis_change += (plane.values[idx(plane.width, x, y)] - plane.values[idx(plane.width, x - 1, y)]).abs();
            axis_count += 1;
        }}
    } else {
        for y in 1..plane.height { for x in 0..plane.width {
            axis_change += (plane.values[idx(plane.width, x, y)] - plane.values[idx(plane.width, x, y - 1)]).abs();
            axis_count += 1;
        }}
    }
    let mean_axis_change = axis_change / axis_count.max(1) as f32;
    if mean_axis_change <= 1.0e-6 { 0.0 } else { best }
}

fn stationarity(plane: &SampledPlane) -> f32 {
    if plane.width < 4 || plane.height < 4 { return 0.0; }
    let mid_x = plane.width / 2; let mid_y = plane.height / 2;
    let regions = [(0, mid_x, 0, mid_y), (mid_x, plane.width, 0, mid_y),
        (0, mid_x, mid_y, plane.height), (mid_x, plane.width, mid_y, plane.height)];
    let means: Vec<_> = regions.iter().map(|&(x0, x1, y0, y1)| region_mean(plane, x0, x1, y0, y1)).collect();
    let gradient_means: Vec<_> = regions.iter().map(|&(x0, x1, y0, y1)| {
        let mut total = 0.0;
        let mut count = 0_u32;
        for y in y0..y1.saturating_sub(1) { for x in x0..x1.saturating_sub(1) {
...
fn orientation_variation(plane: &SampledPlane) -> f32 {
    if plane.width < 8 || plane.height < 8 { return 0.0; }
    let mid_x = plane.width / 2; let mid_y = plane.height / 2;
    let regions = [(0, mid_x, 0, mid_y), (mid_x, plane.width, 0, mid_y), (0, mid_x, mid_y, plane.height), (mid_x, plane.width, mid_y, plane.height)];
...
fn radial_symmetry(plane: &SampledPlane) -> f32 {
    if plane.width < 8 || plane.height < 8 { return 0.0; }
    let cx = (plane.width - 1) as f32 * 0.5; let cy = (plane.height - 1) as f32 * 0.5;
    let bins = usize::try_from(plane.width.min(plane.height) / 2).unwrap_or(0).max(2);
    let mut sums = vec![0.0; bins]; let mut counts = vec![0_u32; bins];
    for y in 0..plane.height { for x in 0..plane.width {
        let radius = (((x as f32 - cx).powi(2) + (y as f32 - cy).powi(2)).sqrt() / cx.min(cy).max(1.0) * (bins - 1) as f32) as usize;
        if radius < bins { sums[radius] += plane.values[idx(plane.width, x, y)]; counts[radius] += 1; }
    }}
    let means: Vec<_> = sums.iter().zip(&counts)
    ...
    if total_energy <= 1.0e-8 { 0.0 } else {
...
fn correlation(a: &[f32], b: &[f32], width: u32, height: u32, dx: i8, dy: i8) -> f32 {
    ...
    for y in 0..height { for x in 0..width {
        let bx = i64::from(x) + i64::from(dx); let by = i64::from(y) + i64::from(dy);
        if bx < 0 || by < 0 || bx >= i64::from(width) || by >= i64::from(height) { continue; }
        let av = a[idx(width, x, y)]; let bv = b[idx(width, bx as u32, by as u32)];
        dot += av * bv; aa += av * av; bb += bv * bv;
    }}
    if aa <= 1.0e-9 || bb <= 1.0e-9 { 0.0 } else { dot / (aa * bb).sqrt() }
}

fn region_mean(plane: &SampledPlane, x0: u32, x1: u32, y0: u32, y1: u32) -> f32 {
    let mut total = 0.0; let mut count = 0_u32;
    for y in y0..y1 { for x in x0..x1 { total += plane.values[idx(plane.width, x, y)]; count += 1; }}
    total / count.max(1) as f32
}
...
fn mean(values: &[f32]) -> f32 { values.iter().sum::<f32>() / values.len().max(1) as f32 }
fn score(value: f32) -> u16 { (value.clamp(0.0, 1.0) * 1000.0).round() as u16 }
fn fraction_ppm(part: u64, total: u64) -> u32 { u32::try_from(part.saturating_mul(1_000_000) / total.max(1)).unwrap_or(1_000_000) }
fn idx(width: u32, x: u32, y: u32) -> usize { usize::try_from(u64::from(y) * u64::from(width) + u64::from(x)).expect("bounded sampled plane") }
fn check_cancel(cancellation: &RenderCancellationToken) -> Result<(), SourceAnalysisError> {
    if cancellation.is_cancelled() { Err(SourceAnalysisError::Cancelled) } else { Ok(()) }
}

#[cfg(test)]
mod tests {
    use hot_trimmer_domain::{
        DelightingPassThroughReason, MaterialCorpusManifest, SyntheticFixture,
        SyntheticGenerator,
    };
    ...
    fn source_from_signal(signal: &[f32], companion_offset: Option<(i8, i8)>) -> DelitPreparedExemplar {
        let colors: Vec<_> = signal.iter().map(|value| LinearColor {
            rgb: [*value, (*value * 0.85).clamp(0.0, 1.0), (*value * 0.7).clamp(0.0, 1.0)], alpha: 1.0,
        }).collect();
        let base = ImagePlane::from_row_major(WIDTH, HEIGHT, 8, &colors).unwrap();
        let mut channels = vec![PreparedExemplarChannel::BaseColor {
            plane: base.clone(), alpha_mode: ResolvedAlphaMode::Opaque,
        }];
        if let Some((dx, dy)) = companion_offset {
            ...
            for y in 0..HEIGHT { for x in 0..WIDTH {
                ...
                shifted.push(hot_trimmer_image_io::LinearScalar(signal[idx(WIDTH, sx, sy)]));
            }}
        ...
        DelitPreparedExemplar {
            ...
            channels,
            coverage: Some(ImagePlane::from_row_major(
                WIDTH, HEIGHT, 8, &vec![MaskValue(1.0); (WIDTH * HEIGHT) as usize],
            ).unwrap()),
            masks: None,
            ...
            upstream_stage_result: StageResult::PassThrough { reason: "already planar".into() },
            stage_result: StageResult::PassThrough { reason: "already de-lit".into() },
        }
    }
            ...
            values[i] = match class {
                ...
                MaterialBehaviorClass::StochasticDirectional => {
                    ...
                    for offset in -3_i64..=3 {
                        let sample_y = (i64::from(y) + offset).rem_euclid(i64::from(HEIGHT)) as u32;
                        smoothed += stochastic[idx(WIDTH, x, sample_y)];
                    }
                    smoothed / 7.0
                }
                MaterialBehaviorClass::PeriodicLatticeStructured => lattice[i],
                MaterialBehaviorClass::LayeredBanded => if y % 12 < 5 { 0.78 } else { 0.22 },
                MaterialBehaviorClass::OrganicDirectional => {
                    let mut smoothed = 0.0;
                    for offset in -3_i64..=3 {
                        let (sample_x, sample_y) = if x < WIDTH / 2 {
                            (x, (i64::from(y) + offset).rem_euclid(i64::from(HEIGHT)) as u32)
                        } else {
                            ((i64::from(x) + offset).rem_euclid(i64::from(WIDTH)) as u32, y)
                        };
                ...
                MaterialBehaviorClass::ManufacturedPattern => {
                    ...
                    if motif_x < 3 { 0.8 - row_noise * 0.2 } else { 0.15 + row_noise * 0.25 }
                }
                MaterialBehaviorClass::UniqueDetail => {
                    let dx = i64::from(x) - 23; let dy = i64::from(y) - 37;
                    if dx * dx + dy * dy <= 64 { 0.95 } else { 0.28 + stochastic[i] * 0.08 }
                }
                MaterialBehaviorClass::RadialDetail => {
                    let dx = x as f32 - 31.5; let dy = y as f32 - 31.5;
                    0.5 + ((dx * dx + dy * dy).sqrt() * 0.55).sin() * 0.35
                }
                MaterialBehaviorClass::MixedUnknown => {
                    if x < WIDTH / 2 { lattice[i] } else { 0.495 + stochastic[i] * 0.01 }
                }
            };
        }}
        if class == MaterialBehaviorClass::AlreadyTileable {
            for y in 0..HEIGHT { values[idx(WIDTH, WIDTH - 1, y)] = values[idx(WIDTH, 0, y)]; }
            for x in 0..WIDTH { values[idx(WIDTH, x, HEIGHT - 1)] = values[idx(WIDTH, x, 0)]; }
        }
        values
    }

    fn class_support(report: &SourceAnalysisReport, class: MaterialBehaviorClass) -> u16 {
        report.classification.distribution.iter().find(|entry| entry.class == class).unwrap().support_milli
    }

    fn fixture_signal(fixture: &SyntheticFixture) -> Vec<f32> {
        fixture.planes["base_color"].iter().map(|value| f32::from(*value) / 65_535.0).collect()
    }
    ...
    fn source_from_fixture(fixture: &SyntheticFixture) -> DelitPreparedExemplar {
        ...
        for ((_, values), role) in fixture.planes.iter()
            .filter(|(name, _)| name.as_str() != "base_color")
            .zip(roles)
        {
            let pixels: Vec<_> = values.iter().map(|value| {
                hot_trimmer_image_io::LinearScalar(f32::from(*value) / 65_535.0)
            }).collect();
    ...
    fn dominant_or_ambiguous_error(
        ...
        if intended == MaterialBehaviorClass::MixedUnknown {
            return (report.classification.analyzed_class != MaterialBehaviorClass::MixedUnknown)
                .then(|| format!("{intended:?}: expected explicit MixedUnknown, got {:?}", report.classification.analyzed_class));
        }
        if report.classification.analyzed_class == intended { return None; }
        if report.classification.analyzed_class != MaterialBehaviorClass::MixedUnknown {
            return Some(format!("{intended:?}: hard class {:?} displaced intended class", report.classification.analyzed_class));
        }
        let rank = report.classification.distribution.iter()
            .position(|entry| entry.class == intended)
            .expect("complete distribution");
        if rank > 2 { return Some(format!("{intended:?}: intended class rank {rank} is outside the ambiguity set")); }
        let top = report.classification.distribution[0];
        let intended_evidence = report.classification.distribution[rank];
        if !(
            top.class == MaterialBehaviorClass::MixedUnknown
                || top.probability_milli.saturating_sub(intended_evidence.probability_milli)
                < tolerance.minimum_top_margin_milli
                || top.support_milli < tolerance.minimum_hard_class_confidence_milli
        ) {
            return Some(format!("{intended:?}: MixedUnknown is outside declared margin/support tolerance"));
        }
        None
    }

    struct InvalidProvider { interface_version: u16 }
    impl LocalClassifierProvider for InvalidProvider {
        fn descriptor(&self) -> ClassifierProviderDescriptor {
            ClassifierProviderDescriptor {
                provider_id: "invalid-local".into(), provider_version: "1".into(),
        ...
        fn classify(
            &self, _request: ClassifierProviderRequest<'_>, _cancellation: &RenderCancellationToken,
        ) -> Result<ClassifierProviderOutput, ClassifierProviderError> {
            Ok(ClassifierProviderOutput { distribution: Vec::new(), output_version: CLASSIFIER_PROVIDER_INTERFACE_VERSION })
        }
    }
    ...
    impl LocalClassifierProvider for CancellingProvider {
        fn descriptor(&self) -> ClassifierProviderDescriptor {
            ClassifierProviderDescriptor {
                provider_id: "cancel-local".into(), provider_version: "1".into(),
    ...
    impl LocalClassifierProvider for UnboundedProvider {
        fn descriptor(&self) -> ClassifierProviderDescriptor {
            ClassifierProviderDescriptor {
                provider_id: "unbounded-local".into(), provider_version: "1".into(),
                interface_version: CLASSIFIER_PROVIDER_INTERFACE_VERSION,
                model_digest: ContentDigest::sha256(b"unbounded-local"),
            }
        }
        fn classify(
            &self, request: ClassifierProviderRequest<'_>, _cancellation: &RenderCancellationToken,
        ) -> Result<ClassifierProviderOutput, ClassifierProviderError> {
            let mut distribution = request.heuristic_distribution.to_vec();
            distribution[0].support_milli = 1001;
            Ok(ClassifierProviderOutput { distribution, output_version: CLASSIFIER_PROVIDER_INTERFACE_VERSION })
        }
    }
    ...
    fn algorithm_stage_05_quality_classification() {
        ...
        assert_eq!(first.classification.distribution.len(), MaterialBehaviorClass::ALL.len());
        assert_eq!(first.classification.distribution.iter().map(|entry| u32::from(entry.probability_milli)).sum::<u32>(), 1000);
        assert!(first.classification.distribution.windows(2).all(|pair| pair[0].probability_milli >= pair[1].probability_milli));
        ...
        let corpus_ids: BTreeSet<_> = corpus.behavior_classes.iter().map(|entry| entry.id.clone()).collect();
        let typed_ids: BTreeSet<_> = MaterialBehaviorClass::ALL.into_iter().map(|class| {
            serde_json::to_string(&class).unwrap().trim_matches('"').to_owned()
        }).collect();
        ...
        let generated_fixtures: Vec<_> = corpus.synthetic_fixtures.iter().map(|spec| spec.generate()).collect();
        ...
        for fixture in &generated_fixtures {
            assert_eq!(fixture.planes["base_color"].len(), usize::try_from(fixture.spec.width * fixture.spec.height).unwrap());
        ...
        let fixture_report = |generator| &fixture_reports.iter()
            .find(|(candidate, _)| *candidate == generator)
            .expect("complete Stage 0 fixture set").1;
        assert!(fixture_report(SyntheticGenerator::Structure).classification.measurements.localized_saliency_milli > 300);
        assert!(fixture_report(SyntheticGenerator::Orientation).classification.measurements.orientation_coherence_milli > 600,
            "oblique corpus orientation lost tensor coherence");
        let periodic = fixture_report(SyntheticGenerator::Periodicity).classification.measurements;
        assert!(periodic.periodic_x_milli > 800 && periodic.periodic_y_milli > 800);
        assert!(fixture_report(SyntheticGenerator::Saliency).classification.measurements.localized_saliency_milli
            > fixture_report(SyntheticGenerator::Registration).classification.measurements.localized_saliency_milli + 100);
        ...
        let reports: BTreeMap<_, _> = MaterialBehaviorClass::ALL.into_iter().map(|class| {
            let report = analyze_source(&source_from_signal(&behavior_signal(class), None), &settings, None, &cancellation).unwrap();
            (class, report)
        }).collect();
        let measurement = |class| reports[&class].classification.measurements;
        eprintln!("intended | analyzed | top three probability/support | boundary orientation variation px py bands regularity saliency radial stationarity | MixedUnknown reason");
        for class in MaterialBehaviorClass::ALL {
            ...
            let mixed_reason = if report.classification.analyzed_class != MaterialBehaviorClass::MixedUnknown {
                ...
                format!("top support {} < {}", top.support_milli, settings.tolerance.minimum_hard_class_confidence_milli)
            } else {
                format!(
                    "top margin {} < {}",
                    top.probability_milli.saturating_sub(second.probability_milli),
                    settings.tolerance.minimum_top_margin_milli,
                )
            };
            ...
            eprintln!(
                "{:?} | {:?} | {:?} {}/{}; {:?} {}/{}; {:?} {}/{} | {} {} {} {} {} {} {} {} {} {} | {}",
                class, report.classification.analyzed_class,
                top.class, top.probability_milli, top.support_milli,
                second.class, second.probability_milli, second.support_milli,
                third.class, third.probability_milli, third.support_milli,
                m.boundary_agreement_milli, m.orientation_coherence_milli,
                m.orientation_variation_milli, m.periodic_x_milli, m.periodic_y_milli,
                m.bandedness_milli, m.regularity_milli, m.localized_saliency_milli,
                m.radial_symmetry_milli, m.stationarity_milli, mixed_reason,
            );
        }
        ...
        let measurement_checks = [
            ...
            (measurement(MaterialBehaviorClass::StochasticIsotropic).orientation_coherence_milli < 300,
                "StochasticIsotropic: orientation coherence must be below 300"),
            (measurement(MaterialBehaviorClass::StochasticDirectional).orientation_coherence_milli > 600,
                "StochasticDirectional: orientation coherence must exceed 600"),
            (measurement(MaterialBehaviorClass::PeriodicLatticeStructured).periodic_x_milli > 800
                && measurement(MaterialBehaviorClass::PeriodicLatticeStructured).periodic_y_milli > 800,
            ...
            (measurement(MaterialBehaviorClass::OrganicDirectional).orientation_variation_milli > 20,
        ...
        calibration_failures.extend(measurement_checks.into_iter()
            .filter_map(|(passed, message)| (!passed).then(|| message.to_owned())));
        for class in MaterialBehaviorClass::ALL {
            let report = &reports[&class];
            if !report.classification.distribution.iter().any(|entry| entry.class == class) {
            ...
            match class {
                ...
                | MaterialBehaviorClass::UniqueDetail => {
                    if report.classification.analyzed_class != class {
                        calibration_failures.push(format!(
                            "{class:?}: nonambiguous fixture analyzed as {:?}", report.classification.analyzed_class));
                    }
                }
                MaterialBehaviorClass::PeriodicLatticeStructured => {
                    if report.classification.analyzed_class == MaterialBehaviorClass::MixedUnknown {
                        if let Some(error) = dominant_or_ambiguous_error(report, class, settings.tolerance) {
                            calibration_failures.push(error);
                        }
                    } else if !matches!(report.classification.analyzed_class,
                        MaterialBehaviorClass::PeriodicLatticeStructured | MaterialBehaviorClass::ManufacturedPattern)
                    {
                        calibration_failures.push(format!("{class:?}: unacceptable overlap class {:?}", report.classification.analyzed_class));
                    }
                }
                MaterialBehaviorClass::ManufacturedPattern => {
                    if report.classification.analyzed_class == MaterialBehaviorClass::MixedUnknown {
                ...
                MaterialBehaviorClass::MixedUnknown => {
                    if report.classification.analyzed_class != MaterialBehaviorClass::MixedUnknown {
                        calibration_failures.push(format!("MixedUnknown: analyzed as {:?}", report.classification.analyzed_class));
                    }
                }
            }
        }
        if class_support(&reports[&MaterialBehaviorClass::PeriodicLatticeStructured], MaterialBehaviorClass::PeriodicLatticeStructured)
            <= class_support(&first, MaterialBehaviorClass::PeriodicLatticeStructured)
        {
            calibration_failures.push("Periodic lattice support must exceed stochastic baseline".into());
        }
        assert!(calibration_failures.is_empty(), "behavior calibration failures:\n{}", calibration_failures.join("\n"));
        ...
        assert_eq!(ambiguous.classification.analyzed_class, MaterialBehaviorClass::MixedUnknown);
        let measured_quality = ambiguous.quality.clone();
        let measured_distribution = ambiguous.classification.distribution.clone();
        ambiguous.apply_command(ClassificationCommand::Override { class: MaterialBehaviorClass::OrganicDirectional });
        assert_eq!(ambiguous.classification.routed_class(), MaterialBehaviorClass::OrganicDirectional);
        assert_eq!(ambiguous.classification.analyzed_class, MaterialBehaviorClass::MixedUnknown);
        ...
        assert_eq!(ambiguous.classification.routed_class(), MaterialBehaviorClass::MixedUnknown);

        // Every report-changing input participates in the cache key.
        let registration_fixture = generated_fixtures.iter()
        ...
        let aligned = analyze_source(&source_from_fixture(registration_fixture), &settings, None, &cancellation).unwrap();
        let companion_changed = analyze_source(&source_from_signal(&registration_signal, Some((3, -2))), &settings, None, &cancellation).unwrap();
        assert_ne!(aligned.cache_key, companion_changed.cache_key);
        let mut coverage_changed_source = stochastic.clone();
        coverage_changed_source.coverage = Some(ImagePlane::from_row_major(
            WIDTH, HEIGHT, 8, &vec![MaskValue(0.0); (WIDTH * HEIGHT) as usize],
        ).unwrap());
        let coverage_changed = analyze_source(&coverage_changed_source, &settings, None, &cancellation).unwrap();
        ...
        let perspective_changed = analyze_source(&perspective_changed_source, &settings, None, &cancellation).unwrap();
        assert_ne!(first.cache_key, perspective_changed.cache_key);
        let provider_v1 = InvalidProvider { interface_version: CLASSIFIER_PROVIDER_INTERFACE_VERSION };
        let provider_v2 = InvalidProvider { interface_version: CLASSIFIER_PROVIDER_INTERFACE_VERSION + 1 };
        assert_ne!(
            analyze_source(&stochastic, &settings, Some(&provider_v1), &cancellation).unwrap().cache_key,
            analyze_source(&stochastic, &settings, Some(&provider_v2), &cancellation).unwrap().cache_key,
        );
        ...
        let downsampled_settings = AnalysisSettings { max_analysis_edge: 32, ..settings };
        let shifted = analyze_source(
            &source_from_signal(&registration_signal, Some((3, -2))), &downsampled_settings, None, &cancellation,
        ).unwrap();
        assert_eq!(shifted.quality.registration_worst_offset_px, (3, -2));
        assert!(shifted.quality.registration_quality_milli < settings.thresholds.minimum_registration_quality_milli);
        assert!(shifted.warnings.iter().any(|warning| warning.code == QualityWarningCode::MisregisteredChannels
        ...
        let fallback = analyze_source(&stochastic, &settings, Some(&provider_v1), &cancellation).unwrap();
        assert_eq!(fallback.classification.authority, ClassificationAuthority::DeterministicHeuristics);
        assert_eq!(fallback.classification.distribution, first.classification.distribution);
        let unbounded = analyze_source(&stochastic, &settings, Some(&UnboundedProvider), &cancellation).unwrap();
        assert_eq!(unbounded.classification.authority, ClassificationAuthority::DeterministicHeuristics);
        assert!(matches!(
            analyze_source(&stochastic, &settings, Some(&CancellingProvider), &cancellation),
