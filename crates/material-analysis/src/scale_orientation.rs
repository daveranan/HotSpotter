...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum OverlayCoordinateSpace { SourcePixels }
...
impl ScaleOrientationCache {
    #[must_use]
    pub fn get(&self, key: &ContentDigest) -> Option<&ScaleOrientationReport> { self.entries.get(key) }
...
pub enum ScaleOrientationError {
    ...
    Cancelled,
    #[error("Stage 6 requires {required_bytes} working bytes and {required_operations} operations, exceeding its declared limits")]
...
pub fn calibrate_scale_orientation(
    ...
    if base.width() < 3 || base.height() < 3 { return Err(ScaleOrientationError::EmptyInput); }
    preflight(base.width(), base.height(), settings)?;

    let mut luminance = Vec::with_capacity((u64::from(base.width()) * u64::from(base.height())) as usize);
    ...
    for y in 0..base.height() {
        ...
        for x in 0..base.width() {
            ...
            valid.push(match source.coverage.as_ref() {
                Some(coverage) if coverage.width() == base.width() && coverage.height() == base.height() => {
    ...
    let gradients = scharr_gradients(
        base.width(), base.height(), &luminance, &valid, cancellation,
    )?;
    drop(luminance);
    drop(valid);
    ...
    let measured_confidence = ((u32::from(global_anisotropy) * u32::from(energy_confidence)) / 1000) as u16;
    let measured_axis = (measured_confidence >= settings.orientation_confidence_threshold_milli)
        .then_some(measured_axis);
    let (axis, authority, destructive_rotation_allowed) = if let Some(overridden) = intent.orientation_override_millidegrees {
    ...
    let local_orientation = local_field(base.width(), base.height(), &integral, settings, cancellation)?;
    ...
    let diagnostics = scale_diagnostics.iter().map(|diagnostic| CompilationDiagnostic {
    ...
    Ok(ScaleOrientationReport {
        ...
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance {
                algorithm_id: STAGE_06_ALGORITHM_ID.into(), version: STAGE_06_ALGORITHM_VERSION.into(),
            },
...
fn validate_settings(settings: &ScaleOrientationSettings) -> Result<(), ScaleOrientationError> {
    if !(1..=32).contains(&settings.local_grid_x) || !(1..=32).contains(&settings.local_grid_y)
    ...
    { Err(ScaleOrientationError::InvalidSettings) } else { Ok(()) }
}

fn preflight(width: u32, height: u32, settings: &ScaleOrientationSettings) -> Result<(), ScaleOrientationError> {
    let pixels = u64::from(width).checked_mul(u64::from(height))
        .ok_or(ScaleOrientationError::ResourceLimitExceeded { required_bytes: u64::MAX, required_operations: u64::MAX })?;
    let integral_pixels = u64::from(width + 1).checked_mul(u64::from(height + 1))
        .ok_or(ScaleOrientationError::ResourceLimitExceeded { required_bytes: u64::MAX, required_operations: u64::MAX })?;
    // Peak: gradients plus four f64/count integral planes; luminance/coverage are dropped first.
    let required_bytes = pixels.checked_mul(16)
    ...
    let required_operations = pixels.checked_mul(96)
        .and_then(|value| value.checked_add(cells.checked_mul(96)?))
        .ok_or(ScaleOrientationError::ResourceLimitExceeded { required_bytes, required_operations: u64::MAX })?;
    if required_bytes > settings.max_working_bytes || required_operations > settings.max_operations {
        Err(ScaleOrientationError::ResourceLimitExceeded { required_bytes, required_operations })
    } else { Ok(()) }
}

#[derive(Clone, Copy, Debug, Default)]
struct GradientSample { x: f32, y: f32, valid: bool }

fn scharr_gradients(
    width: u32, height: u32, values: &[f32], valid: &[bool],
    cancellation: &RenderCancellationToken,
    ...
    for y in 1..height - 1 {
        if y % 32 == 0 { check_cancel(cancellation)?; }
        for x in 1..width - 1 {
            let neighborhood_valid = (y - 1..=y + 1).all(|sy| {
                (x - 1..=x + 1).all(|sx| valid[(sy * width + sx) as usize])
            });
            if !neighborhood_valid { continue; }
            ...
            out[(y * width + x) as usize] = GradientSample { x: gx * cap, y: gy * cap, valid: true };
        }
    }
    Ok(out)
}
...
#[derive(Clone, Copy, Debug, Default)]
struct TensorStats { jxx: f64, jxy: f64, jyy: f64, sample_count: f64 }

impl TensorStats {
    fn tensor(self) -> (f64, f64, f64) { (self.jxx, self.jxy, self.jyy) }
}
...
impl TensorIntegral {
    fn build(
        width: u32, height: u32, gradients: &[GradientSample],
        cancellation: &RenderCancellationToken,
    ) -> Result<Self, ScaleOrientationError> {
        let stride = width + 1;
        let len = usize::try_from(u64::from(stride) * u64::from(height + 1))
            .map_err(|_| ScaleOrientationError::ResourceLimitExceeded { required_bytes: u64::MAX, required_operations: u64::MAX })?;
        let mut result = Self { width, height, jxx: vec![0.0; len], jxy: vec![0.0; len], jyy: vec![0.0; len], count: vec![0.0; len] };
        for y in 0..height {
            if y % 32 == 0 { check_cancel(cancellation)?; }
            for x in 0..width {
                let gradient = gradients[(y * width + x) as usize];
                let values = if gradient.valid {
                    [f64::from(gradient.x * gradient.x), f64::from(gradient.x * gradient.y), f64::from(gradient.y * gradient.y), 1.0]
                } else { [0.0; 4] };
                let index = ((y + 1) * stride + x + 1) as usize;
                let left = index - 1;
                let up = index - stride as usize;
                let diagonal = up - 1;
                result.jxx[index] = values[0] + result.jxx[left] + result.jxx[up] - result.jxx[diagonal];
                result.jxy[index] = values[1] + result.jxy[left] + result.jxy[up] - result.jxy[diagonal];
                result.jyy[index] = values[2] + result.jyy[left] + result.jyy[up] - result.jyy[diagonal];
                result.count[index] = values[3] + result.count[left] + result.count[up] - result.count[diagonal];
            }
        }
        Ok(result)
    }
    ...
    fn query(&self, x0: u32, x1: u32, y0: u32, y1: u32) -> TensorStats {
        let x0 = x0.min(self.width); let x1 = x1.min(self.width);
        let y0 = y0.min(self.height); let y1 = y1.min(self.height);
        let stride = self.width + 1;
        let indices = [
            (y1 * stride + x1) as usize, (y1 * stride + x0) as usize,
            (y0 * stride + x1) as usize, (y0 * stride + x0) as usize,
        ];
        let sum = |plane: &[f64]| plane[indices[0]] - plane[indices[1]] - plane[indices[2]] + plane[indices[3]];
        TensorStats { jxx: sum(&self.jxx), jxy: sum(&self.jxy), jyy: sum(&self.jyy), sample_count: sum(&self.count) }
    }
}

fn tensor_axis((jxx, jxy, jyy): (f64, f64, f64)) -> (u32, u16, f64) {
    let trace = jxx + jyy;
    if trace <= 1.0e-12 { return (0, 0, 0.0); }
    ...
    let axis = normalize_axis_millidegrees((gradient_angle + std::f64::consts::FRAC_PI_2).to_degrees());
    (axis, (anisotropy * 1000.0).round() as u16, trace)
}

fn energy_confidence(energy: f64, sample_count: f64) -> u16 {
    if sample_count <= 0.0 { return 0; }
    let mean = energy / sample_count;
    ((mean / (mean + 0.02)).clamp(0.0, 1.0) * 1000.0).round() as u16
}

fn local_field(
    width: u32, height: u32, integral: &TensorIntegral, settings: &ScaleOrientationSettings,
    cancellation: &RenderCancellationToken,
) -> Result<Vec<LocalOrientationSample>, ScaleOrientationError> {
    let mut result = Vec::with_capacity(usize::from(settings.local_grid_x) * usize::from(settings.local_grid_y));
    ...
    for gy in 0..u32::from(settings.local_grid_y) {
        ...
        for gx in 0..u32::from(settings.local_grid_x) {
            let cx = ((f64::from(gx) + 0.5) * cell_w).round().clamp(1.0, f64::from(width - 2)) as u32;
            let cy = ((f64::from(gy) + 0.5) * cell_h).round().clamp(1.0, f64::from(height - 2)) as u32;
            ...
            for multiplier in [1_u32, 2, 4] {
                ...
                let tensor = integral.query(
                    cx.saturating_sub(radius).max(1), (cx + radius + 1).min(width - 1),
                    cy.saturating_sub(radius).max(1), (cy + radius + 1).min(height - 1),
                );
            ...
            let confidence = ((u32::from(anisotropy) * u32::from(energy_confidence(energy, weighted_samples))) / 1000) as u16;
            result.push(LocalOrientationSample {
                source_x_milli: u64::from(cx) * 1000,
                source_y_milli: u64::from(cy) * 1000,
                axis_millidegrees: (confidence >= settings.orientation_confidence_threshold_milli).then_some(axis),
...
fn scale_diagnostics(scale: PhysicalScaleEvidence, tolerance_milli: u16) -> Vec<ScaleDiagnostic> {
    ...
    match scale.world_scale {
        WorldScaleAvailability::UnavailableRelativeOnly => diagnostics.push(ScaleDiagnostic {
            ...
            message: "Relative material scale is usable, but world-size claims are unavailable.".into(),
        }),
        WorldScaleAvailability::UnavailablePriorEstimate => diagnostics.push(ScaleDiagnostic {
            ...
            message: "The material-class prior is only an estimate; world-size claims are unavailable.".into(),
        }),
        WorldScaleAvailability::Available => {}
    }
    if let (Some(x), Some(y)) = (
        scale.source_pixels_per_meter_x_milli, scale.source_pixels_per_meter_y_milli,
    ) {
...
fn scale_label(scale: PhysicalScaleEvidence) -> String {
    match scale.world_scale {
        ...
        WorldScaleAvailability::UnavailablePriorEstimate => "Prior estimate only — world scale unavailable".into(),
        WorldScaleAvailability::UnavailableRelativeOnly => "Relative scale only — world scale unavailable".into(),
    }
}

fn report_key(
    stage_five: &SourceAnalysisReport, intent: &MaterialCalibrationIntent,
    settings: &ScaleOrientationSettings, footprint: bool,
) -> ContentDigest {
    let scale = intent.scale;
    let bytes = format!(
        "{}|{}|{}|{:?}|{}|{:?}|{}|{}|{}|{}|{}|{}|{}|{}|{}",
        STAGE_06_ALGORITHM_VERSION, stage_five.cache_key.0.0,
        scale.source_pixels_per_meter_x_milli.unwrap_or(0),
        scale.source_pixels_per_meter_y_milli,
        scale.confidence_milli, scale.provenance, intent.revision,
        intent.orientation_override_millidegrees.unwrap_or(u32::MAX),
        settings.local_grid_x, settings.local_grid_y,
        settings.orientation_confidence_threshold_milli,
        settings.anisotropic_scale_tolerance_milli,
        settings.max_working_bytes, settings.max_operations, footprint,
    );
    ContentDigest::sha256(bytes.as_bytes())
}

fn settings_hash(settings: &ScaleOrientationSettings) -> ContentDigest {
    ContentDigest::sha256(format!("{}|{}|{}|{}|{}|{}", settings.local_grid_x, settings.local_grid_y,
        settings.orientation_confidence_threshold_milli, settings.anisotropic_scale_tolerance_milli,
        settings.max_working_bytes, settings.max_operations).as_bytes())
}

fn check_cancel(cancellation: &RenderCancellationToken) -> Result<(), ScaleOrientationError> {
    if cancellation.is_cancelled() { Err(ScaleOrientationError::Cancelled) } else { Ok(()) }
}
...
mod tests {
    ...
    use crate::{
        AnalysisSettings, ReflectanceProvenance, RouteExecution, analyze_source,
    };
    use super::*;
    ...
    fn source_with_coverage(width: u32, height: u32, signal: &[f32], coverage: &[bool]) -> DelitPreparedExemplar {
        let colors: Vec<_> = signal.iter().map(|value| LinearColor { rgb: [*value; 3], alpha: 1.0 }).collect();
        ...
        DelitPreparedExemplar {
            ...
            coverage: Some(ImagePlane::from_row_major(width, height, 8, &coverage.iter().map(|valid| MaskValue(if *valid { 1.0 } else { 0.0 })).collect::<Vec<_>>()).unwrap()),
            masks: None,
            ...
            stage_result: StageResult::PassThrough { reason: "de-lit".into() },
        }
    }
    ...
    fn algorithm_stage_06_scale_orientation() {
        ...
        let directional = corpus.synthetic_fixtures.iter()
            .find(|fixture| fixture.generator == SyntheticGenerator::Orientation).unwrap().generate();
        let values: Vec<_> = directional.planes["base_color"].iter().map(|value| f32::from(*value) / 65_535.0).collect();
        let directional_source = source(directional.spec.width, directional.spec.height, &values);
        let stage_five = analyze_source(&directional_source, &AnalysisSettings::default(), None, &cancellation).unwrap();
        let report = calibrate_scale_orientation(
            &directional_source, &stage_five, &MaterialCalibrationIntent::default(), &settings, &cancellation,
        ).unwrap();
        let expected = ((directional.spec.expected_properties["rise"] as f64)
            .atan2(directional.spec.expected_properties["run"] as f64).to_degrees() * 1000.0).round() as u32;
        assert!(axis_error(report.global_orientation.axis_millidegrees.unwrap(), expected) <= 5_000,
            "directional corpus axis {:?} must be within 5 degrees of {expected}", report.global_orientation);
        assert!(report.global_orientation.confidence_milli >= settings.orientation_confidence_threshold_milli);
        assert!(report.local_orientation.iter().filter(|sample| sample.axis_millidegrees.is_some()).count() > report.local_orientation.len() / 2);
        ...
        for (width, height) in [(64, 64), (96, 48), (48, 96)] {
            for seed in [1, 2, 5, 17] {
                ...
                let noisy_stage_five = analyze_source(&noisy_source, &AnalysisSettings::default(), None, &cancellation).unwrap();
                let noisy = calibrate_scale_orientation(
                    &noisy_source, &noisy_stage_five, &MaterialCalibrationIntent::default(), &settings, &cancellation,
                ).unwrap();
                assert_eq!(noisy.global_orientation.axis_millidegrees, None,
                    "2D noise seed {seed} at {width}x{height} must not produce an axis: {:?}", noisy.global_orientation);
                assert!(!noisy.global_orientation.destructive_rotation_allowed);
                assert_eq!(noisy.scale_diagnostics.iter().filter(|d| d.code == ScaleDiagnosticCode::InconsistentAnisotropicScale).count(), 0,
        ...
        for y in 0..64 { for x in 32..64 {
            padded[y * 64 + x] = if y % 2 == 0 { 1.0 } else { 0.0 };
            covered[y * 64 + x] = false;
        }}
        let padded_source = source_with_coverage(64, 64, &padded, &covered);
        let padded_stage_five = analyze_source(&padded_source, &AnalysisSettings::default(), None, &cancellation).unwrap();
        let padded_report = calibrate_scale_orientation(
            &padded_source, &padded_stage_five, &MaterialCalibrationIntent::default(), &settings, &cancellation,
        ).unwrap();
        assert_eq!(padded_report.global_orientation.axis_millidegrees, None);
        assert!(!padded_report.global_orientation.destructive_rotation_allowed);

        // Local energy normalization follows the weighted window samples, not full image size.
        let grid_four = ScaleOrientationSettings { local_grid_x: 4, local_grid_y: 4, ..settings };
        let grid_eight = ScaleOrientationSettings { local_grid_x: 8, local_grid_y: 8, ..settings };
        let four = calibrate_scale_orientation(&directional_source, &stage_five, &MaterialCalibrationIntent::default(), &grid_four, &cancellation).unwrap();
        let eight = calibrate_scale_orientation(&directional_source, &stage_five, &MaterialCalibrationIntent::default(), &grid_eight, &cancellation).unwrap();
        let mean_confidence = |samples: &[LocalOrientationSample]| -> u32 {
            samples.iter().map(|sample| u32::from(sample.confidence_milli)).sum::<u32>() / samples.len() as u32
        };
        assert!(mean_confidence(&four.local_orientation).abs_diff(mean_confidence(&eight.local_orientation)) <= 75);

        let limited = ScaleOrientationSettings { max_working_bytes: 1, ..settings };
        assert!(matches!(calibrate_scale_orientation(
            &directional_source, &stage_five, &MaterialCalibrationIntent::default(), &limited, &cancellation,
        ), Err(ScaleOrientationError::ResourceLimitExceeded { .. })));
        let cancelled = RenderCancellationToken::new();
        cancelled.cancel();
        assert_eq!(calibrate_scale_orientation(
            &directional_source, &stage_five, &MaterialCalibrationIntent::default(), &settings, &cancelled,
        ), Err(ScaleOrientationError::Cancelled));

        let mut intent = MaterialCalibrationIntent::default();
        intent.apply(MaterialCalibrationCommand::MeasureTwoPoints {
            start: SourcePixelPointMilli { x: 10_000, y: 20_000 },
            end: SourcePixelPointMilli { x: 110_000, y: 20_000 },
            distance_micrometers: 250_000,
        }).unwrap();
        assert_eq!(intent.scale.source_pixels_per_meter_x_milli, Some(400_000));
        assert_eq!(intent.scale.source_pixels_per_meter_x_milli, intent.scale.source_pixels_per_meter_y_milli,
            "two-point isotropic measurement must round-trip identically on X/Y");
        let measured = calibrate_scale_orientation(&directional_source, &stage_five, &intent, &settings, &cancellation).unwrap();
        assert!(measured.scale.claims_world_accuracy());
        assert_ne!(measured.downstream_footprint_key, report.downstream_footprint_key);
        intent.apply(MaterialCalibrationCommand::ResetScale).unwrap();
        let reset = calibrate_scale_orientation(&directional_source, &stage_five, &intent, &settings, &cancellation).unwrap();
        assert!(!reset.scale.claims_world_accuracy());
        assert_ne!(reset.downstream_footprint_key, measured.downstream_footprint_key);

        intent.apply(MaterialCalibrationCommand::OverrideScale {
        ...
        let prior = calibrate_scale_orientation(&directional_source, &stage_five, &intent, &settings, &cancellation).unwrap();
        assert_eq!(prior.scale.world_scale, WorldScaleAvailability::UnavailablePriorEstimate);
        assert!(!prior.measurement_overlay.world_scale_available);
        assert!(prior.scale_diagnostics.iter().any(|d| d.code == ScaleDiagnosticCode::InconsistentAnisotropicScale));

        intent.apply(MaterialCalibrationCommand::OverrideOrientation { axis_millidegrees: 179_000 }).unwrap();
        let overridden = calibrate_scale_orientation(&noisy_source, &noisy_stage_five, &intent, &settings, &cancellation).unwrap();
        assert_eq!(overridden.global_orientation.authority, OrientationAuthority::UserOverride);
        assert!(overridden.global_orientation.destructive_rotation_allowed);
        assert_eq!(overridden.orientation_overlay.coordinate_space, OverlayCoordinateSpace::SourcePixels);
    }
}
