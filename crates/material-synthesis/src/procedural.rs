...
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub enum ProceduralParameterName {
    ModelKind,
    Orientation,
    MotifScale,
    Palette,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ProceduralContentIntent {
    MaterialOnly,
    ContainsSemanticDetail,
}
...
impl Default for ProceduralFitSettings {
    fn default() -> Self {
        Self {
            minimum_confidence_milli: 260,
            limits: ProceduralLimits::default(),
        }
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ProceduralCoordinateTopology {
    Cartesian,
    PolarCompatible,
}
...
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub enum ProceduralContribution {
    BaseColor,
    Height,
    Normal,
    Roughness,
...
pub enum ProceduralDiagnosticCode {
    RelativeScaleOnly,
    EstimatedPbrContribution,
    UserCorrectionApplied,
    SemanticDetailExcluded,
    EndGrainEstimated,
...
impl ProceduralDomainModel for FittedProceduralDomainModel {
    ...
    fn generate(
        ...
        generate_model(self, domain, cancellation)
    }
}
...
pub fn fit_procedural_domain_model(
    ...
    if request.content_intent == ProceduralContentIntent::ContainsSemanticDetail
        || matches!(
            request.stage_five.classification.routed_class(),
    ...
    let kind = if let Some(kind) = override_value.and_then(|value| value.model_kind) {
        kind
    ...
    } else if let Some(kind) = kind_from_evidence(routed) {
        kind
    } else {
        return Ok(ProceduralFitOutcome::Unsupported(
            UnsupportedProceduralModel {
                state: ProceduralUnsupportedState::BehaviorNotProcedural,
                message: "measured behavior does not support a V1 procedural material family"
                    .into(),
    ...
    let confidence = request.stage_five.classification.confidence_milli.min(
        request
            .stage_six
            .global_orientation
            .confidence_milli
    ...
    if confidence < request.settings.minimum_confidence_milli && !explicit_family {
        return Ok(ProceduralFitOutcome::Unsupported(
            UnsupportedProceduralModel {
                ...
                message: format!(
                    "measured procedural confidence {confidence} is below threshold {}",
                    request.settings.minimum_confidence_milli
                ),
                recovery_choices: vec![
    ...
    if matches!(
        ...
        && override_value.and_then(|value| value.motif_scale).is_none()
    {
        return Ok(ProceduralFitOutcome::Unsupported(UnsupportedProceduralModel {
    ...
    if override_value.and_then(|v| v.model_kind).is_some() {
        overridden.insert(ProceduralParameterName::ModelKind);
    }
    if override_value
        .and_then(|v| v.orientation_millidegrees)
        .is_some()
    {
        overridden.insert(ProceduralParameterName::Orientation);
    }
    if override_value.and_then(|v| v.motif_scale).is_some() {
        overridden.insert(ProceduralParameterName::MotifScale);
    }
    if override_value.and_then(|v| v.palette).is_some() {
        overridden.insert(ProceduralParameterName::Palette);
    }
    if override_value.and_then(|v| v.roughness_milli).is_some() {
        overridden.insert(ProceduralParameterName::Roughness);
    }
    if override_value.and_then(|v| v.noise_milli).is_some() {
        overridden.insert(ProceduralParameterName::Noise);
    }
    let measured_period =
        lattice.map(|candidate| (candidate.first.dx_pixels, candidate.first.dy_pixels));
    let measured_scale = fitted_scale(
        request.stage_six.as_ref(),
        measured_period,
        request.source.base_color().dimensions(),
    );
    let measured_palette = fit_palette(
        request.source.base_color(),
        request.settings.limits.max_fit_samples,
        cancellation,
    )?;
    ...
    let motif_scale = override_value
        .and_then(|v| v.motif_scale)
        .unwrap_or(measured_scale);
    let palette = override_value
        .and_then(|v| v.palette)
        .unwrap_or(measured_palette);
    let orientation = override_value
        .and_then(|v| v.orientation_millidegrees)
        .or(measured_orientation)
        .unwrap_or(0);
    let roughness = override_value
        .and_then(|v| v.roughness_milli)
        .unwrap_or(measured_roughness);
    let noise = override_value
        .and_then(|v| v.noise_milli)
        .unwrap_or(measured_noise);
    ...
    let evidence = ProceduralParameterEvidence {
        ...
        measured_orientation_millidegrees: request
            .stage_six
            .global_orientation
            .measured_axis_millidegrees,
    ...
    if motif_scale.units == ProceduralUnits::RelativeMillionths {
        diagnostics.push(ProceduralDiagnostic {
            code: ProceduralDiagnosticCode::RelativeScaleOnly,
            message: "motif scale is relative because Stage 6 made no world-accurate scale claim"
                .into(),
        });
    }
    if !overridden.is_empty() {
        diagnostics.push(ProceduralDiagnostic {
            code: ProceduralDiagnosticCode::UserCorrectionApplied,
            message: "typed corrections were applied while measured evidence was retained for QA"
                .into(),
        });
    }
    ...
    diagnostics.push(ProceduralDiagnostic {
        code: ProceduralDiagnosticCode::EstimatedPbrContribution,
        message:
            "generated structural/PBR contributions are registered estimates from the fitted model"
                .into(),
    });
    let parameters = FittedProceduralParameters {
        kind,
        orientation_millidegrees: orientation,
        motif_scale,
        palette,
        roughness_milli: roughness,
        layers,
    };
    let digest = model_digest(
        &parameters,
        request.seed,
        &request.stage_seven.registration_digest,
    );
    let compilation_diagnostics = diagnostics
        .iter()
        .map(|item| CompilationDiagnostic {
    ...
    Ok(ProceduralFitOutcome::Fitted(FittedProceduralDomainModel {
        ...
        channel_outputs: outputs_for(kind),
        diagnostics,
        qa_views: vec![
            ProceduralQaView::ParameterSummary,
            ProceduralQaView::CoordinateGrid,
            ProceduralQaView::MotifScale,
            ProceduralQaView::Orientation,
            ProceduralQaView::LayerMasks,
            ProceduralQaView::ChannelRegistration,
        ],
        ...
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance {
                algorithm_id: STAGE_08E_PROCEDURAL_ALGORITHM_ID.into(),
                version: STAGE_08E_PROCEDURAL_ALGORITHM_VERSION.into(),
            },
...
fn validate_fit_request(request: &ProceduralFitRequest) -> Result<(), ProceduralError> {
    if request.settings.minimum_confidence_milli > 1000
        || request.settings.limits.max_output_pixels == 0
        || request.settings.limits.max_working_bytes == 0
        || request.settings.limits.max_operations == 0
        || request.settings.limits.max_fit_samples == 0
    {
    ...
    if let Some(value) = &request.user_override {
        let palette_valid = value.palette.is_none_or(|palette| {
            [palette.dark, palette.mid, palette.light]
                .into_iter()
                .flatten()
        ...
        if value
            ...
            .is_some_and(|angle| angle >= 180_000)
            || value
                .motif_scale
                .is_some_and(|scale| scale.x == 0 || scale.y == 0)
            || value
                .roughness_milli
                .is_some_and(|roughness| roughness > 1000)
...
fn fit_preflight(request: &ProceduralFitRequest) -> Result<(), ProceduralError> {
    let base_pixels = u64::from(request.source.base_color().width())
        .checked_mul(u64::from(request.source.base_color().height()))
        .ok_or(ProceduralError::ResourceLimitExceeded)?;
    let roughness_pixels = request
        .source
        .channels
        .iter()
        .find_map(|channel| match channel {
            PreparedExemplarChannel::Scalar {
                ...
                plane,
            } => Some(u64::from(plane.width()) * u64::from(plane.height())),
            _ => None,
        })
        .unwrap_or(0);
    ...
    let sort_operations = samples
        .checked_mul(samples.max(1).ilog2().into())
        .ok_or(ProceduralError::ResourceLimitExceeded)?;
    let operations = base_pixels
        .checked_mul(5)
    ...
    let bytes = samples
        .checked_mul(16)
        .ok_or(ProceduralError::ResourceLimitExceeded)?;
    if operations > request.settings.limits.max_operations
        || bytes > request.settings.limits.max_working_bytes
    {
...
fn level_zero_pixels<T>(pyramid: &hot_trimmer_image_io::ResolutionPyramid<T>) -> u64 {
    pyramid.levels().first().map_or(0, |plane| {
...
fn parameter_authorities(
    overridden: &BTreeSet<ProceduralParameterName>,
    measured_orientation: bool,
) -> BTreeMap<ProceduralParameterName, ParameterAuthority> {
    [
        ProceduralParameterName::ModelKind,
        ProceduralParameterName::Orientation,
        ProceduralParameterName::MotifScale,
        ProceduralParameterName::Palette,
        ProceduralParameterName::Roughness,
        ProceduralParameterName::Noise,
    ]
    .into_iter()
    .map(|name| {
        let authority = if overridden.contains(&name) {
            ParameterAuthority::UserOverride
        } else if name == ProceduralParameterName::Orientation && !measured_orientation {
            ParameterAuthority::EstimatedFromMeasuredEvidence
        } else {
            ParameterAuthority::Measured
        };
        (name, authority)
    })
    .collect()
...
fn kind_from_evidence(class: MaterialBehaviorClass) -> Option<ProceduralModelKind> {
    match class {
        ...
        MaterialBehaviorClass::PeriodicLatticeStructured => {
            Some(ProceduralModelKind::BrickTileLattice)
        }
...
fn fitted_scale(
    scale: &ScaleOrientationReport,
    period: Option<(i32, i32)>,
    dimensions: (u32, u32),
) -> MotifScale {
    let (px, py) = period
        .map(|(x, y)| (x.unsigned_abs().max(1), y.unsigned_abs().max(1)))
    ...
    if scale.scale.claims_world_accuracy() {
        ...
        let sy = scale
            ...
            .max(1);
        MotifScale {
            x: u64::from(px) * 1_000_000_000 / sx,
            y: u64::from(py) * 1_000_000_000 / sy,
            units: ProceduralUnits::PhysicalMicrometers,
        }
    } else {
        MotifScale {
            x: u64::from(px) * 1_000_000 / u64::from(dimensions.0.max(1)),
            y: u64::from(py) * 1_000_000 / u64::from(dimensions.1.max(1)),
...
fn fit_palette(
    ...
    for y in 0..base.height() {
        ...
        for x in 0..base.width() {
            if index % stride == 0 {
                samples.push(base.pixel(x, y).rgb);
            }
            index += 1;
        }
    }
    ...
    Ok(ProceduralPalette {
        dark: at(samples.len() / 10),
        mid: at(samples.len() / 2),
        light: at(samples.len() * 9 / 10),
    })
}
...
fn fit_roughness(
    ...
    Ok(((sum / count.max(1) as f64) * 1000.0)
        .round()
        .clamp(0.0, 1000.0) as u16)
}

fn fit_layers(
    kind: ProceduralModelKind,
    fields: &FeatureFieldReport,
    noise: u16,
    cancellation: &RenderCancellationToken,
) -> Result<ProceduralLayerParameters, ProceduralError> {
    let level = fields.structure.grid.levels().first();
    let grid = match level {
        Some(plane) => mean_scalar(plane, cancellation)?,
        None => 0.0,
    };
    ...
    Ok(ProceduralLayerParameters {
        primary_width_milli: primary,
        secondary_width_milli: secondary,
        density_milli: density.min(1000),
        noise_milli: noise,
    })
}
...
fn outputs_for(kind: ProceduralModelKind) -> BTreeSet<ProceduralContribution> {
    let mut set = BTreeSet::from([
        ProceduralContribution::BaseColor,
        ProceduralContribution::Height,
        ProceduralContribution::Normal,
        ProceduralContribution::Roughness,
        ProceduralContribution::LayerMask,
    ]);
    if matches!(
        kind,
        ProceduralModelKind::BrushedMetal | ProceduralModelKind::PaintedMetalLayers
    ) {
        set.insert(ProceduralContribution::Metallic);
    }
    if matches!(
        kind,
        ProceduralModelKind::BrickTileLattice | ProceduralModelKind::ConcreteAggregate
    ) {
...
fn generate_model(
    model: &FittedProceduralDomainModel,
    domain: ProceduralDomainSpec,
    cancellation: &RenderCancellationToken,
) -> Result<GeneratedProceduralDomain, ProceduralError> {
    let pixels = u64::from(domain.width)
        .checked_mul(u64::from(domain.height))
        .ok_or(ProceduralError::ResourceLimitExceeded)?;
    if domain.width == 0
        || domain.height == 0
        || domain.extent_x == 0
        || domain.extent_y == 0
        || pixels > model.limits.max_output_pixels
    {
        return Err(ProceduralError::InvalidSettings);
    }
    let tile = 128.min(domain.width).min(domain.height).max(1);
    let tiles = u64::from(domain.width.div_ceil(tile))
        .checked_mul(u64::from(domain.height.div_ceil(tile)))
        .ok_or(ProceduralError::ResourceLimitExceeded)?;
    let output_plane_count = 6_u64
        + model
            .channel_outputs
            .contains(&ProceduralContribution::Metallic) as u64
        + model
            .channel_outputs
            .contains(&ProceduralContribution::AmbientOcclusion) as u64;
    ...
    let bytes = pixels
        .checked_mul(112)
        .and_then(|value| {
            value.checked_add(tiles.checked_mul(output_plane_count)?.checked_mul(64)?)
        })
    ...
    if cancellation.is_cancelled() {
        return Err(ProceduralError::Cancelled);
    }
    ...
    let mut ao = Vec::with_capacity(count);
    let mut masks = Vec::with_capacity(count);
    for y in 0..domain.height {
        if y % 32 == 0 && cancellation.is_cancelled() {
            return Err(ProceduralError::Cancelled);
        }
        for x in 0..domain.width {
            let coordinate =
                model_coordinate(domain, x, y, model.parameters.orientation_millidegrees);
            let sample = sample_model(model, coordinate);
            colors.push(LinearColor {
                rgb: sample.color,
                alpha: 1.0,
            });
            heights.push(LinearScalar(sample.height));
            roughness.push(LinearScalar(sample.roughness));
            metallic.push(LinearScalar(sample.metallic));
            ao.push(LinearScalar(sample.ao));
    ...
    let mut channels = vec![
        ...
        PreparedExemplarChannel::Scalar {
            role: MaterialChannelRole::Roughness,
            plane: plane(domain, tile, &roughness)?,
        },
    ];
    if model
        .channel_outputs
        .contains(&ProceduralContribution::Metallic)
    {
        channels.push(PreparedExemplarChannel::Scalar {
            role: MaterialChannelRole::Metallic,
            plane: plane(domain, tile, &metallic)?,
        });
    }
    if model
        .channel_outputs
        .contains(&ProceduralContribution::AmbientOcclusion)
    {
        channels.push(PreparedExemplarChannel::Scalar {
            role: MaterialChannelRole::AmbientOcclusion,
            plane: plane(domain, tile, &ao)?,
        });
    }
    channels.push(PreparedExemplarChannel::Mask {
        role: MaterialChannelRole::EdgeMask,
        plane: plane(domain, tile, &masks)?,
    });
    let digest = model_digest(
        &model.parameters,
        model.seed,
        &model.source_registration_digest,
    );
    Ok(GeneratedProceduralDomain {
        width: domain.width,
        height: domain.height,
        units: model.parameters.motif_scale.units,
        topology: domain.topology,
        channels,
        layer_mask: plane(domain, tile, &masks)?,
        model_digest: digest,
        qa_views: model.qa_views.clone(),
        stage_result: model.stage_result.clone(),
    })
}
...
fn sample_model(model: &FittedProceduralDomainModel, p: (f64, f64)) -> ModelSample {
    ...
    let sx = params.motif_scale.x.max(1) as f64;
    let sy = params.motif_scale.y.max(1) as f64;
    let (u, v) = (p.0 / sx, p.1 / sy);
    let noise = f64::from(params.layers.noise_milli) / 1000.0;
    ...
    let (tone, height, mask, metallic, ao, rough_delta) = match params.kind {
        ...
        ProceduralModelKind::WoodEndGrain => {
            ...
            let angle = v.atan2(u);
            let irregular = (angle * 7.0 + radius * 0.35).sin() * 0.16
                + signed_hash(model.seed, (angle * 50.0) as i64, radius as i64) * noise * 0.12;
        ...
        ProceduralModelKind::BrickTileLattice => {
            ...
            let bx = (u + if row & 1 == 0 { 0.0 } else { 0.5 }).rem_euclid(1.0);
            let by = v.rem_euclid(1.0);
            let mortar = bx.min(1.0 - bx) < 0.055 || by.min(1.0 - by) < 0.075;
            let variation = signed_hash(
                model.seed,
                (u + if row & 1 == 0 { 0.0 } else { 0.5 }).floor() as i64,
                row,
            );
            if mortar {
                (0.08, 0.12, 1.0, 0.0, 0.62, 0.18)
            } else {
                (
                    (0.56 + variation * 0.18).clamp(0.0, 1.0),
                    0.68 + variation * 0.08,
                    0.0,
                    0.0,
                    1.0,
        ...
        ProceduralModelKind::Corrugation => {
            let ridge = (std::f64::consts::TAU * u).cos();
            let valley = (1.0 - ridge) * 0.5;
            (
                0.44 + ridge * 0.12,
                0.5 + ridge * 0.45,
                (valley > 0.75) as u8 as f64,
                0.35,
                0.72 + ridge * 0.18,
                valley * 0.08,
            )
        ...
        ProceduralModelKind::ConcreteAggregate => {
            ...
            if aggregate {
                (0.68 + fine * 0.12, 0.67 + fine * 0.08, 1.0, 0.0, 0.78, 0.1)
            } else {
                (
                    0.4 + fine * 0.1,
                    0.48 + fine * 0.05,
    ...
    ModelSample {
        color: palette_mix(params.palette, tone as f32),
        height: height.clamp(0.0, 1.0) as f32,
        roughness: (base_rough + rough_delta as f32).clamp(0.0, 1.0),
        metallic: metallic as f32,
        ao: ao.clamp(0.0, 1.0) as f32,
        mask: mask as f32,
    }
}
...
fn model_coordinate(
    ...
    x: u32,
    y: u32,
    angle_millidegrees: u32,
) -> (f64, f64) {
    let px = if domain.width == 1 {
        domain.extent_x as f64 * 0.5
    } else {
        f64::from(x) * domain.extent_x as f64 / f64::from(domain.width - 1)
    };
    let py = if domain.height == 1 {
        domain.extent_y as f64 * 0.5
    } else {
        f64::from(y) * domain.extent_y as f64 / f64::from(domain.height - 1)
    };
    let (px, py) = if domain.topology == ProceduralCoordinateTopology::PolarCompatible {
        let cx = px - domain.extent_x as f64 * 0.5;
        let cy = py - domain.extent_y as f64 * 0.5;
        (
            cx.hypot(cy),
            cy.atan2(cx) * domain.extent_y as f64 / std::f64::consts::TAU,
        )
    } else {
        (px, py)
    };
...
fn height_normals(
    ...
    for y in 0..height_px {
        ...
        for x in 0..width {
            ...
            let dy =
                derivative_1d(y, height_px, step_y, |sample_y| at(x, sample_y)) * motif.y as f32;
...
fn derivative_1d(sample: u32, count: u32, step: f32, at: impl Fn(u32) -> f32) -> f32 {
    ...
    match sample {
        0 => {
            (-25.0 * at(0) + 48.0 * at(1) - 36.0 * at(2) + 16.0 * at(3) - 3.0 * at(4)) / denominator
        }
        ...
        _ => {
            (at(sample - 2) - 8.0 * at(sample - 1) + 8.0 * at(sample + 1) - at(sample + 2))
                / denominator
        }
    }
}
...
fn plane<T: Clone>(
    ...
    tile: u32,
    values: &[T],
) -> Result<ImagePlane<T>, ProceduralError> {
    ImagePlane::from_row_major(domain.width, domain.height, tile, values)
        .map_err(|_| ProceduralError::PlaneConstruction)
}

fn palette_mix(palette: ProceduralPalette, t: f32) -> [f32; 3] {
    let (a, b, local) = if t < 0.5 {
        (palette.dark, palette.mid, t * 2.0)
    } else {
        (palette.mid, palette.light, (t - 0.5) * 2.0)
    };
    [
        a[0] + (b[0] - a[0]) * local,
        a[1] + (b[1] - a[1]) * local,
...
fn model_digest(
    parameters: &FittedProceduralParameters,
    seed: u64,
    registration: &ContentDigest,
) -> ContentDigest {
    ContentDigest::sha256(
        format!(
            "{parameters:?}|{seed}|{}|{STAGE_08E_PROCEDURAL_ALGORITHM_VERSION}",
            registration.0
        )
        .as_bytes(),
    )
...
trait PlaneDimensions {
    fn dimensions(&self) -> (u32, u32);
}
impl<T> PlaneDimensions for ImagePlane<T> {
    fn dimensions(&self) -> (u32, u32) {
...
mod tests {
    use super::*;
    use hot_trimmer_domain::{DelightingPassThroughReason, MaterialCalibrationIntent};
    use hot_trimmer_material_analysis::{
    ...
    fn stage_result() -> StageResult {
        StageResult::PassThrough {
            reason: "procedural fixture".into(),
        }
    }
    ...
    fn source() -> Arc<DelitPreparedExemplar> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                colors.push(LinearColor {
                    rgb: [value, value * 0.72, value * 0.48],
                    alpha: 1.0,
                });
    ...
    fn fit_request(kind: ProceduralModelKind) -> ProceduralFitRequest {
        ...
        let mut six = calibrate_scale_orientation(
            &source,
            &five,
        ...
        let seven = extract_feature_fields(&source, &six, &FeatureFieldSettings::default(), &token)
            .unwrap();
        ProceduralFitRequest {
            source,
            stage_five: Arc::new(five),
            stage_six: Arc::new(six),
            stage_seven: Arc::new(seven),
            user_override: Some(ProceduralUserOverride {
                model_kind: Some(kind),
                orientation_millidegrees: None,
                motif_scale: Some(MotifScale {
                    x: 125_000,
                    y: 125_000,
                    units: ProceduralUnits::RelativeMillionths,
                }),
                palette: None,
                roughness_milli: None,
                noise_milli: None,
            }),
            content_intent: ProceduralContentIntent::MaterialOnly,
            seed: 44,
            settings: ProceduralFitSettings {
                minimum_confidence_milli: 260,
                limits: ProceduralLimits {
    ...
    fn fitted(
        request: &ProceduralFitRequest,
        cancellation: &RenderCancellationToken,
    ) -> FittedProceduralDomainModel {
        let ProceduralFitOutcome::Fitted(model) =
            fit_procedural_domain_model(request, cancellation).unwrap()
        else {
    ...
    fn scalar_channel(
        domain: &GeneratedProceduralDomain,
    ...
    fn corresponding_normal_axes(
        ...
        for y in 2..low.height() - 2 {
            for x in 2..low.width() - 2 {
        ...
        (
            axis(low_xx, low_yy, low_xy),
            axis(high_xx, high_yy, high_xy),
        )
    }

    fn axis_difference(a: f64, b: f64) -> f64 {
        let difference = (a - b).abs().rem_euclid(180.0);
    ...
    fn algorithm_stage_08e_procedural() {
        ...
        for kind in kinds {
            ...
            assert_eq!(
                model,
                fitted(&request, &cancellation),
                "fitting must replay deterministically"
            );
            for (width, height, topology) in [
                (96, 32, ProceduralCoordinateTopology::Cartesian),
                (32, 96, ProceduralCoordinateTopology::Cartesian),
                (64, 64, ProceduralCoordinateTopology::Cartesian),
                (64, 64, ProceduralCoordinateTopology::PolarCompatible),
            ] {
                let spec = ProceduralDomainSpec {
                    width,
                    height,
                    extent_x: 1_000_000,
                    extent_y: 1_000_000,
                ...
                assert!(output.channels.len() >= 5);
                assert!(
                    output
                        .channels
                        .iter()
                        .all(|channel| channel.dimensions() == (width, height))
                );
            }
        }
        ...
        let corrected_palette = ProceduralPalette {
            dark: [0.01; 3],
            mid: [0.2; 3],
            light: [0.9; 3],
        };
        let correction = corrected.user_override.as_mut().unwrap();
        correction.palette = Some(corrected_palette);
        correction.roughness_milli = Some(123);
        ...
        assert_ne!(
            corrected_model.evidence.measured_parameters.palette,
            corrected_palette
        );
        assert_ne!(
            corrected_model.evidence.measured_parameters.roughness_milli,
            123
        );
        assert_eq!(
            corrected_model.evidence.effective_authorities[&ProceduralParameterName::Palette],
            ParameterAuthority::UserOverride
        );
        ...
        Arc::make_mut(&mut partial.stage_five)
            .classification
            .confidence_milli = 1;
        partial.user_override = Some(ProceduralUserOverride {
            roughness_milli: Some(600),
            ..ProceduralUserOverride::default()
        });
        assert!(matches!(
            fit_procedural_domain_model(&partial, &cancellation).unwrap(),
        ...
        Arc::make_mut(&mut foreign_five.stage_five).prepared_source_digest =
            ContentDigest::sha256(b"foreign");
        assert_eq!(
            fit_procedural_domain_model(&foreign_five, &cancellation),
            Err(ProceduralError::RegistrationDrift)
        );
        ...
        Arc::make_mut(&mut foreign_six.stage_six).stage_five_cache_key =
            hot_trimmer_material_analysis::SourceAnalysisCacheKey(ContentDigest::sha256(
                b"foreign-five",
            ));
        assert_eq!(
            fit_procedural_domain_model(&foreign_six, &cancellation),
            Err(ProceduralError::RegistrationDrift)
        );
        ...
        assert!(matches!(
            fit_procedural_domain_model(&end_grain, &cancellation).unwrap(),
            ProceduralFitOutcome::Unsupported(UnsupportedProceduralModel {
                state: ProceduralUnsupportedState::EndGrainRequiresExplicitModelOrSource,
                ..
            })
        ...
        assert!(matches!(
            fit_procedural_domain_model(&semantic, &cancellation).unwrap(),
            ProceduralFitOutcome::Unsupported(UnsupportedProceduralModel {
                state: ProceduralUnsupportedState::SemanticDetailRequiresDetailContract,
                ..
            })
        ...
        cancelled.cancel();
        assert_eq!(
            fit_procedural_domain_model(
                &fit_request(ProceduralModelKind::BrushedMetal),
                &cancelled
            ),
            Err(ProceduralError::Cancelled)
        );
        ...
        assert_eq!(
            fit_procedural_domain_model(&bounded, &cancellation),
            Err(ProceduralError::ResourceLimitExceeded)
        );
        bounded.settings.limits.max_operations = 100_000_000;
        bounded.settings.limits.max_working_bytes = 1;
        assert_eq!(
            fit_procedural_domain_model(&bounded, &cancellation),
            Err(ProceduralError::ResourceLimitExceeded)
        );
        ...
        let model = fitted(
            &fit_request(ProceduralModelKind::Corrugation),
            &cancellation,
        );
        let low = model
            .generate(
                ProceduralDomainSpec {
                    ...
                    extent_x: 1_000_000,
                    extent_y: 1_000_000,
                    topology: ProceduralCoordinateTopology::Cartesian,
                },
                &cancellation,
            )
            .unwrap();
        ...
        for y in 0..65 {
            for x in 0..65 {
                assert!(
                    (low_height.pixel(x, y).0 - high_height.pixel(x * 2, y * 2).0).abs() < 1.0e-6
                );
            }
        }
        let low_normal = normal_channel(&low);
        let high_normal = normal_channel(&high);
        let mut dot_sum = 0.0_f64;
        let mut count = 0_u64;
        for y in 2..63 {
            for x in 2..63 {
                let a = low_normal.pixel(x, y).xyz;
                let b = high_normal.pixel(x * 2, y * 2).xyz;
                dot_sum += f64::from(a[0] * b[0] + a[1] * b[1] + a[2] * b[2]);
                count += 1;
            }
        }
        assert!(
            dot_sum / count as f64 > 0.98,
            "coordinate-scaled normals must remain stable across resolution"
        );
        let (low_axis, high_axis) = corresponding_normal_axes(low_normal, high_normal);
        assert!(
            axis_difference(low_axis, high_axis) < 1.0,
            "orientation must remain stable across resolution"
        );
        let crossings = |plane: &ImagePlane<LinearScalar>, y: u32| {
            (1..plane.width())
                .filter(|x| (plane.pixel(*x - 1, y).0 >= 0.5) != (plane.pixel(*x, y).0 >= 0.5))
                .count()
        };
        assert_eq!(
            crossings(low_height, 32),
            crossings(high_height, 64),
            "motif count must not scale with pixels"
        );

        // The revised peak estimate rejects a budget that covered the old 64-byte charge.
        let mut memory_model = model.clone();
        memory_model.limits.max_working_bytes = 400_000;
        assert_eq!(
            memory_model.generate(
                ProceduralDomainSpec {
                    width: 64,
                    height: 64,
                    extent_x: 1_000_000,
