...
pub const STAGE_08A_GRAPHCUT_ALGORITHM_ID: &str =
    "hot_trimmer.multichannel_graphcut_periodic_closure";
pub const STAGE_08A_ALGORITHM_VERSION: &str = "8.1.0";
const MAX_CACHE_ENTRIES: usize = 12;

#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub enum ClosureAxes {
    X,
    Y,
    XY,
}
...
impl ClosureAxes {
    const fn includes_x(self) -> bool {
        matches!(self, Self::X | Self::XY)
    }
    const fn includes_y(self) -> bool {
...
pub enum DomainOperation {
    ...
    SeamCompose {
        ...
        categorical_selected_contribution: u8,
    },
...
impl PreparedMaterialDomain {
        ...
        self.registered_channels()
            ...
            .find_map(|channel| match channel {
                PreparedExemplarChannel::BaseColor { plane, .. }
                    if role == MaterialChannelRole::BaseColor =>
                {
                    Some(RegisteredChannelRef::BaseColor(plane))
                }
                PreparedExemplarChannel::Scalar { role: found, plane } if *found == role => {
                    Some(RegisteredChannelRef::Scalar(*found, plane))
                }
                PreparedExemplarChannel::Normal { plane, .. }
                    if role == MaterialChannelRole::Normal =>
                {
                    Some(RegisteredChannelRef::Normal(plane))
                }
                PreparedExemplarChannel::MaterialId { plane }
                    if role == MaterialChannelRole::MaterialId =>
                {
                    Some(RegisteredChannelRef::MaterialId(plane))
                }
                PreparedExemplarChannel::Mask { role: found, plane } if *found == role => {
                    Some(RegisteredChannelRef::Mask(*found, plane))
                }
                _ => None,
            })
    }
    ...
    pub fn from_registered_channels(
        ...
        Self::from_registered_channels_with_seams(
            cache_key,
    ...
    pub fn from_registered_channels_with_route_and_seams(
        ...
        let dimensions = channels
            .first()
            .map(channel_dimensions)
            .ok_or(DomainError::RegistrationDrift)?;
        if dimensions.0 == 0
            || dimensions.1 == 0
            || channels
                .iter()
                .any(|channel| channel_dimensions(channel) != dimensions)
        {
            return Err(DomainError::RegistrationDrift);
        }
        let (width, height) = dimensions;
        let pixel_count = usize::try_from(u64::from(width) * u64::from(height))
            .map_err(|_| DomainError::ResourceLimitExceeded)?;
        let validity = plane(width, height, 128, vec![MaskValue(1.0); pixel_count])?;
        let provenance = plane(
            width,
            height,
            128,
            vec![ProvenanceValue::Original; pixel_count],
        )?;
        let algorithm = AlgorithmProvenance {
            algorithm_id: STAGE_08A_DIRECT_ALGORITHM_ID.into(),
            version: STAGE_08A_ALGORITHM_VERSION.into(),
        };
        Ok(Self {
            cache_key: cache_key.clone(),
            prepared_source_digest,
            analysis_digest: cache_key.clone(),
            route,
            width,
            height,
            ...
            diagnostics: DomainDiagnostics {
                ...
                cache_key,
                available_seam_terms: BTreeSet::new(),
                normalized_weight_milli: BTreeMap::new(),
                pass_through: None,
                seams: Vec::new(),
                boundary_cost_before_milli: (0, 0),
            ...
            stage_result: StageResult::Executed {
                algorithm,
                settings_hash: ContentDigest::sha256(b"registered-domain-rehydration"),
                diagnostics: Vec::new(),
            },
        })
    }
...
pub struct MaterialDomainCache {
    entries: BTreeMap<ContentDigest, PreparedMaterialDomain>,
}

impl MaterialDomainCache {
    #[must_use]
    pub fn get(&self, key: &ContentDigest) -> Option<&PreparedMaterialDomain> {
        self.entries.get(key)
    }
    pub fn insert_complete(&mut self, domain: PreparedMaterialDomain) {
        if self.entries.len() >= MAX_CACHE_ENTRIES
            && !self.entries.contains_key(&domain.cache_key)
...
pub enum DomainError {
    ...
    RegistrationDrift,
    #[error(
        "Stage 8A graph-cut {axis:?} overlap requires {required} pixels but only {available} are available"
    )]
    InsufficientOverlap {
        axis: SeamAxis,
        required: u32,
    ...
    #[error(
        "Stage 8A graph-cut {axis:?} seam cost {cost_milli}/1000 exceeds the accepted maximum {maximum_milli}/1000"
    )]
    UnacceptableSeam {
        axis: SeamAxis,
        cost_milli: u16,
    ...
    #[error(
        "Stage 8B quilting found no source patch that avoids unusable pixels ({rejected_candidates} candidates rejected)"
    )]
    UnusableQuiltingSource { rejected_candidates: u32 },
    #[error(
        "Stage 8B quilting cannot cover the requested output with the bounded patch/iteration limits"
    )]
    QuiltingCoverageFailed,
    #[error(
        "Stage 8B quilting seam for patch {patch_index} on {axis:?} costs {cost_milli}/1000, above {maximum_milli}/1000"
    )]
    UnacceptableQuiltingSeam {
        patch_index: u32,
        axis: SeamAxis,
        cost_milli: u16,
        maximum_milli: u16,
    ...
    #[error(
        "Stage 8B quilting boundary error {horizontal_milli}/1000 x {vertical_milli}/1000 exceeds {maximum_milli}/1000"
    )]
    UnacceptableQuiltingBoundary {
        horizontal_milli: u16,
        vertical_milli: u16,
        maximum_milli: u16,
    },
    ...
    #[error(
        "Stage 8C PatchMatch found no usable source patch ({rejected_candidates} candidates rejected)"
    )]
    ...
    #[error(
        "Stage 8C PatchMatch did not converge after {iterations} iterations ({changed_pixels} pixels still changing)"
    )]
    PatchMatchNonConverged {
        iterations: u16,
        changed_pixels: u64,
    },
    #[error(
        "Stage 8C PatchMatch boundary error {horizontal_milli}/1000 x {vertical_milli}/1000 exceeds {maximum_milli}/1000"
    )]
    UnacceptablePatchMatchBoundary {
        horizontal_milli: u16,
        vertical_milli: u16,
        maximum_milli: u16,
    },
    #[error(
        "statistical, procedural, and learned Stage 8 routes are reachable only through the authoritative material-domain router"
    )]
    RouterRequired,
}
...
impl DomainError {
    ...
    pub fn stage_result(&self) -> StageResult {
        let (code, recovery_choices) = match self {
            ...
            Self::IncompatibleQuilting { .. }
            | Self::UnusableQuiltingSource { .. }
            | Self::QuiltingCoverageFailed
            | Self::UnacceptableQuiltingSeam { .. }
            | Self::UnacceptableQuiltingBoundary { .. } => (
                DiagnosticCode::InsufficientInput,
                vec![
                    RecoveryChoice::ChooseAnotherSource,
                    RecoveryChoice::AdjustSettings,
            ...
            Self::IncompatiblePatchMatch { .. }
            | Self::UnusablePatchMatchSource { .. }
            | Self::IncompletePatchMatch { .. }
            | Self::PatchMatchNonConverged { .. } => (
                DiagnosticCode::InsufficientInput,
                vec![
                    RecoveryChoice::ChooseAnotherSource,
                    RecoveryChoice::AdjustSettings,
...
fn select_route(request: &DomainRequest) -> DomainRoute {
    match request.route {
        DomainRoute::Auto
            if request.analysis.seamability.horizontal_cost_milli
                <= request.direct_boundary_threshold_milli
                && request.analysis.seamability.vertical_cost_milli
                    <= request.direct_boundary_threshold_milli =>
        {
            DomainRoute::DirectSource
        }
...
fn validate_request(request: &DomainRequest) -> Result<(), DomainError> {
    ...
    if request.direct_boundary_threshold_milli > 1000
        || settings.overlap_pixels < 2
        || settings.max_search_positions < 2
        || settings.max_search_positions > 4096
        || settings.continuity_milli > 1000
        || settings.max_accepted_seam_cost_milli > 1000
        || settings.max_working_bytes == 0
        || settings.max_operations == 0
        || weight_sum(settings.weights) == 0
    {
        return Err(DomainError::InvalidSettings);
    }
    let (width, height) = (
        request.source.base_color().width(),
        request.source.base_color().height(),
    );
    ...
    if request.prepared_source_digest != request.source.prepared_source_digest
        ...
        || consumed_fields
            .iter()
            .any(|plane| (plane.width(), plane.height()) != (width, height))
    {
        return Err(DomainError::RegistrationDrift);
    }
    Ok(())
...
fn expected_stage_six_registration_digest(request: &DomainRequest) -> ContentDigest {
    ...
    ContentDigest::sha256(
        format!(
            "{}|{}|{}|{}|{}|{}",
            request.source.prepared_source_digest.0,
            request.source.exemplar_id,
            width,
            height,
            request.scale_orientation.cache_key.0,
            request
                ...
                .channels
                .iter()
                .map(|channel| format!("{:?}", channel.role()))
                .collect::<Vec<_>>()
                .join(",")
        )
        .as_bytes(),
...
fn consumed_level_zero_fields(request: &DomainRequest) -> Vec<&ImagePlane<LinearScalar>> {
    ...
    [
        ...
        &fields.fiber,
        &fields.intersection,
        &request.analysis.seamability.confidence,
        &request.analysis.usability.confidence,
    ]
    .into_iter()
    .filter_map(|pyramid| pyramid.level(0))
    .collect()
}
...
fn direct_domain(
    ...
    let provenance = plane(
        width,
        height,
        request.source.base_color().tile_edge(),
        vec![ProvenanceValue::Original; pixel_count(width, height)?],
    )?;
...
fn graph_cut_domain(
    ...
    if request.graph_cut.axes.includes_x() && width < overlap.saturating_mul(2) {
        return Err(DomainError::InsufficientOverlap {
            axis: SeamAxis::X,
            required: overlap * 2,
            available: width,
        });
    ...
    let operations = samples
        ...
        .map(|sample| {
            ...
            if count == 1 {
                DomainOperation::PassThrough
            } else {
                DomainOperation::SeamCompose {
    ...
    Ok(PreparedMaterialDomain {
        ...
        qa_views: vec![
            DomainQaView::RegisteredChannels,
            DomainQaView::SeamCost,
            DomainQaView::SeamPath,
        ...
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance {
                algorithm_id: STAGE_08A_GRAPHCUT_ALGORITHM_ID.into(),
                version: STAGE_08A_ALGORITHM_VERSION.into(),
            },
...
fn ensure_acceptable_seam(request: &DomainRequest, seam: &SelectedSeam) -> Result<(), DomainError> {
    ...
    if seam.normalized_cost_milli > maximum_milli {
        Err(DomainError::UnacceptableSeam {
            axis: seam.axis,
            cost_milli: seam.normalized_cost_milli,
            maximum_milli,
        })
    } else {
        Ok(())
...
fn solve_seam(
    ...
    axis: SeamAxis,
    cancellation: &RenderCancellationToken,
) -> Result<SelectedSeam, DomainError> {
    let (width, height) = (
        request.source.base_color().width(),
        request.source.base_color().height(),
    );
    ...
    for line in 1..length {
        ...
        for position in 0..candidates {
            ...
            for prior in start..=end {
                let movement = position.abs_diff(prior) as f64
                    * f64::from(request.graph_cut.continuity_milli)
                    / 1000.0;
    ...
    for position in 1..candidates {
        if previous[position as usize] < previous[ending as usize] {
            ending = position;
        }
    }
    ...
    Ok(SelectedSeam {
        axis,
        positions,
        normalized_cost_milli: score(normalized as f32),
    })
}
...
fn seam_pixel_cost(request: &DomainRequest, axis: SeamAxis, line: u32, position: u32) -> f64 {
    ...
    let ca = base.pixel(a.0, a.1).rgb;
    let cb = base.pixel(b.0, b.1).rgb;
    let color = (0..3)
        .map(|i| f64::from((ca[i] - cb[i]).abs()))
        .sum::<f64>()
        / 3.0;
    let gradient =
        (luminance_gradient(base, a.0, a.1) - luminance_gradient(base, b.0, b.1)).abs() as f64;
...
fn normalized_weight_f64(request: &DomainRequest) -> [f64; 6] {
    ...
    let raw = [
        w.color,
        w.gradient,
        if available.contains(&SeamTerm::Height) {
            w.height
        } else {
            0
        },
        if available.contains(&SeamTerm::VectorNormal) {
            w.vector_normal
        } else {
            0
        },
        if available.contains(&SeamTerm::Roughness) {
            w.roughness
        } else {
            0
        },
...
fn normalized_weights(request: &DomainRequest) -> BTreeMap<SeamTerm, u16> {
    ...
    .map(|(term, weight)| (term, score(weight as f32)))
    .collect()
}
...
fn scalar_difference(
    ...
    request
        ...
        .iter()
        .find_map(|channel| match channel {
            PreparedExemplarChannel::Scalar { role: found, plane } if *found == role => Some(
                f64::from((plane.pixel(a.0, a.1).0 - plane.pixel(b.0, b.1).0).abs()),
            ),
            _ => None,
        })
        .unwrap_or(0.0)
...
fn normal_difference(request: &DomainRequest, a: (u32, u32), b: (u32, u32)) -> f64 {
    request
        ...
        .find_map(|channel| match channel {
            PreparedExemplarChannel::Normal { plane, .. } => {
                let na = plane.pixel(a.0, a.1).xyz;
                let nb = plane.pixel(b.0, b.1).xyz;
...
fn structure_penalty(request: &DomainRequest, point: (u32, u32)) -> f64 {
    ...
    .iter()
    .filter_map(|pyramid| pyramid.level(0))
    .map(|plane| f64::from(plane.pixel(point.0, point.1).0))
...
fn build_correspondence(
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            let mut index = 0;
            for (sx, wx) in x_options {
                for (sy, wy) in &y_options {
                    let weight = wx * *wy;
                    if weight > 0.0 {
                        sources[index] = Some(WeightedSource {
                            coordinate: SourceCoordinate { x: sx, y: *sy },
                            weight,
                        });
                        index += 1;
                    }
...
fn canonical_boundary_coordinate(
    coordinate: u32,
    dimension: u32,
    overlap: u32,
    closed: bool,
) -> u32 {
    if closed && coordinate >= dimension - overlap {
        dimension - 1 - coordinate
    } else {
        coordinate
    }
    ...
    for channel in &request.source.channels {
        ...
        output.push(match channel {
            PreparedExemplarChannel::BaseColor {
                ...
                let values = samples
                    .iter()
                    .map(|sample| blend_color(source, sample))
                    .collect();
                PreparedExemplarChannel::BaseColor {
                    plane: plane(width, height, tile_edge, values)?,
            ...
            PreparedExemplarChannel::Scalar {
                ...
                let values = samples
                    .iter()
                    .map(|sample| LinearScalar(blend_scalar(source, sample)))
                    .collect();
                PreparedExemplarChannel::Scalar {
                    role: *role,
                    plane: plane(width, height, tile_edge, values)?,
            ...
            PreparedExemplarChannel::Normal {
                ...
                let values = samples
                    .iter()
                    .map(|sample| blend_normal(source, sample))
                    .collect();
                PreparedExemplarChannel::Normal {
                    plane: plane(width, height, tile_edge, values)?,
            ...
            PreparedExemplarChannel::MaterialId { plane: source } => {
                let values = samples
                    .iter()
                    .map(|sample| {
                        *source.pixel(categorical_source(sample).x, categorical_source(sample).y)
                    })
                    .collect();
            ...
            PreparedExemplarChannel::Mask {
                ...
                let values = if *role == MaterialChannelRole::Opacity {
                    samples
                        .iter()
                        .map(|sample| MaskValue(blend_mask(source, sample)))
                        .collect()
                } else {
                    samples
                        .iter()
                        .map(|sample| {
                            *source
                                .pixel(categorical_source(sample).x, categorical_source(sample).y)
                        })
                        .collect()
                };
                PreparedExemplarChannel::Mask {
...
fn blend_color(source: &ImagePlane<LinearColor>, sample: &CorrespondenceSample) -> LinearColor {
    ...
    for contribution in sample.sources.into_iter().flatten() {
        ...
        for (out, input) in rgb.iter_mut().zip(value.rgb) {
            *out += input * contribution.weight;
        }
...
fn blend_scalar(source: &ImagePlane<LinearScalar>, sample: &CorrespondenceSample) -> f32 {
    sample
        ...
        .into_iter()
        .flatten()
        .map(|c| source.pixel(c.coordinate.x, c.coordinate.y).0 * c.weight)
        .sum()
}
...
fn blend_normal(
    source: &ImagePlane<TangentNormal>,
    sample: &CorrespondenceSample,
) -> TangentNormal {
    let mut xyz = [0.0; 3];
    let mut alpha = 0.0;
    for contribution in sample.sources.into_iter().flatten() {
...
fn categorical_source(sample: &CorrespondenceSample) -> SourceCoordinate {
    sample.sources[categorical_source_index(sample)]
        .expect("selected contribution exists")
        .coordinate
}
...
fn categorical_source_index(sample: &CorrespondenceSample) -> usize {
    sample
        ...
        .flatten()
        .enumerate()
        .max_by(|(ia, a), (ib, b)| a.weight.total_cmp(&b.weight).then_with(|| ib.cmp(ia)))
        .map(|(index, _)| index)
        .expect("every correspondence has a source")
}
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            for source in sample.sources.into_iter().flatten() {
                ...
                if let Some(coverage) = &request.source.coverage {
                    confidence *= coverage.pixel(source.coordinate.x, source.coordinate.y).0;
                }
...
fn direct_validity(
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            if let Some(coverage) = &request.source.coverage {
                confidence *= coverage.pixel(x, y).0;
            }
...
fn measured_boundary_cost(
    ...
    let mut result = (
        request.analysis.seamability.horizontal_cost_milli,
        request.analysis.seamability.vertical_cost_milli,
    );
...
fn measured_axis_boundary_cost(
    ...
    let weights = normalized_weight_f64(request);
    let length = if seam.axis == SeamAxis::X {
        height
    } else {
        width
    };
    ...
    for line in 0..length {
        ...
        let gradient = f64::from(
            (periodic_luminance_gradient(base, a.0, a.1)
                - periodic_luminance_gradient(base, b.0, b.1))
            .abs(),
        );
        ...
        let structure = structure_penalty(request, source_pair.0)
            .max(structure_penalty(request, source_pair.1));
        total += weights[0] * color
            + weights[1] * gradient
            + weights[2] * height_cost
            + weights[3] * normal
            + weights[4] * roughness
...
fn output_normal_difference(
    ...
    channels
        ...
        .find_map(|channel| match channel {
            PreparedExemplarChannel::Normal { plane, .. } => {
                let na = normalize3(plane.pixel(a.0, a.1).xyz);
                let nb = normalize3(plane.pixel(b.0, b.1).xyz);
...
fn color_difference(a: &LinearColor, b: &LinearColor) -> f32 {
    (0..3)
        .map(|index| (a.rgb[index] - b.rgb[index]).abs())
        .sum::<f32>()
        / 3.0
}
...
fn luminance_gradient(plane: &ImagePlane<LinearColor>, x: u32, y: u32) -> f32 {
    ...
    let right = (x + 1).min(plane.width() - 1);
    let top = y.saturating_sub(1);
    let bottom = (y + 1).min(plane.height() - 1);
    let lum = |x, y| {
...
fn periodic_luminance_gradient(plane: &ImagePlane<LinearColor>, x: u32, y: u32) -> f32 {
    let left = (x + plane.width() - 1) % plane.width();
    let right = (x + 1) % plane.width();
    let top = (y + plane.height() - 1) % plane.height();
    let bottom = (y + 1) % plane.height();
    let lum = |x, y| {
        let c = plane.pixel(x, y).rgb;
        c[0] * 0.2126 + c[1] * 0.7152 + c[2] * 0.0722
    };
...
fn preflight(request: &DomainRequest, width: u32, height: u32) -> Result<(), DomainError> {
    ...
    let bytes = pixels
        .checked_mul(128 + channels * 24)
        .ok_or(DomainError::ResourceLimitExceeded)?;
    let search = u64::from(
        request
            .graph_cut
            .overlap_pixels
            .min(request.graph_cut.max_search_positions),
    );
    let operations = pixels
        .checked_mul(20 + channels * 8 + search * 3)
        .ok_or(DomainError::ResourceLimitExceeded)?;
...
fn weight_sum(weights: SeamWeights) -> u64 {
    [
        ...
        weights.gradient,
        weights.height,
        weights.vector_normal,
        weights.roughness,
        weights.structure_cut,
    ]
    .into_iter()
    .map(u64::from)
    .sum()
...
fn settings_hash(request: &DomainRequest) -> ContentDigest {
    ContentDigest::sha256(
        format!(
            "{:?}|{}|{}|{}|{}|{:?}|{}",
            request.graph_cut.axes,
            request.graph_cut.overlap_pixels,
            request.graph_cut.max_search_positions,
            request.graph_cut.continuity_milli,
            request.graph_cut.max_accepted_seam_cost_milli,
            request.graph_cut.weights,
            request.seed
        )
        .as_bytes(),
    )
...
fn active_sources(sample: &CorrespondenceSample) -> usize {
    sample.sources.iter().flatten().count()
}
...
fn plane<T: Clone>(
    ...
    tile_edge: u32,
    values: Vec<T>,
) -> Result<ImagePlane<T>, DomainError> {
    ImagePlane::from_row_major(width, height, tile_edge, &values)
        .map_err(|_| DomainError::PlaneConstruction)
}
...
mod tests {
    ...
    use hot_trimmer_image_io::{NormalAlphaPolicy, ResolutionPyramid, ResolvedAlphaMode};
    use hot_trimmer_material_analysis::{
        AnalysisSettings, FeatureFieldSettings, ReflectanceProvenance, RouteExecution,
        ScaleOrientationSettings, analyze_source, calibrate_scale_orientation,
        extract_feature_fields,
    };
    use hot_trimmer_material_analysis::{
        FeatureDebugView, FeatureFieldQa, GlobalOrientation, MeasurementOverlay,
        OrientationAuthority, OrientationOverlay, OverlayCoordinateSpace,
        PeriodicityEvidenceMethod, PeriodicityField, ScaleOrientationReport, SeamabilityField,
        StructureFields, UsabilityField,
    };
    ...
    fn scalar_plane(width: u32, height: u32, values: Vec<f32>) -> ImagePlane<LinearScalar> {
        plane(
            ...
            height,
            4,
            values.into_iter().map(LinearScalar).collect(),
        )
        .expect("scalar plane")
    }
    ...
    fn fixture(width: u32, height: u32, boundary_cost: u16) -> DomainRequest {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                normals.push(TangentNormal {
                    xyz: normalize3([(x as f32 / width as f32 - 0.5) * 0.3, 0.1, 1.0]),
                    alpha: 1.0,
                });
                ids.push(CategoryId(if x < width / 2 { 3 } else { 19 }));
                roughness.push(LinearScalar(0.2 + value * 0.6));
                opacity.push(MaskValue(
                    (x as f32 / (width - 1).max(1) as f32).clamp(0.0, 1.0),
                ));
            }
        }
        ...
        let source = DelitPreparedExemplar {
            ...
            channels: vec![
                ...
                PreparedExemplarChannel::Scalar {
                    ...
                    plane: plane(width, height, 4, roughness).expect("roughness"),
                },
                PreparedExemplarChannel::Normal {
                    plane: plane(width, height, 4, normals).expect("normals"),
                ...
                PreparedExemplarChannel::MaterialId {
                    plane: plane(width, height, 4, ids).expect("ids"),
                },
                PreparedExemplarChannel::Mask {
            ...
            route_execution: RouteExecution::PassThrough(
                hot_trimmer_domain::DelightingPassThroughReason::AuthoredTextureOrPbrSet,
            ),
        ...
        let scale_orientation = Arc::new(ScaleOrientationReport {
            ...
            prepared_source_digest: prepared.clone(),
            stage_five_cache_key: hot_trimmer_material_analysis::SourceAnalysisCacheKey(
                ContentDigest::sha256(b"stage-five"),
            ),
            scale: hot_trimmer_domain::PhysicalScaleEvidence::default(),
            scale_diagnostics: Vec::new(),
            global_orientation: GlobalOrientation {
                axis_millidegrees: None,
                measured_axis_millidegrees: None,
                anisotropy_milli: 0,
                confidence_milli: 0,
                authority: OrientationAuthority::UnavailableLowConfidence,
                destructive_rotation_allowed: false,
            },
            local_orientation: Vec::new(),
            measurement_overlay: MeasurementOverlay {
                ...
                world_scale_available: false,
                label: "relative-only fixture".into(),
            },
            orientation_overlay: OrientationOverlay {
                coordinate_space: OverlayCoordinateSpace::SourcePixels,
                global: GlobalOrientation {
                    axis_millidegrees: None,
                    measured_axis_millidegrees: None,
                    anisotropy_milli: 0,
                    confidence_milli: 0,
        ...
        let registered = ContentDigest::sha256(
            format!(
                "{}|{}|{}|{}|{}|{}",
                prepared.0,
                source.exemplar_id,
        ...
        let analysis = FeatureFieldReport {
            ...
            periodicity: PeriodicityField {
                confidence: pyramid(scalar_plane(width, height, zeros.clone())),
                candidates: Vec::new(),
                evidence_method: PeriodicityEvidenceMethod::BoundedNormalizedAutocorrelation,
            },
            seamability: SeamabilityField {
                confidence: pyramid(scalar_plane(
                    width,
                    height,
                    vec![0.8; (width * height) as usize],
                )),
                available_terms: BTreeSet::from([
                    SeamTerm::Color,
                    SeamTerm::Gradient,
                    SeamTerm::VectorNormal,
                    SeamTerm::Roughness,
                ...
                horizontal_cost_milli: boundary_cost,
        ...
        DomainRequest {
            ...
            graph_cut: GraphCutSettings {
                axes: ClosureAxes::XY,
                overlap_pixels: 3,
                max_search_positions: 3,
                continuity_milli: 20,
                ..GraphCutSettings::default()
            },
    ...
    fn algorithm_stage_08a_graphcut_registered_closure() {
        ...
        let domain = prepare_material_domain(&graph_request, &mut cache, &cancellation)
            .expect("graph-cut domain");
        ...
        assert!(
            domain
                .seams
                .iter()
                .find(|seam| seam.axis == SeamAxis::X)
                .expect("X seam")
                .positions
                .iter()
                .all(|position| *position > 0),
            "lower-cost seam must avoid structured boundary"
        );
        assert_eq!(
            domain.registered_channels().len(),
            graph_request.source.channels.len()
        );
        let RegisteredChannelRef::BaseColor(color) = domain
            .channel(MaterialChannelRole::BaseColor)
            .expect("base color")
        else {
            panic!("typed base color")
        };
        for y in 0..domain.height {
            assert_eq!(color.pixel(0, y), color.pixel(domain.width - 1, y));
        }
        for x in 0..domain.width {
            assert_eq!(color.pixel(x, 0), color.pixel(x, domain.height - 1));
        }
        let RegisteredChannelRef::Normal(normal) =
            domain.channel(MaterialChannelRole::Normal).expect("normal")
        else {
            panic!("typed normal")
        };
        for tile in normal.tiles() {
            for value in &tile.pixels {
                assert!((dot(value.xyz, value.xyz) - 1.0).abs() < 1.0e-5);
            }
        }
        let RegisteredChannelRef::MaterialId(ids) = domain
            .channel(MaterialChannelRole::MaterialId)
            .expect("IDs")
        else {
            panic!("typed IDs")
        };
        for tile in ids.tiles() {
            for id in &tile.pixels {
                assert!(matches!(id.0, 3 | 19), "IDs are selected, never blended");
            }
        }
        let RegisteredChannelRef::Mask(MaterialChannelRole::Opacity, opacity) = domain
            .channel(MaterialChannelRole::Opacity)
            .expect("opacity")
        else {
            panic!("typed opacity")
        };
        let source_opacity = graph_request
            .source
            .channels
            .iter()
            .find_map(|channel| match channel {
        ...
        let CorrespondenceField::Registered(correspondence) = &domain.correspondence else {
            panic!("registered correspondence")
        };
        ...
        for y in 0..domain.height {
            for x in 0..domain.width {
                ...
                if active_sources(sample) > 1 {
                    ...
                    assert!(
                        (opacity.pixel(x, y).0 - expected).abs() < 1.0e-6,
                        "opacity follows continuous weights"
                    );
                }
            }
        }
        ...
        assert_eq!(
            after.0, 0,
            "X Base Color and all available registered terms close exactly"
        );
        assert!(
            after.1 <= graph_request.graph_cut.max_accepted_seam_cost_milli,
            "post-closure multi-channel Y cost stays below the declared threshold"
        );
        assert!(
            after.0 < domain.diagnostics.boundary_cost_before_milli.0
        ...
        assert!(matches!(
            ...
            CorrespondenceField::Registered(_)
        ));
        assert!(matches!(domain.operations, OperationField::Registered(_)));
        assert_eq!(
            prepare_material_domain(&graph_request, &mut cache, &cancellation)
                .expect("cached")
                .cache_key,
            domain.cache_key
        );
        ...
        let direct = prepare_material_domain(&direct_request, &mut cache, &cancellation)
            .expect("direct domain");
        assert_eq!(direct.route, DomainRoute::DirectSource);
        assert!(matches!(
            direct.correspondence,
            CorrespondenceField::Identity { .. }
        ));
        ...
        assert!(
            matches!(
                prepare_material_domain(&unacceptable, &mut cache, &cancellation),
                Err(DomainError::UnacceptableSeam { .. })
            ),
            "visible high-cost closure must fail explicitly"
        );

        let mut foreign_analysis = fixture(12, 10, 700);
        Arc::make_mut(&mut foreign_analysis.analysis).prepared_source_digest =
            ContentDigest::sha256(b"foreign-prepared");
        assert_eq!(
            prepare_material_domain(&foreign_analysis, &mut cache, &cancellation),
            Err(DomainError::RegistrationDrift)
        );

        let mut bounded_direct = fixture(12, 10, 20);
        bounded_direct.graph_cut.max_working_bytes = 1;
        assert_eq!(
            prepare_material_domain(&bounded_direct, &mut cache, &cancellation),
            Err(DomainError::ResourceLimitExceeded)
        );
        ...
        assert!(matches!(
            prepare_material_domain(&insufficient, &mut cache, &cancellation),
            Err(DomainError::InsufficientOverlap {
                axis: SeamAxis::X,
                ..
            })
        ));
    ...
    fn quilting_fixture(duplicate_weight: u16) -> DomainRequest {
        ...
        request.quilting = QuiltingSettings {
            output_width: 27,
            output_height: 23,
            patch_size: QuiltingPatchSize::RelativeMilli {
                width: 500,
                height: 500,
            },
            overlap_milli: 400,
            ...
            candidate_count: 42,
            near_best_count: 1,
            near_best_threshold_milli: 0,
            weights: QuiltingWeights {
                overlap: 1,
                histogram: 0,
                structure: 0,
                duplicate_use: duplicate_weight,
                boundary_periodicity: 0,
            },
        ...
        for channel in &mut source.channels {
            match channel {
                PreparedExemplarChannel::BaseColor { plane: image, .. } => {
                    *image = plane(
                        12,
                        10,
                        4,
                ...
                PreparedExemplarChannel::Scalar { plane: image, .. } => {
                    *image = plane(12, 10, 4, vec![LinearScalar(0.5); 120]).unwrap()
                }
                PreparedExemplarChannel::Normal { plane: image, .. } => {
                    *image = plane(
                        12,
                        10,
                        4,
                ...
                PreparedExemplarChannel::MaterialId { plane: image } => {
                    *image = plane(12, 10, 4, vec![CategoryId(3); 120]).unwrap()
                }
                PreparedExemplarChannel::Mask { plane: image, .. } => {
                    *image = plane(12, 10, 4, vec![MaskValue(1.0); 120]).unwrap()
                }
            }
        }
        let analysis = Arc::make_mut(&mut request.analysis);
        let zero = || pyramid(scalar_plane(12, 10, vec![0.0; 120]));
        analysis.structure.edge = zero();
        analysis.structure.line = zero();
        analysis.structure.boundary = zero();
        analysis.structure.grid = zero();
        analysis.structure.fiber = zero();
        analysis.structure.intersection = zero();
        request
    }
    ...
    fn corpus_quilting_request(generator: SyntheticGenerator) -> DomainRequest {
        ...
        let generated = corpus
            .synthetic_fixtures
            .iter()
            .find(|fixture| fixture.generator == generator)
            .unwrap()
            .generate();
        let (width, height) = (generated.spec.width, generated.spec.height);
        let values: Vec<f32> = generated.planes["base_color"]
            .iter()
            ...
            match channel {
                ...
                PreparedExemplarChannel::Scalar { plane: image, .. } => {
                    *image = plane(
                        ...
                        height,
                        16,
                        values.iter().copied().map(LinearScalar).collect(),
                    )
                    .unwrap()
                }
                PreparedExemplarChannel::Normal { plane: image, .. } => {
                ...
                PreparedExemplarChannel::MaterialId { plane: image } => {
                    *image = plane(width, height, 16, vec![CategoryId(3); values.len()]).unwrap()
                }
                PreparedExemplarChannel::Mask { plane: image, .. } => {
                    *image = plane(width, height, 16, vec![MaskValue(1.0); values.len()]).unwrap()
                }
            }
        }
        source.original_prepared_base_color = source.base_color().clone();
        source.coverage =
            Some(plane(width, height, 16, vec![MaskValue(1.0); values.len()]).unwrap());
        let five = analyze_source(
            source,
        ...
        .unwrap();
        let six = calibrate_scale_orientation(
            source,
            &five,
            &hot_trimmer_domain::MaterialCalibrationIntent::default(),
            &ScaleOrientationSettings::default(),
            &RenderCancellationToken::new(),
        )
        .unwrap();
        let seven = extract_feature_fields(
            source,
            &six,
        ...
        .unwrap();
        request.scale_orientation = Arc::new(six);
        request.analysis = Arc::new(seven);
        request.route = DomainRoute::TextureQuilting;
        request.quilting = QuiltingSettings {
            output_width: 97,
            output_height: 89,
            ...
            near_best_count: 6,
            max_candidate_count: 64,
            max_output_pixels: 16_384,
            max_patch_count: 128,
            max_iterations: 128,
            max_working_bytes: 64_000_000,
            max_operations: 100_000_000,
            max_accepted_seam_cost_milli: 1000,
            max_boundary_periodicity_error_milli: 1000,
            ..QuiltingSettings::default()
        };
        request
    }
    ...
    fn dominant_axis_degrees(image: &ImagePlane<LinearColor>) -> f64 {
        ...
        let lum = |x, y| {
            let c = image.pixel(x, y).rgb;
            f64::from(c[0] * 0.2126 + c[1] * 0.7152 + c[2] * 0.0722)
        };
    ...
    fn algorithm_stage_08b_quilting() {
        ...
        let low_domain =
            prepare_material_domain(&low, &mut MaterialDomainCache::default(), &cancellation)
                .unwrap();
        let replay =
            prepare_material_domain(&low, &mut MaterialDomainCache::default(), &cancellation)
                .unwrap();
        assert_eq!(
            low_domain.correspondence, replay.correspondence,
            "fixed seed must replay exactly"
        );
        assert_eq!(low_domain.route, DomainRoute::TextureQuilting);
        assert!(
            low_domain.width > low.source.base_color().width()
                && low_domain.height > low.source.base_color().height()
        );
        let low_qa = low_domain.quilting.as_ref().unwrap();
        assert!(low_qa.placements.len() > 4 && !low_qa.overlap_seams.is_empty());
        assert!(
            low_domain.qa_views.contains(&DomainQaView::Quilting)
                && low_domain.qa_views.contains(&DomainQaView::SourceUsage)
        );
        let CorrespondenceField::Registered(correspondence) = &low_domain.correspondence else {
            panic!("registered quilting correspondence")
        };
        assert!(
            correspondence
                .tiles()
                ...
                .all(|sample| active_sources(sample) == 1),
            "one shared discrete seam cut must drive all channels"
        );
        let RegisteredChannelRef::Normal(normal) =
            low_domain.channel(MaterialChannelRole::Normal).unwrap()
        else {
            panic!("normal")
        };
        assert!(
        ...
        let high_domain =
            prepare_material_domain(&high, &mut MaterialDomainCache::default(), &cancellation)
                .unwrap();
        assert!(
            high_domain.quilting.as_ref().unwrap().source_usage.len() > low_qa.source_usage.len(),
        ...
        let stochastic_domain = prepare_material_domain(
            &stochastic,
            &mut MaterialDomainCache::default(),
            &cancellation,
        )
        ...
        let first_row: Vec<_> = stochastic_qa
            .placements
            .iter()
            .filter(|placement| placement.output_y == 0)
            .map(|placement| placement.output_x)
            .collect();
        let advances: BTreeSet<_> = first_row.windows(2).map(|pair| pair[1] - pair[0]).collect();
        assert!(
            advances.len() > 1,
            "stochastic corpus quilting must not use a fixed output grid advance"
        );
        let RegisteredChannelRef::BaseColor(stochastic_color) = stochastic_domain
            .channel(MaterialChannelRole::BaseColor)
            .unwrap()
        else {
            panic!("color")
        };
        let RegisteredChannelRef::Scalar(MaterialChannelRole::Roughness, stochastic_roughness) =
            stochastic_domain
                .channel(MaterialChannelRole::Roughness)
                .unwrap()
        else {
            panic!("roughness")
        };
        for y in 0..stochastic_domain.height {
            for x in 0..stochastic_domain.width {
                assert!(
                    (stochastic_color.pixel(x, y).rgb[0] - stochastic_roughness.pixel(x, y).0)
                        .abs()
                        < 1.0e-6,
        ...
        let mut exact_repeats = 0_u64;
        let mut comparisons = 0_u64;
        for y in 0..stochastic_domain.height {
            for x in 0..stochastic_domain.width - shift {
                exact_repeats +=
                    u64::from(stochastic_color.pixel(x, y) == stochastic_color.pixel(x + shift, y));
                comparisons += 1;
            }
        }
        assert!(
            exact_repeats * 5 < comparisons,
            "corpus expansion must not expose a dominant fixed-patch repeat"
        );

        let mut directional_corpus = corpus_quilting_request(SyntheticGenerator::Orientation);
        let stage6_axis = directional_corpus
            .scale_orientation
            .global_orientation
            .axis_millidegrees
        ...
        let directional_domain = prepare_material_domain(
            &directional_corpus,
            &mut MaterialDomainCache::default(),
            &cancellation,
        )
        .unwrap();
        let RegisteredChannelRef::BaseColor(directional_output) = directional_domain
            .channel(MaterialChannelRole::BaseColor)
            .unwrap()
        else {
            panic!("directional output")
        };
        assert!(
            axis_difference_degrees(measured_input, dominant_axis_degrees(directional_output))
                <= 8.0,
        ...
        assert!(
            matches!(
                ...
                Err(DomainError::IncompatibleQuilting { .. })
            ),
            "unique semantics must route away, never smear"
        );

        let mut physical = quilting_fixture(1000);
        physical.quilting.patch_size = QuiltingPatchSize::PhysicalMicrometers {
            width: 6_000,
            height: 5_000,
        };
        assert!(
            ...
            "relative/prior Stage 6 scale must not authorize physical sizing"
        );
        ...
        let physical_domain = prepare_material_domain(
            &physical,
            &mut MaterialDomainCache::default(),
            &cancellation,
        )
        .unwrap();
        assert_eq!(
            physical_domain.quilting.as_ref().unwrap().patch_size_pixels,
            (6, 5)
        );
        ...
        boundary_rejected
            .quilting
            .max_boundary_periodicity_error_milli = 0;
        assert!(matches!(
            prepare_material_domain(
                &boundary_rejected,
                &mut MaterialDomainCache::default(),
        ...
        Arc::make_mut(&mut unusable.analysis).usability.confidence =
            pyramid(scalar_plane(12, 10, vec![0.0; 120]));
        assert!(
            matches!(prepare_material_domain(&unusable, &mut MaterialDomainCache::default(), &cancellation),
            Err(DomainError::UnusableQuiltingSource { rejected_candidates }) if rejected_candidates > 0)
        );
        let cancelled = RenderCancellationToken::new();
        cancelled.cancel();
        assert_eq!(
            prepare_material_domain(&low, &mut MaterialDomainCache::default(), &cancelled),
            Err(DomainError::Cancelled)
        );
        let mut bounded = quilting_fixture(1000);
        bounded.quilting.max_working_bytes = 1;
        assert_eq!(
            prepare_material_domain(&bounded, &mut MaterialDomainCache::default(), &cancellation),
            Err(DomainError::ResourceLimitExceeded)
        );
    }
    ...
    fn patchmatch_settings(width: u32, height: u32) -> PatchMatchSettings {
        PatchMatchSettings {
            ...
            random_search_radius: 32,
            random_candidates_per_radius: 2,
            convergence_change_threshold_milli: 50,
            max_output_pixels: 32_768,
            max_working_bytes: 64_000_000,
    ...
    fn algorithm_stage_08c_patchmatch() {
        ...
        let domain = prepare_material_domain(
            &stochastic,
            &mut MaterialDomainCache::default(),
            &cancellation,
        )
        .unwrap();
        let replay = prepare_material_domain(
            &stochastic,
            &mut MaterialDomainCache::default(),
            &cancellation,
        )
        .unwrap();
        assert_eq!(
            domain.correspondence, replay.correspondence,
            "NNF must replay byte-exactly"
        );
        assert_eq!(
            domain.patch_match, replay.patch_match,
            "diagnostics must replay byte-exactly"
        );
        let qa = domain.patch_match.as_ref().unwrap();
        assert!(qa.converged && qa.incomplete_pixels == 0 && qa.operation_count > 0);
        assert!(
            qa.final_changed_pixels * 1000 <= qa.synthesized_pixels * 50,
            "convergence requires actual NNF stabilization below the configured changed fraction"
        );
        assert_eq!(
            qa.boundary_error_milli,
            (0, 0),
            "seamless expansion boundaries are measured after reconstruction"
        );
        assert!(
            domain
                .qa_views
                .contains(&DomainQaView::NearestNeighborField)
                && domain.qa_views.contains(&DomainQaView::Coherence)
                && domain.qa_views.contains(&DomainQaView::SourceUsage)
        );
        let CorrespondenceField::Registered(nnf) = &domain.correspondence else {
            panic!("registered NNF")
        };
        assert!(
            nnf.tiles()
                .iter()
        ...
        let RegisteredChannelRef::BaseColor(color) =
            domain.channel(MaterialChannelRole::BaseColor).unwrap()
        else {
            panic!("color")
        };
        let RegisteredChannelRef::Scalar(MaterialChannelRole::Roughness, roughness) =
            domain.channel(MaterialChannelRole::Roughness).unwrap()
        else {
            panic!("roughness")
        };
        for y in 0..domain.height {
            for x in 0..domain.width {
                assert!(
                    (color.pixel(x, y).rgb[0] - roughness.pixel(x, y).0).abs() < 1.0e-6,
                    "one NNF must reconstruct every registered channel"
                );
            }
        }
        let RegisteredChannelRef::Normal(normal) =
            domain.channel(MaterialChannelRole::Normal).unwrap()
        else {
            panic!("normal")
        };
        assert!(
            normal
                ...
                .iter()
                .flat_map(|tile| &tile.pixels)
                .all(|n| (dot(n.xyz, n.xyz) - 1.0).abs() < 1.0e-6)
        );
        ...
        directional.patch_match.semantics = PatchMatchSemanticConstraint::Directional {
            behavior: hot_trimmer_domain::MaterialBehaviorClass::StochasticDirectional,
            requested_angle_millidegrees: axis as i32,
            tolerance_millidegrees: 8_000,
        };
        ...
        .unwrap();
        let RegisteredChannelRef::BaseColor(output) = directional_domain
            .channel(MaterialChannelRole::BaseColor)
            .unwrap()
        else {
            panic!("output")
        ...
        assert!(
            axis_difference_degrees(measured, dominant_axis_degrees(output)) <= 8.0,
            "unrotated constrained correspondences must preserve direction"
        );

        let mut completion = fixture(12, 10, 700);
        completion.route = DomainRoute::PatchMatch;
        completion.patch_match = patchmatch_settings(12, 10);
        completion.patch_match.seamless_x = false;
        completion.patch_match.seamless_y = false;
        completion.patch_match.semantics = PatchMatchSemanticConstraint::ProtectedUniqueCompletion;
        completion.patch_match.completion_mask = Some(Arc::new(
            plane(
                12,
                10,
                4,
                (0..120)
                    .map(|i| {
                        MaskValue(
                            if (i % 12) >= 4 && (i % 12) <= 7 && (i / 12) >= 3 && (i / 12) <= 6 {
                                1.0
                            } else {
                                0.0
                            },
        ...
        completion.patch_match.source_exclusion_mask = Some(Arc::new(
            plane(
                12,
                10,
                4,
                (0..120)
                    .map(|i| MaskValue(if i % 12 < 3 { 1.0 } else { 0.0 }))
                    .collect(),
            )
            .unwrap(),
        ));
        let completed = prepare_material_domain(
            &completion,
            &mut MaterialDomainCache::default(),
            &cancellation,
        )
        .unwrap();
        let CorrespondenceField::Registered(completion_nnf) = &completed.correspondence else {
            panic!("completion NNF")
        };
        for y in 3..=6 {
            for x in 4..=7 {
                assert!(
                    completion_nnf.pixel(x, y).sources[0].unwrap().coordinate.x >= 3,
                    "completion may never sample an excluded source region"
                );
            }
        }
        assert_eq!(
            completed.patch_match.as_ref().unwrap().preserved_pixels,
            104
        );

        let mut unique = stochastic.clone();
        unique.patch_match.semantics = PatchMatchSemanticConstraint::UniqueDetail;
        assert!(
            matches!(
                prepare_material_domain(
                    &unique,
            ...
            "unique content must not repeat without explicit compatible constraints"
        );
        let mut manufactured = stochastic.clone();
        manufactured.patch_match.semantics = PatchMatchSemanticConstraint::ManufacturedPattern;
        assert!(matches!(
            prepare_material_domain(
                &manufactured,
                &mut MaterialDomainCache::default(),
        ...
        let mut nonconverged = stochastic.clone();
        nonconverged.patch_match.iterations_per_level = 1;
        ...
        assert!(
            matches!(
                ...
                Err(DomainError::PatchMatchNonConverged { .. })
            ),
            "nonconvergence must never publish output"
        );
        let mut bounded = stochastic.clone();
        bounded.patch_match.max_working_bytes = 1;
        assert_eq!(
            prepare_material_domain(&bounded, &mut MaterialDomainCache::default(), &cancellation),
            Err(DomainError::ResourceLimitExceeded)
        );
        ...
        let legacy_estimate = output_pixels
            * (96 + descriptor_bounded.source.channels.len() as u64 * 24)
            + source_pixels * 24;
        let actual_requirement = patchmatch_required_working_bytes(&descriptor_bounded).unwrap();
        assert!(
            actual_requirement > legacy_estimate,
            "the complete typed descriptor pyramid must be charged at its actual allocation size"
        );
        ...
        assert_eq!(
            ...
            Err(DomainError::ResourceLimitExceeded),
            "preflight must reject before allocating a descriptor pyramid over budget"
        );
        let cancelled = RenderCancellationToken::new();
        cancelled.cancel();
        assert_eq!(
            prepare_material_domain(&stochastic, &mut MaterialDomainCache::default(), &cancelled),
            Err(DomainError::Cancelled)
        );
    }
    ...
    fn router_fixture(behavior: hot_trimmer_domain::MaterialBehaviorClass) -> Stage8RouterRequest {
        ...
        domain.patch_match = PatchMatchSettings {
            ...
            random_search_radius: 4,
            random_candidates_per_radius: 1,
            max_iterations: 4,
            max_operations: 1_000_000,
            max_working_bytes: 4_000_000,
            ..PatchMatchSettings::default()
        };
        let mut stage_five = analyze_source(
            &domain.source,
            &AnalysisSettings::default(),
            None,
            &cancellation,
        ...
        Arc::make_mut(&mut domain.scale_orientation).stage_five_cache_key =
            stage_five.cache_key.clone();
        Stage8RouterRequest {
            domain,
            stage_five: Arc::new(stage_five),
            policy: MaterialDomainRoutePolicy::default(),
            procedural_override: None,
            procedural_settings: ProceduralFitSettings {
                limits: ProceduralLimits {
                    max_output_pixels: 65_536,
                    max_working_bytes: 16_000_000,
                    max_operations: 100_000_000,
                    max_fit_samples: 4096,
                },
                ..ProceduralFitSettings::default()
            },
            output_width: 12,
            output_height: 10,
        }
    ...
    struct TestLearnedProvider {
        device: LearnedExecutionDevice,
        include_estimated_maps: bool,
    }
    impl LocalLearnedMaterialProvider for TestLearnedProvider {
        fn descriptor(&self) -> LearnedProviderDescriptor {
            LearnedProviderDescriptor {
                provider_id: "approved-test-provider".into(),
                provider_version: "1.0.0".into(),
                interface_version: LEARNED_PROVIDER_INTERFACE_VERSION,
                model_digest: ContentDigest::sha256(b"approved-test-model"),
                capabilities: if self.include_estimated_maps {
                    BTreeSet::from([
                        LearnedCapability::SeamlessExpansion,
                        LearnedCapability::EstimatedHeight,
                        LearnedCapability::EstimatedNormal,
                    ])
                } else {
                    BTreeSet::from([LearnedCapability::SeamlessExpansion])
                },
                device_policy: LearnedDevicePolicy::CpuOnly,
                deterministic: true,
                maximum_input_pixels: 1024,
                maximum_output_pixels: 1024,
        ...
        fn infer(
            ...
            _: &RenderCancellationToken,
        ) -> Result<LearnedMaterialOutput, LearnedProviderError> {
            if !self.include_estimated_maps
                && request.requested.iter().any(|capability| {
                    matches!(
                        capability,
            ...
            let channels = request
                ...
                .map(|channel| match channel {
                    PreparedExemplarChannel::BaseColor { plane, alpha_mode } => {
                        PreparedExemplarChannel::BaseColor {
                            plane: ImagePlane::from_row_major(
                                ...
                                4,
                                &vec![*plane.pixel(0, 0); count],
                            )
                            .unwrap(),
                            alpha_mode: *alpha_mode,
                        }
                    }
                    PreparedExemplarChannel::Scalar { role, plane } => {
                        PreparedExemplarChannel::Scalar {
                            role: *role,
                            plane: ImagePlane::from_row_major(
                                request.output_width,
                    ...
                    PreparedExemplarChannel::Mask { role, plane } => {
                        PreparedExemplarChannel::Mask {
                            role: *role,
                            plane: ImagePlane::from_row_major(
                                request.output_width,
                ...
                .collect::<Vec<_>>();
            Ok(LearnedMaterialOutput {
                output_digest: canonical_learned_output_digest(&channels),
                channels,
                confidence_milli: 800,
                model_digest: ContentDigest::sha256(b"approved-test-model"),
                deterministic: true,
                device: self.device,
                diagnostics: vec!["fixture provider".into()],
            })
        }
    }
    ...
    fn algorithm_stage_08_router_domain_route_goldens() {
        ...
        for behavior in hot_trimmer_domain::MaterialBehaviorClass::ALL {
            ...
            let first = prepare_stage_08_material_domain(
                &request,
                None,
                &mut MaterialDomainCache::default(),
                &cancellation,
            ...
            match first {
                Ok(result) => {
                    let allowed: &[DomainRoute] = match behavior {
                        hot_trimmer_domain::MaterialBehaviorClass::AlreadyTileable => &[
                            DomainRoute::DirectSource,
                            DomainRoute::GraphCutPeriodicClosure,
                        ],
                        hot_trimmer_domain::MaterialBehaviorClass::StochasticIsotropic => &[
                        ...
                        hot_trimmer_domain::MaterialBehaviorClass::UniqueDetail
                        | hot_trimmer_domain::MaterialBehaviorClass::RadialDetail => {
                            &[DomainRoute::DirectSource]
                        }
                        hot_trimmer_domain::MaterialBehaviorClass::MixedUnknown => &[
                            DomainRoute::GraphCutPeriodicClosure,
                            DomainRoute::TextureQuilting,
                            DomainRoute::PatchMatch,
                        ],
                        hot_trimmer_domain::MaterialBehaviorClass::PeriodicLatticeStructured
                        | hot_trimmer_domain::MaterialBehaviorClass::ManufacturedPattern => panic!(
                            "structured fixture without period evidence must be insufficient"
                        ),
                    };
                    assert!(
                        allowed.contains(&result.domain.route),
                        "unexpected {behavior:?} route {:?}",
                        result.domain.route
                    );
                    ...
                    assert!(
                        result.diagnostics.registration_valid
                            && result.diagnostics.seam_or_period_valid
                            && result.diagnostics.correspondence_valid
                            && result.diagnostics.deterministic
                            && result.diagnostics.cache_provenance_valid
                    );
                    let replay = prepare_stage_08_material_domain(
                        &request,
                        None,
                        &mut MaterialDomainCache::default(),
                        &cancellation,
                    ...
                    .unwrap();
                    assert_eq!(
                        result.domain, replay.domain,
                        "{behavior:?} must replay byte-identically"
                    );
                }
                Err(Stage8RouterError::ActionableInsufficiency(diagnostics)) => {
                    ...
                    if matches!(
                        ...
                        assert!(diagnostics.compared_routes.iter().any(|route| {
                            route.route == DomainRoute::DirectSource
                                && !route.applicable
                                && route
                                    .rejection
                                    .as_deref()
                                    .is_some_and(|reason| reason.contains("period"))
                        }));
                    }
                }
                Err(error) => {
                    panic!("unexpected router contract failure for {behavior:?}: {error}")
                }
            }
        }

        let mut statistical =
            router_fixture(hot_trimmer_domain::MaterialBehaviorClass::StochasticIsotropic);
        statistical.policy.pinned_route = Some(DomainRoute::StatisticalSynthesis);
        let statistical_result = prepare_stage_08_material_domain(
            &statistical,
            None,
            &mut MaterialDomainCache::default(),
            &cancellation,
        ...
        .unwrap();
        assert_eq!(
            statistical_result.domain.route,
            DomainRoute::StatisticalSynthesis
        );
        let CorrespondenceField::Registered(field) = &statistical_result.domain.correspondence
        else {
            panic!("registered statistical field")
        };
        let coherent = (0..field.height())
            .flat_map(|y| {
                (0..field.width() - 1).map(move |x| {
            ...
            .count();
        assert!(
            coherent * 2 > (field.width() as usize - 1) * field.height() as usize,
            "statistical synthesis must retain local neighborhoods, not independently resample pixels"
        );
        let mut transposed = statistical.clone();
        transposed.output_width = 10;
        transposed.output_height = 12;
        let transposed_result = prepare_stage_08_material_domain(
            &transposed,
            None,
            &mut MaterialDomainCache::default(),
            &cancellation,
        ...
        .unwrap();
        assert_ne!(
            statistical_result.domain.cache_key, transposed_result.domain.cache_key,
        ...
        let Err(Stage8RouterError::ActionableInsufficiency(pinned_diagnostics)) =
            ...
            panic!("inapplicable pin must fail closed")
        };
        ...
        let provider = TestLearnedProvider {
            device: LearnedExecutionDevice::Cpu,
            include_estimated_maps: true,
        };
        let learned_result = prepare_stage_08_material_domain(
            &learned,
            Some(&provider),
            &mut MaterialDomainCache::default(),
            &cancellation,
        ...
        assert!(matches!(
            learned_result.diagnostics.learned_provider,
            LearnedProviderState::Used { .. }
        ));
        let expansion_only = TestLearnedProvider {
            device: LearnedExecutionDevice::Cpu,
            include_estimated_maps: false,
        };
        let expansion_result = prepare_stage_08_material_domain(
            &learned,
            Some(&expansion_only),
            &mut MaterialDomainCache::default(),
            &cancellation,
        ...
        .unwrap();
        assert_eq!(
            expansion_result.domain.route,
            DomainRoute::LearnedProvider,
            "estimated PBR maps are optional for a seamless-expansion provider"
        );
        let wrong_device = TestLearnedProvider {
            device: LearnedExecutionDevice::Gpu,
            include_estimated_maps: true,
        };
        let Err(Stage8RouterError::ActionableInsufficiency(device_diagnostics)) =
            prepare_stage_08_material_domain(
                &learned,
                Some(&wrong_device),
                &mut MaterialDomainCache::default(),
            ...
            panic!("device mismatch must fail closed")
        };
        assert!(device_diagnostics.compared_routes.iter().any(|route| {
            route.route == DomainRoute::LearnedProvider
                && route
                    .rejection
                    .as_deref()
                    .is_some_and(|reason| reason.contains("device-policy"))
        }));

        let mut policy = MaterialDomainRoutePolicy::default();
        policy.apply(RoutePolicyCommand::Override(
            DomainRoute::GraphCutPeriodicClosure,
        ));
