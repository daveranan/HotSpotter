...
pub enum EffectScaleSpace { World, SlotMinorRelative, SlotMajorRelative, SlotAreaRelative, Pixels }

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SlotAxis { X, Y }

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum MirrorPolicy { Forbidden, Allowed }

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum VisualImportance { Background, Standard, Hero }

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SourceFootprintUnit { SourcePixels, RelativeTexels }

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum WorldDimensionSource { Stage9Authored, DestinationTexelDensity }
...
pub enum EffectVariant {
    Full, Simplified, Strip, Radial, Cap, MergedRoundedProfile, FullyRoundedProfile,
    NormalOnly, RoughnessOnly, Disabled,
}
...
pub enum CapacityDiagnosticCode {
    RelativeSourceFootprint, OpposingProfilesDoNotFit, FlatCenterImpossible,
    IsotropicFeatureDoesNotFit, FeatureBelowRasterThreshold, SupersampleLimitReached,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum DiagnosticSeverity { Info, Warning, Insufficient }
...
#[derive(Clone, Copy, Debug, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PhysicalFeatureIntent { pub value: f64, pub scale_space: EffectScaleSpace }
...
pub fn resolve_slot_demands_with_guard(
    ...
    Ok(ResolvedSlotDemandSet {
        stage_result: StageResult::Executed {
            algorithm: AlgorithmProvenance { algorithm_id: STAGE_10_ALGORITHM_ID.into(), version: STAGE_10_ALGORITHM_VERSION.into() },
...
pub fn resolve_slot_demand(
    ...
    let (world_width, world_height) = match intent.world_dimension_source {
        WorldDimensionSource::Stage9Authored => (
            region.uv_fit.world_size_meters[0], region.uv_fit.world_size_meters[1],
        ),
    ...
    let required_source_footprint = source_footprint(
        world_width, world_height, pixel_width, pixel_height, intent.source_scale,
    );
    let minimum_density = ppm_x.min(ppm_y);
    let resolved_features = intent.requested_features.iter()
    ...
    let required_supersampling = supersample_factor(
        resolved_features.iter().map(|feature| feature.raster_width_px)
            .chain(std::iter::once(intent.minimum_survivable_feature_m * minimum_density)),
    );
    let effect_capacity = capacity(
        world_width, world_height, minor_axis_m, minimum_density,
        intent.minimum_flat_center_m, required_supersampling, region.role,
    );
    ...
    diagnose_physical_fit(intent, &resolved_features, &effect_capacity, minor_axis_m, &mut diagnostics);
    ...
    let supported_feature_lods = supported_lods(effect_capacity.maximum_isotropic_feature_m * minimum_density);

    Ok(ResolvedSlotDemand {
        slot_id: region.id, slot_role: region.role, orientation: region.orientation,
        hotspot_rect: region.hotspot_rect, allocation_rect: region.allocation_rect,
        destination_pixel_width: pixel_width, destination_pixel_height: pixel_height,
        world_width_m: world_width, world_height_m: world_height,
        world_dimension_source: intent.world_dimension_source,
        major_axis, minor_axis, major_axis_m, minor_axis_m,
        aspect_ratio: world_width / world_height,
        pixels_per_meter_x: ppm_x, pixels_per_meter_y: ppm_y,
        meters_per_pixel_x: world_width / f64::from(pixel_width),
        meters_per_pixel_y: world_height / f64::from(pixel_height),
        desired_texel_density: intent.desired_texel_density, required_source_footprint,
        mapping_mode, allowed_mapping_modes,
        allowed_rotations: region.uv_fit.allowed_rotations.clone(),
        mirror_policy: if region.uv_fit.mirror_allowed { MirrorPolicy::Allowed } else { MirrorPolicy::Forbidden },
        material_group: region.material_group.clone(), variation_group: region.weathering_group.clone(),
        profile_type: region.structural_profile, weathering_class: region.weathering_group.clone(),
        ...
        required_supersampling, supported_feature_lods, effect_capacity, diagnostics,
    })
}
...
pub fn effect_scale_to_slot_meters(
    space: EffectScaleSpace, value: f64, slot: &ResolvedSlotDemand,
) -> [f64; 2] {
    match space {
        ...
        EffectScaleSpace::Pixels => [value * slot.meters_per_pixel_x, value * slot.meters_per_pixel_y],
    }
}
...
fn resolve_feature_scale(
    ...
    let physical_width_m = match feature.scale_space {
        ...
        EffectScaleSpace::SlotAreaRelative => Some(feature.value * (world_width * world_height).sqrt()),
...
fn validate(region: &RegionDefinition, intent: &SlotDemandIntent) -> Result<(), SlotDemandError> {
    ...
    if !world_width.is_finite() || !world_height.is_finite() || world_width <= 0.0 || world_height <= 0.0 {
        return Err(SlotDemandError::InvalidWorldDimensions);
    }
    if !intent.desired_texel_density.is_finite() || intent.desired_texel_density <= 0.0
        || !intent.minimum_survivable_feature_m.is_finite() || intent.minimum_survivable_feature_m <= 0.0
        || !intent.minimum_flat_center_m.is_finite() || intent.minimum_flat_center_m < 0.0
        || intent.requested_features.iter().any(|f| !f.value.is_finite() || f.value <= 0.0)
        || intent.opposing_profile_widths_m.is_some_and(|v| v.iter().any(|x| !x.is_finite() || *x < 0.0))
    ...
    if matches!(scale.world_scale, WorldScaleAvailability::Available) && !scale.claims_world_accuracy() {
        return Err(SlotDemandError::InvalidSourceScale);
    }
    Ok(())
}
...
fn source_footprint(
    world_width: f64, world_height: f64, pixel_width: u32, pixel_height: u32,
    scale: PhysicalScaleEvidence,
) -> RequiredSourceFootprint {
    if scale.claims_world_accuracy() {
        let ppm_x = scale.source_pixels_per_meter_x_milli.expect("validated x scale") as f64 / 1_000.0;
        let ppm_y = scale.source_pixels_per_meter_y_milli.expect("validated y scale") as f64 / 1_000.0;
        RequiredSourceFootprint {
            width: world_width * ppm_x, height: world_height * ppm_y,
            unit: SourceFootprintUnit::SourcePixels, scale_provenance: scale.provenance,
            world_scale: scale.world_scale, confidence_milli: scale.confidence_milli,
        }
    } else {
        let area = f64::from(pixel_width) * f64::from(pixel_height);
        let aspect = f64::from(pixel_width) / f64::from(pixel_height);
        RequiredSourceFootprint {
            width: (area * aspect).sqrt(), height: (area / aspect).sqrt(),
            unit: SourceFootprintUnit::RelativeTexels, scale_provenance: scale.provenance,
            world_scale: scale.world_scale, confidence_milli: scale.confidence_milli,
        }
    }
}
...
fn capacity(
    width_m: f64, height_m: f64, minor_m: f64, density: f64, flat_m: f64,
    supersample: u8, role: TemplateSlotRole,
) -> EffectCapacity {
    let horizontal = (width_m - flat_m).max(0.0) / 2.0;
    let vertical = (height_m - flat_m).max(0.0) / 2.0;
    let isotropic = width_m.min(height_m);
    let mut variants = vec![EffectVariant::Full, EffectVariant::Simplified,
        EffectVariant::NormalOnly, EffectVariant::RoughnessOnly, EffectVariant::Disabled];
    ...
    variants.push(if minor_m < flat_m { EffectVariant::FullyRoundedProfile }
        else { EffectVariant::MergedRoundedProfile });
    variants.sort(); variants.dedup();
    EffectCapacity {
        can_have_flat_center: minor_m >= flat_m, minimum_flat_center_m: flat_m,
        maximum_left_profile_width_m: horizontal, maximum_right_profile_width_m: horizontal,
        maximum_top_profile_width_m: vertical, maximum_bottom_profile_width_m: vertical,
        maximum_isotropic_feature_m: isotropic, maximum_radial_feature_m: isotropic / 2.0,
        ...
        recommended_supersample_factor: supersample, allowed_effect_variants: variants,
    }
}

fn diagnose_physical_fit(
    intent: &SlotDemandIntent, features: &[ResolvedFeatureScale], capacity: &EffectCapacity, minor_m: f64,
    ...
    for feature in features {
        if feature.physical_width_m.is_some_and(|width| width > capacity.maximum_isotropic_feature_m) {
            ...
            diagnostics.push(CapacityDiagnostic {
                ...
                measurements: BTreeMap::from([("feature_width_m".into(), physical_width_m),
                    ("maximum_isotropic_feature_m".into(), capacity.maximum_isotropic_feature_m)]),
                recovery_choices: vec![RecoveryChoice::AdjustSettings, RecoveryChoice::DisableEffect],
            });
        }
    }
...
fn diagnose_raster_fit(
    features: &[ResolvedFeatureScale], supersample: u8,
    diagnostics: &mut Vec<CapacityDiagnostic>,
) {
    if let Some(pixels) = features.iter().map(|feature| feature.raster_width_px).reduce(f64::min) {
        if pixels < 1.5 {
            diagnostics.push(CapacityDiagnostic {
                code: CapacityDiagnosticCode::FeatureBelowRasterThreshold,
                severity: DiagnosticSeverity::Warning,
                message: "the narrowest feature needs a reduced LOD at this output resolution".into(),
...
fn supersample_factor(widths_px: impl Iterator<Item = f64>) -> u8 {
    ...
    if needed <= 1.0 { 1 } else if needed <= 2.0 { 2 } else if needed <= 4.0 { 4 } else { 8 }
}

fn supported_lods(maximum_pixels: f64) -> Vec<FeatureLod> {
    let mut lods = vec![FeatureLod::RoughnessColorIndication];
    if maximum_pixels >= 1.5 { lods.push(FeatureLod::NormalRoughness); }
    if maximum_pixels >= 3.0 { lods.push(FeatureLod::SimplifiedHeightNormalRoughness); }
    if maximum_pixels >= 6.0 { lods.push(FeatureLod::FullHeightNormalColorRoughnessAo); }
    lods
}

fn mapping_modes(role: TemplateSlotRole, orientation: RegionOrientation) -> (SamplingMode, Vec<SamplingMode>) {
    let modes = match role {
        TemplateSlotRole::Planar => vec![SamplingMode::DirectCrop, SamplingMode::PeriodicTile,
            SamplingMode::TextureSynthesis, SamplingMode::NineSlicePanel],
        TemplateSlotRole::RepeatingStrip => vec![if orientation == RegionOrientation::Vertical {
            SamplingMode::RepeatY } else { SamplingMode::RepeatX }, SamplingMode::DirectCrop,
            SamplingMode::TextureSynthesis],
        TemplateSlotRole::UniqueDetail => vec![SamplingMode::UniqueContain, SamplingMode::UniqueCover],
...
fn compilation_diagnostic(diagnostic: &CapacityDiagnostic) -> CompilationDiagnostic {
    CompilationDiagnostic {
        ...
        stage: Some(10), message: diagnostic.message.clone(),
        context: diagnostic.measurements.iter().map(|(k, v)| (k.clone(), v.to_string())).collect(),
    }
}
...
mod tests {
    ...
    fn region(role: TemplateSlotRole, orientation: RegionOrientation, world: [f64; 2]) -> RegionDefinition {
        RegionDefinition {
            id: RegionId::from_bytes([7; 16]), display_name: "fixture".into(),
            id_color: IdColor([1, 2, 3]),
            allocation_rect: CanonicalRect { x: 0, y: 0, width: 2048, height: 2048 },
            hotspot_rect: CanonicalRect { x: 16, y: 16, width: 2016, height: 2016 },
            role, orientation,
            uv_fit: UvFitPolicy {
                kind: match role {
                ...
                fit_axis: FitAxis::Automatic, keep_proportion: true,
                allowed_rotations: vec![QuarterTurn::Zero],
                mirror_allowed: role != TemplateSlotRole::Radial,
                world_size_meters: world, classification_tags: Vec::new(),
            },
    ...
    fn intent(pixels: [u32; 2]) -> SlotDemandIntent {
        SlotDemandIntent {
            destination_rect: CanonicalRect { x: 0, y: 0, width: pixels[0], height: pixels[1] },
            ...
            minimum_survivable_feature_m: 0.01, minimum_flat_center_m: 0.04,
            requested_features: vec![PhysicalFeatureIntent {
                value: 0.01, scale_space: EffectScaleSpace::World,
    ...
    fn algorithm_stage_10_slot_capacity() {
        let broad_region = region(TemplateSlotRole::Planar, RegionOrientation::Horizontal, [4.0, 2.0]);
        let broad_intent = intent([2048, 1024]);
        let broad = resolve_slot_demand(&broad_region, &broad_intent).unwrap();
        assert_eq!((broad.destination_pixel_width, broad.destination_pixel_height), (2048, 1024));
        close(broad.pixels_per_meter_x, 512.0); close(broad.pixels_per_meter_y, 512.0);
        close(broad.effect_capacity.maximum_isotropic_feature_m, 2.0);
        close(broad.effect_capacity.maximum_radial_feature_m, 1.0);
        assert_eq!(broad.required_source_footprint.unit, SourceFootprintUnit::RelativeTexels);
        close(broad.required_source_footprint.width, 2048.0);
        assert_eq!(broad.major_axis, SlotAxis::X);
        assert!(broad.diagnostics.iter().any(|d| d.code == CapacityDiagnosticCode::RelativeSourceFootprint));

        let horizontal = resolve_slot_demand(
            &region(TemplateSlotRole::RepeatingStrip, RegionOrientation::Horizontal, [10.0, 0.5]),
            &intent([320, 16]),
        ).unwrap();
        let vertical = resolve_slot_demand(
            &region(TemplateSlotRole::RepeatingStrip, RegionOrientation::Vertical, [0.5, 10.0]),
            &intent([16, 320]),
        ).unwrap();
        close(horizontal.minor_axis_m, 0.5); close(vertical.minor_axis_m, 0.5);
        close(horizontal.effect_capacity.maximum_isotropic_feature_m,
        ...
        let radial = resolve_slot_demand(
            &region(TemplateSlotRole::Radial, RegionOrientation::Unspecified, [1.0, 1.0]),
            &intent([256, 256]),
        ).unwrap();
        close(radial.effect_capacity.maximum_radial_feature_m, 0.5);
        assert_eq!(radial.mirror_policy, MirrorPolicy::Forbidden);
        assert!(radial.allowed_mapping_modes.contains(&SamplingMode::PolarRadial));

        let cap = resolve_slot_demand(
            &region(TemplateSlotRole::TrimCap, RegionOrientation::Horizontal, [0.8, 0.4]),
            &intent([128, 64]),
        ).unwrap();
        assert_eq!(cap.mapping_mode, SamplingMode::ThreeSliceCap);
        assert!(cap.effect_capacity.allowed_effect_variants.contains(&EffectVariant::Cap));
        ...
        let subpixel = resolve_slot_demand(
            &region(TemplateSlotRole::RepeatingStrip, RegionOrientation::Horizontal, [4.0, 0.25]),
            &subpixel_intent,
        ).unwrap();
        assert_eq!(subpixel.required_supersampling, 8);
        assert_eq!(subpixel.minimum_survivable_feature_m, 0.000_1);
        assert!(subpixel.diagnostics.iter().any(|d| d.code == CapacityDiagnosticCode::FeatureBelowRasterThreshold));
        ...
        let opposing = resolve_slot_demand(
            &region(TemplateSlotRole::RepeatingStrip, RegionOrientation::Horizontal, [10.0, 0.1]),
            &opposing_intent,
        ).unwrap();
        assert!(!opposing_profiles_fit(0.1, 0.03, 0.03, 0.06));
        close(opposing.maximum_bevel_width_m, 0.02);
        assert!(opposing.diagnostics.iter().any(|d| d.code == CapacityDiagnosticCode::OpposingProfilesDoNotFit));
        ...
        known_intent.source_scale = PhysicalScaleEvidence {
            ...
            provenance: ScaleProvenance::UserMeasured, confidence_milli: 1000,
            world_scale: WorldScaleAvailability::Available,
        };
        let known = resolve_slot_demand(&broad_region, &known_intent).unwrap();
        assert_eq!(known.required_source_footprint.unit, SourceFootprintUnit::SourcePixels);
        ...
        let density_only = resolve_slot_demand(
            &region(TemplateSlotRole::Planar, RegionOrientation::Horizontal, [8.0, 4.0]),
            &density_only_intent,
        ).unwrap();
        ...
        assert_eq!(density_only.world_dimension_source, WorldDimensionSource::DestinationTexelDensity);

        let strip_region = region(
            TemplateSlotRole::RepeatingStrip, RegionOrientation::Horizontal, [10.0, 0.5],
        );
        ...
        pixel_feature.requested_features = vec![PhysicalFeatureIntent {
            value: 4.0, scale_space: EffectScaleSpace::Pixels,
        }];
        ...
        relative_feature.requested_features = vec![PhysicalFeatureIntent {
            value: 0.1, scale_space: EffectScaleSpace::SlotMinorRelative,
        }];
        ...
        close(effect_scale_to_slot_meters(
            EffectScaleSpace::SlotMinorRelative, 0.1, &relative_feature,
        )[0], 0.05);
        ...
        let opposing_set = resolve_slot_demands(&[(&region(
            TemplateSlotRole::RepeatingStrip, RegionOrientation::Horizontal, [10.0, 0.1],
        ), opposing_intent)]).unwrap();
        ...
        close(high.minimum_survivable_feature_m, broad.minimum_survivable_feature_m);
        assert_eq!(high.allocation_rect, broad.allocation_rect);
        assert!(high.pixels_per_meter_x > broad.pixels_per_meter_x);
        close(effect_scale_to_slot_meters(EffectScaleSpace::World, 0.125, &horizontal)[0], 0.125);
        let circle_px = [0.1 * horizontal.pixels_per_meter_x, 0.1 * horizontal.pixels_per_meter_y];
        close(circle_px[0] / horizontal.pixels_per_meter_x,
