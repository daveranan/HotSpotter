...
pub enum SlotSynthesisError {
    ...
    PlaneConstruction,
    #[error(
        "Stage 14 synthesized slice center is unavailable or too small for the requested physical center"
    )]
...
pub fn synthesize_slot_material_with_guard(
    ...
    validate(&request)?;
    if cancelled() {
        return Err(SlotSynthesisError::Cancelled);
    }
    let [width, height] = request.output_dimensions;
    let pixels = u64::from(width)
        .checked_mul(u64::from(height))
        .ok_or(SlotSynthesisError::ResourceLimitExceeded)?;
    let channel_count =
        u64::try_from(request.domain.registered_channels().len()).unwrap_or(u64::MAX);
    if width > request.limits.max_dimension
        || height > request.limits.max_dimension
        || pixels > request.limits.max_pixels
        || pixels
            .checked_mul(channel_count.saturating_add(2))
            .is_none_or(|n| n > request.limits.max_operations)
    {
        return Err(SlotSynthesisError::ResourceLimitExceeded);
    }

    let mut positions = Vec::with_capacity(
        usize::try_from(pixels).map_err(|_| SlotSynthesisError::ResourceLimitExceeded)?,
    );
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            let source_valid =
                position.valid && sample_validity(request.domain, position.x, position.y);
            ...
            seam_positions.push(
                position
                    .seam_sample
                    .map_or([position.x as f32, position.y as f32], |sample| {
    ...
    let algorithm = AlgorithmProvenance {
        algorithm_id: STAGE_14_ALGORITHM_ID.into(),
        version: STAGE_14_ALGORITHM_VERSION.into(),
    };
    let settings_hash = ContentDigest::sha256(
        format!(
            "{:?}|{:?}|{:?}|{:?}|{:?}",
            request.plan.candidate.mapping_mode,
            request.plan.sampling_policy,
            request.output_dimensions,
            request.plan.slot_physical_size,
            request.plan.candidate.transform
        )
        .as_bytes(),
    );
    Ok(SynthesizedSlotMaterial {
        ...
        diagnostics: SlotSynthesisDiagnostics {
            ...
            explicit_stretch_user_override: matches!(
                request.plan.stretch_override,
                StretchOverrideProvenance::UserOverride { .. }
            ),
            messages: vec![
                if request.plan.candidate.mapping_mode == SamplingMode::ExplicitStretch
                    || matches!(
                        request.plan.slice_geometry,
                        SliceGeometry::Nine {
                    ...
                    )
                {
                    ...
                    "registered slot-local physical correspondence executed without non-uniform scaling".into()
                },
            ],
        },
        qa_views: vec![
            SlotSynthesisQaView::SamplingCoordinates,
            SlotSynthesisQaView::Correspondence,
            SlotSynthesisQaView::RegisteredMarkers,
            SlotSynthesisQaView::Validity,
            SlotSynthesisQaView::MappingMode,
        ],
...
fn validate(r: &SlotSynthesisRequest<'_>) -> Result<(), SlotSynthesisError> {
    let p = r.plan;
    if r.output_dimensions.contains(&0)
        || p.slot_physical_size
            .iter()
            .any(|v| !v.is_finite() || *v <= 0.0)
        || !p.candidate.isotropic_scale.is_finite()
        || p.candidate.isotropic_scale <= 0.0
        || !p.source_pixels_per_physical_unit.is_finite()
        || p.source_pixels_per_physical_unit <= 0.0
        || !p.sampling_policy.scale.is_finite()
        || p.sampling_policy.scale <= 0.0
        || !p.candidate.eligibility.mapping_permitted
        || !p.candidate.eligibility.transform_permitted
        || (p.candidate.mapping_mode != SamplingMode::ExplicitStretch
            && !p.candidate.eligibility.isotropic_scale)
    {
        return Err(SlotSynthesisError::InvalidPlan);
    }
    ...
    if p.candidate.domain_id != r.domain.cache_key
        || p.candidate.source_id != r.domain.prepared_source_digest
    ...
    if c.width == 0
        ...
        || c.x
            .checked_add(c.width)
            .is_none_or(|end| end > r.domain.width)
        || c.y
            .checked_add(c.height)
            .is_none_or(|end| end > r.domain.height)
    {
        return Err(SlotSynthesisError::InvalidPlan);
    }
    ...
    if p.radial_mapping.is_some_and(|radial| {
    ...
    match (p.candidate.mapping_mode, p.slice_geometry) {
        ...
        (
            ...
            && top_pixels
                ...
                .is_some_and(|sum| sum < c.height)
            && (center != SliceCenterPolicy::ExplicitStretch
                || matches!(
                    p.stretch_override,
                    StretchOverrideProvenance::UserOverride { .. }
                )) => {}
        (SamplingMode::ThreeSliceCap | SamplingMode::NineSlicePanel, _) => {
            return Err(SlotSynthesisError::InvalidPlan);
        }
    ...
    for index in &p.candidate.seam_indices {
        let Some(seam) = usize::try_from(*index)
            .ok()
            .and_then(|index| r.domain.seams.get(index))
        else {
            return Err(SlotSynthesisError::InvalidPlan);
        };
        let (expected_length, lower, upper, allowed) = match seam.axis {
            SeamAxis::X => (
                ...
                matches!(
                    p.candidate.mapping_mode,
                    SamplingMode::RepeatX | SamplingMode::PeriodicTile
                ),
            ),
            SeamAxis::Y => (
                r.domain.width,
                ...
                matches!(
                    p.candidate.mapping_mode,
                    SamplingMode::RepeatY | SamplingMode::PeriodicTile
                ),
            ),
        };
        if !allowed
            || seam.normalized_cost_milli > p.maximum_seam_cost_milli
            || seam.positions.len() != usize::try_from(expected_length).unwrap_or(usize::MAX)
            || seam
                .positions
                .iter()
                .any(|position| u32::from(*position) < lower || u32::from(*position) >= upper)
        {
            return Err(SlotSynthesisError::InvalidPlan);
        }
    }
    let transformed = p.candidate.transform.rotation != QuarterTurn::Zero
        || p.candidate.transform.mirror != MirrorTransform::None;
    if transformed
        && !p.sampling_policy.correct_tangent_normals
        && r.domain
            .registered_channels()
            .iter()
            .any(|channel| matches!(channel, PreparedExemplarChannel::Normal { .. }))
    {
        return Err(SlotSynthesisError::InvalidPlan);
    }
    validate_synthesized_center(r, c)?;
    if r.limits.tile_edge == 0 {
        return Err(SlotSynthesisError::ResourceLimitExceeded);
    }
    Ok(())
}
...
fn validate_slice_destination(p: &SamplingPlan) -> Result<(), SlotSynthesisError> {
    ...
    if fits {
        ...
        Err(SlotSynthesisError::InvalidPlan)
    }
}

fn validate_synthesized_center(
    r: &SlotSynthesisRequest<'_>,
    c: SourceCrop,
) -> Result<(), SlotSynthesisError> {
    let p = r.plan;
    ...
    let fits = match p.slice_geometry {
        SliceGeometry::Three {
            ...
            synthesized
                && requested >= 0.0
                && requested * scale
                    <= f64::from(c.width - leading_cap_pixels - trailing_cap_pixels) + 1.0e-9
        }
        SliceGeometry::Nine {
            left_pixels,
            right_pixels,
    ...
    if fits {
        ...
        Err(SlotSynthesisError::InsufficientSynthesizedCenter)
    }
}
...
fn map_position(r: &SlotSynthesisRequest<'_>, q: [f64; 2]) -> Position {
    let c = crop(r);
    let cw = f64::from(c.width);
    let ch = f64::from(c.height);
    ...
    let mut p = match mode {
        SamplingMode::UniqueContain | SamplingMode::UniqueCover => {
            let pixels_per_unit = if mode == SamplingMode::UniqueContain {
                ...
                (cw / source_size[0]).min(ch / source_size[1])
            } * r.plan.sampling_policy.scale;
            ...
            let valid = mode != SamplingMode::UniqueContain
                || (m[0] >= origin[0]
                    && m[0] < origin[0] + extent[0]
                    && m[1] >= origin[1]
                    && m[1] < origin[1] + extent[1]);
            Position::single(
                f64::from(c.x) + (m[0] - origin[0]) * pixels_per_unit,
                f64::from(c.y) + (m[1] - origin[1]) * pixels_per_unit,
                valid,
            )
        }
        SamplingMode::ExplicitStretch => Position::single(
            f64::from(c.x) + m[0] / source_size[0] * cw,
            f64::from(c.y) + m[1] / source_size[1] * ch,
            true,
        ),
        SamplingMode::PolarRadial => {
            let (
                ...
                .plan
                .radial_mapping
                .map_or((0.5, 0.5, 0.0, 0.5, 1.0, 0.0, 0.0), |radial| {
                    (
                        radial.center_x,
                        radial.center_y,
                        radial.inner_radius,
                        ...
                        radial.falloff,
                        radial.blend_width,
                        radial.seam_blend_width,
                    )
                });
            let radial_local = transform_local(
                [q[0] - center_x, q[1] - center_y],
                transform.rotation,
                transform.mirror,
            );
            let dx = radial_local[0];
            let dy = radial_local[1];
            let radius = dx.hypot(dy);
            let radial_span = (outer_radius - inner_radius).max(f64::EPSILON);
            let theta = dy.atan2(dx).rem_euclid(std::f64::consts::TAU) / std::f64::consts::TAU;
            if radius < inner_radius {
                Position::single(
                    f64::from(c.x) + (center_x + dx) * cw,
                    f64::from(c.y) + (center_y + dy) * ch,
                    true,
                )
            } else {
                ...
                let polar = [
                    (theta * cw).min(cw - f64::EPSILON),
                    (((sample_radius - inner_radius) / radial_span)
                        .clamp(0.0, 1.0)
                        .powf(falloff)
                        * ch)
                        .min(ch - f64::EPSILON),
                ...
                let t = if transition <= f64::EPSILON {
                    1.0
                } else {
                    ((radius - inner_radius) / transition).clamp(0.0, 1.0)
                };
                ...
                let (seam_sample, seam_blend) =
                    if seam_blend_width > f64::EPSILON && seam_distance < seam_blend_width {
                        ...
                        (
                            Some([
                                f64::from(c.x) + planar[0] * (1.0 - blend) + other_polar_x * blend,
                                y,
                            ]),
                            feather,
                        )
        ...
        SamplingMode::PlanarRadial => {
            let (center_x, center_y, inner_radius, outer_radius, falloff) = r
                .plan
                .radial_mapping
                .map_or((0.5, 0.5, 0.0, f64::INFINITY, 1.0), |radial| {
                    (
                        radial.center_x,
                        radial.center_y,
                        radial.inner_radius,
            ...
            let radial_scale = if radius > f64::EPSILON {
            ...
            Position::single(
                ...
                (f64::from(c.y) + center_y * ch + radial_local[1] * scale).clamp(min_y, max_y),
                true,
            )
        }
        ...
        _ => Position::single(
            f64::from(c.x) + cw * 0.5 + source_local[0] * scale,
            f64::from(c.y) + ch * 0.5 + source_local[1] * scale,
            true,
        ),
    };
    match mode {
        SamplingMode::PeriodicTile => {
            let period = r
                ...
                .candidate
                .period_pixels
                .expect("validated periodic plan");
            let phase_x = seam_phase(r, SeamAxis::X, p.y);
            let phase_y = seam_phase(r, SeamAxis::Y, p.x);
            p.x =
                f64::from(c.x) + (p.x - f64::from(c.x) + phase_x).rem_euclid(f64::from(period[0]));
            p.y =
                f64::from(c.y) + (p.y - f64::from(c.y) + phase_y).rem_euclid(f64::from(period[1]));
        }
...
fn seam_phase(r: &SlotSynthesisRequest<'_>, axis: SeamAxis, cross_coordinate: f64) -> f64 {
    ...
    r.plan
        ...
        .find_map(|index| {
            ...
            let cross_limit = match axis {
                SeamAxis::X => r.domain.height,
                SeamAxis::Y => r.domain.width,
            };
            let cross = (cross_coordinate - 0.5)
                .round()
                .clamp(0.0, f64::from(cross_limit - 1)) as usize;
...
fn transform_local(mut p: [f64; 2], rotation: QuarterTurn, mirror: MirrorTransform) -> [f64; 2] {
    match mirror {
        MirrorTransform::X => p[0] = -p[0],
        MirrorTransform::Y => p[1] = -p[1],
        MirrorTransform::None => {}
    }
    match rotation {
        QuarterTurn::Zero => p,
        QuarterTurn::Ninety => [p[1], -p[0]],
        QuarterTurn::OneEighty => [-p[0], -p[1]],
        QuarterTurn::TwoSeventy => [-p[1], p[0]],
    }
}
...
fn three_slice(
    ...
    Position::single(
        x,
        f64::from(c.y) + (m[1] - size[1] * 0.5) * scale + f64::from(c.height) * 0.5,
        valid,
    )
}
...
fn slice_axis(
    ...
    let leading_world = f64::from(leading) / scale;
    let trailing_world = f64::from(trailing) / scale;
    if value < leading_world {
        return (f64::from(origin) + value * scale, true);
    }
    if value >= destination - trailing_world {
        return (
            f64::from(origin + extent - trailing)
                + (value - (destination - trailing_world)) * scale,
            true,
        );
    }
    let offset = (value - leading_world) * scale;
    let center_pixels = extent - leading - trailing;
    match center {
        SliceCenterPolicy::Repeat => (
            f64::from(origin + leading) + offset.rem_euclid(f64::from(center_pixels)),
            true,
        ),
        ...
        SliceCenterPolicy::ExplicitStretch => {
            ...
            (
                f64::from(origin + leading)
                    + (value - leading_world) / destination_center * f64::from(center_pixels),
                true,
            )
        }
    }
...
fn sample_validity(domain: &PreparedMaterialDomain, x: f64, y: f64) -> bool {
    if x < 0.0 || y < 0.0 || x >= f64::from(domain.width) || y >= f64::from(domain.height) {
        return false;
    }
...
fn sample_channel(
    ...
    positions: &[[f32; 2]],
    seam_positions: &[[f32; 2]],
    seam_blends: &[f32],
    width: u32,
    height: u32,
    r: &SlotSynthesisRequest<'_>,
    cancelled: &dyn Fn() -> bool,
) -> Result<PreparedExemplarChannel, SlotSynthesisError> {
    let edge = r.limits.tile_edge;
    let linear = r.plan.sampling_policy.filter != SourceSamplingMode::Nearest;
    Ok(match channel {
        ...
        PreparedExemplarChannel::Scalar { role, plane: src } => PreparedExemplarChannel::Scalar {
            role: *role,
            plane: plane(
                width,
                height,
                ...
                &rasterize_blended(
                    ...
                    width,
                    cancelled,
                    |p| LinearScalar(sample_f32(src, p, linear, |v| v.0)),
                    |a, b, t| LinearScalar(a.0 * (1.0 - t) + b.0 * t),
                )?,
            )?,
        },
        PreparedExemplarChannel::Normal {
            ...
            plane: plane(
                ...
                &rasterize_blended(
                    ...
                    width,
                    cancelled,
                    |p| sample_normal(src, p, linear),
                    blend_normal,
                )?
                .into_iter()
                .map(|normal| transform_normal(normal, r))
                .collect::<Vec<_>>(),
            )?,
        ...
        PreparedExemplarChannel::MaterialId { plane: src } => PreparedExemplarChannel::MaterialId {
            plane: plane(
                ...
                &rasterize_blended(
                    ...
                    width,
                    cancelled,
                    |p| sample_nearest(src, p),
                    |a, b, t| if t < 0.5 { a } else { b },
                )?,
            )?,
        },
        PreparedExemplarChannel::Mask { role, plane: src } => PreparedExemplarChannel::Mask {
            role: *role,
            plane: plane(
                width,
                height,
                ...
                &rasterize_blended(
                    ...
                    width,
                    cancelled,
                    |p| MaskValue(sample_f32(src, p, linear, |v| v.0)),
                    |a, b, t| MaskValue(a.0 * (1.0 - t) + b.0 * t),
                )?,
            )?,
        },
    })
...
fn rasterize_blended<T>(
    positions: &[[f32; 2]],
    seam_positions: &[[f32; 2]],
    seam_blends: &[f32],
    width: u32,
    cancelled: &dyn Fn() -> bool,
    mut sample: impl FnMut([f32; 2]) -> T,
    mut blend: impl FnMut(T, T, f32) -> T,
) -> Result<Vec<T>, SlotSynthesisError> {
    let mut values = Vec::with_capacity(positions.len());
    let row_width =
        usize::try_from(width).map_err(|_| SlotSynthesisError::ResourceLimitExceeded)?;
    for row_start in (0..positions.len()).step_by(row_width) {
        ...
        for index in row_start..row_end {
            ...
            values.push(if amount > 0.0 {
                blend(primary, sample(seam_positions[index]), amount)
            } else {
                primary
            });
        }
...
fn sample_nearest<T: Copy>(p: &ImagePlane<T>, at: [f32; 2]) -> T {
    let b = bounds(p, at);
    *p.pixel(
        if b.4 < 0.5 { b.0 } else { b.2 },
        if b.5 < 0.5 { b.1 } else { b.3 },
    )
}
fn sample_f32<T: Copy>(
    ...
    if !linear {
        return f(&sample_nearest(p, at));
    }
    let (x0, y0, x1, y1, tx, ty) = bounds(p, at);
    let a = f(p.pixel(x0, y0)) * (1.0 - tx) + f(p.pixel(x1, y0)) * tx;
    let b = f(p.pixel(x0, y1)) * (1.0 - tx) + f(p.pixel(x1, y1)) * tx;
    a * (1.0 - ty) + b * ty
}
fn sample_color(p: &ImagePlane<LinearColor>, at: [f32; 2], linear: bool) -> LinearColor {
    LinearColor {
        rgb: std::array::from_fn(|i| sample_f32(p, at, linear, |v| v.rgb[i])),
        alpha: sample_f32(p, at, linear, |v| v.alpha),
    }
}
fn sample_normal(p: &ImagePlane<TangentNormal>, at: [f32; 2], linear: bool) -> TangentNormal {
    let mut n = TangentNormal {
        xyz: std::array::from_fn(|i| sample_f32(p, at, linear, |v| v.xyz[i])),
        alpha: sample_f32(p, at, linear, |v| v.alpha),
    };
    let l = (n.xyz.iter().map(|v| v * v).sum::<f32>())
        .sqrt()
        .max(f32::EPSILON);
    n.xyz = n.xyz.map(|v| v / l);
    n
}
fn transform_normal(mut n: TangentNormal, r: &SlotSynthesisRequest<'_>) -> TangentNormal {
    if !r.plan.sampling_policy.correct_tangent_normals {
        return n;
    }
    let transform = r.plan.candidate.transform;
    ...
    match transform.mirror {
        MirrorTransform::X => xy[0] = -xy[0],
        MirrorTransform::Y => xy[1] = -xy[1],
        MirrorTransform::None => {}
    }
    n.xyz[0] = xy[0] as f32;
    n.xyz[1] = xy[1] as f32;
...
fn plane<T: Clone>(
    ...
    edge: u32,
    values: &[T],
) -> Result<ImagePlane<T>, SlotSynthesisError> {
    ImagePlane::from_row_major(w, h, edge, values)
        .map_err(|_| SlotSynthesisError::PlaneConstruction)
}
...
mod tests {
    ...
    ) -> (PreparedMaterialDomain, ContentDigest, ContentDigest) {
        let width = 8;
        let height = 8;
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                colors.push(LinearColor {
                    rgb: [marker, x as f32 / 7.0, y as f32 / 7.0],
                    alpha: 1.0,
                });
                scalars.push(LinearScalar(marker));
                normals.push(TangentNormal {
                    xyz: [1.0, 0.0, 0.0],
                    alpha: 1.0,
                });
                ids.push(CategoryId(x + y * width));
                masks.push(MaskValue(marker));
            }
        }
        let channels = vec![
            PreparedExemplarChannel::BaseColor {
            ...
            PreparedExemplarChannel::Scalar {
                ...
                plane: ImagePlane::from_row_major(width, height, 4, &scalars).unwrap(),
            },
            PreparedExemplarChannel::Normal {
            ...
            PreparedExemplarChannel::MaterialId {
                plane: ImagePlane::from_row_major(width, height, 4, &ids).unwrap(),
            },
            PreparedExemplarChannel::Mask {
                role: MaterialChannelRole::Opacity,
                plane: ImagePlane::from_row_major(width, height, 4, &masks).unwrap(),
            },
        ];
        let domain_id = ContentDigest::sha256(b"stage-14-domain");
        let source_id = ContentDigest::sha256(b"stage-14-source");
        (
    ...
    fn plan(
        ...
        SamplingPlan {
            slot_id: RegionId::from_bytes([14; 16]),
            role: TemplateSlotRole::Planar,
            variation_group: "stage-14".into(),
            prepared_domain_dimensions: [8, 8],
            candidate: CropCandidate {
                candidate_id: ContentDigest::sha256(format!("{mode:?}").as_bytes()),
                source_id,
            ...
            radial_mapping: None,
            stretch_override: if mode == SamplingMode::ExplicitStretch {
                StretchOverrideProvenance::UserOverride {
            ...
            slice_geometry: match mode {
                SamplingMode::ThreeSliceCap => SliceGeometry::Three {
                    leading_cap_pixels: 2,
                    trailing_cap_pixels: 2,
                    center: SliceCenterPolicy::Repeat,
                },
                SamplingMode::NineSlicePanel => SliceGeometry::Nine {
                    left_pixels: 2,
                    right_pixels: 2,
                    top_pixels: 2,
                    bottom_pixels: 2,
    ...
    fn execute(mode: SamplingMode) -> SynthesizedSlotMaterial {
        let (domain, d, s) = fixture();
        let p = plan(mode, d, s);
    ...
    fn channels(
        output: &SynthesizedSlotMaterial,
    ) -> (
        &ImagePlane<LinearColor>,
        &ImagePlane<LinearScalar>,
        &ImagePlane<CategoryId>,
    ) {
        let PreparedExemplarChannel::BaseColor { plane: color, .. } = &output.channels[0] else {
            panic!()
        };
        let PreparedExemplarChannel::Scalar { plane: scalar, .. } = &output.channels[1] else {
            panic!()
        };
        let PreparedExemplarChannel::MaterialId { plane: id } = &output.channels[3] else {
            panic!()
        };
        (color, scalar, id)
    }
    ...
    fn numbered_grid_crop_coordinates_are_preserved_through_stage_14() {
        ...
        selected.candidate.crop = Some(SourceCrop {
            x: 2,
            y: 1,
            width: 4,
            height: 4,
        ...
        let output = synthesize_slot_material(
            SlotSynthesisRequest {
                plan: &selected,
                domain: &domain,
        ...
        let PreparedExemplarChannel::MaterialId { plane } = &output.channels[3] else {
            panic!()
        };
    ...
    fn truthful_base_color_stage_14_registered_channels() {
        ...
        for mode in modes {
            ...
            for y in 0..output.height {
                for x in 0..output.width {
                    if output.validity.pixel(x, y).0 >= 0.5 {
                        let marker = id.pixel(x, y).0 as f32 / 63.0;
                        assert!(
                            (color.pixel(x, y).rgb[0] - marker).abs() < 1.0e-6,
                            "color/ID drift in {mode:?}"
                        );
                        assert!(
                            (scalar.pixel(x, y).0 - marker).abs() < 1.0e-6,
                            "scalar/ID drift in {mode:?}"
                        );
                    }
                }
            }
        ...
        let xs = (0..8)
            .map(|x| sliced.correspondence.pixel(x, 4)[0])
            .collect::<Vec<_>>();
        assert_eq!(&xs[..2], &[0.5, 1.5]);
        assert_eq!(&xs[6..], &[6.5, 7.5]);

        // Every legal mode applies the same quarter-turn to correspondence and all channels.
        for mode in modes {
            let baseline = execute(mode);
            let (domain, d, s) = fixture();
            let mut transformed = plan(mode, d, s);
            ...
            .unwrap();
            assert_ne!(
                output.correspondence.to_row_major(),
                baseline.correspondence.to_row_major(),
            ...
            for y in 0..8 {
                for x in 0..8 {
                    if output.validity.pixel(x, y).0 >= 0.5 {
                        ...
                        assert!(
                            (color.pixel(x, y).rgb[0] - marker).abs() < 1.0e-6,
                            "transformed color drift in {mode:?}"
                        );
                        assert!(
                            (scalar.pixel(x, y).0 - marker).abs() < 1.0e-6,
                            "transformed scalar drift in {mode:?}"
                        );
                    }
                }
            }
        ...
        let expected = [
            (QuarterTurn::Zero, MirrorTransform::None, [0.5, 0.25]),
            (QuarterTurn::Ninety, MirrorTransform::None, [0.25, -0.5]),
            (QuarterTurn::OneEighty, MirrorTransform::None, [-0.5, -0.25]),
            (QuarterTurn::TwoSeventy, MirrorTransform::None, [-0.25, 0.5]),
            (QuarterTurn::Zero, MirrorTransform::X, [-0.5, 0.25]),
            (QuarterTurn::Ninety, MirrorTransform::X, [0.25, 0.5]),
            (QuarterTurn::OneEighty, MirrorTransform::X, [0.5, -0.25]),
            (QuarterTurn::TwoSeventy, MirrorTransform::X, [-0.25, -0.5]),
            (QuarterTurn::Zero, MirrorTransform::Y, [0.5, -0.25]),
            (QuarterTurn::Ninety, MirrorTransform::Y, [-0.25, -0.5]),
            (QuarterTurn::OneEighty, MirrorTransform::Y, [-0.5, 0.25]),
            (QuarterTurn::TwoSeventy, MirrorTransform::Y, [0.25, 0.5]),
        ];
        for (rotation, mirror, offset) in expected {
            let (domain, d, s) = fixture();
            let mut p = plan(SamplingMode::DirectCrop, d, s);
            p.slot_physical_size = [2.0, 1.0];
            p.source_pixels_per_physical_unit = 2.0;
            p.candidate.transform = CandidateTransform { rotation, mirror };
            ...
            assert!(
                (mapped.x - (4.0 + offset[0] * 2.0)).abs() < 1.0e-9
                    && (mapped.y - (4.0 + offset[1] * 2.0)).abs() < 1.0e-9
            );
        }
        ...
        p.slot_physical_size = [2.0, 1.0];
        p.source_pixels_per_physical_unit = 2.0;
        let direct = synthesize_slot_material(
            SlotSynthesisRequest {
                plan: &p,
                domain: &domain,
                output_dimensions: [12, 6],
                limits: SlotSynthesisLimits::default(),
            },
            &RenderCancellationToken::new(),
        )
        .unwrap();
        let a = direct.correspondence.pixel(5, 2);
        let dx = direct.correspondence.pixel(6, 2);
        let dy = direct.correspondence.pixel(5, 3);
        ...
        let (domain, d, s) = fixture();
        let mut rotated = plan(SamplingMode::DirectCrop, d, s);
        ...
        .unwrap();
        let PreparedExemplarChannel::Normal { plane, .. } = &output.channels[2] else {
            panic!()
        };
        ...
        assert_ne!(
            execute(SamplingMode::PlanarRadial)
                .correspondence
                .to_row_major(),
            execute(SamplingMode::PolarRadial)
                .correspondence
                .to_row_major()
        );
        let (domain, d, s) = fixture();
        let planar = plan(SamplingMode::PlanarRadial, d, s);
        let planar = synthesize_slot_material(
            SlotSynthesisRequest {
                plan: &planar,
                domain: &domain,
                output_dimensions: [16, 8],
                limits: SlotSynthesisLimits::default(),
            },
            &RenderCancellationToken::new(),
        )
        .unwrap();
        let center = planar.correspondence.pixel(8, 4);
        let horizontal = planar.correspondence.pixel(12, 4);
        let vertical = planar.correspondence.pixel(8, 6);
        assert!(
            ((horizontal[0] - center[0]).abs() - (vertical[1] - center[1]).abs()).abs() < 1.0e-6,
        ...
        let (domain, d, s) = fixture();
        let mut stretch = plan(SamplingMode::ExplicitStretch, d, s);
        stretch.stretch_override = StretchOverrideProvenance::NotAuthorized;
        let error = synthesize_slot_material(
            SlotSynthesisRequest {
                plan: &stretch,
                domain: &domain,
                output_dimensions: [8, 8],
        ...
        let StageResult::FailedWithRecovery {
            recovery_choices, ..
        } = error.stage_result()
        else {
            panic!()
        ...
        let (domain, d, s) = fixture();
        let texture = plan(SamplingMode::TextureSynthesis, d, s);
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &texture,
            ...
            .unwrap_err(),
            SlotSynthesisError::InvalidPlan
        );

        // SamplingPolicy changes the raster correspondence and filter, proving it is authoritative.
        let (domain, d, s) = fixture();
        let mut scaled = plan(SamplingMode::DirectCrop, d, s);
        scaled.sampling_policy.scale = 0.5;
        let scaled = synthesize_slot_material(
            SlotSynthesisRequest {
                plan: &scaled,
                domain: &domain,
                output_dimensions: [4, 4],
        ...
        .unwrap();
        assert_ne!(
            scaled.correspondence.pixel(0, 0),
            execute(SamplingMode::DirectCrop).correspondence.pixel(0, 0)
        );

        // Cancellation is observed between channel rows, not only during correspondence construction.
        let cancel = RenderCancellationToken::new();
        let mut samples = 0_usize;
        let raster = rasterize(&vec![[0.0, 0.0]; 16], 4, &|| cancel.is_cancelled(), |p| {
            samples += 1;
            if samples == 4 {
                cancel.cancel();
            }
        ...
        let (domain, d, s) = fixture();
        let mut malformed = plan(SamplingMode::DirectCrop, d, s);
        malformed.candidate.crop = Some(SourceCrop {
            x: 7,
            y: 0,
            width: 2,
            height: 8,
        ...
        let (domain, d, s) = fixture();
        let mut malformed = plan(SamplingMode::PeriodicTile, d, s);
        malformed.candidate.period_pixels = Some([0, 2]);
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &malformed,
        ...
        let (domain, d, s) = fixture();
        let mut malformed = plan(SamplingMode::ThreeSliceCap, d, s);
        malformed.slice_geometry = SliceGeometry::Three {
        ...
        let (domain, d, s) = fixture();
        let mut malformed = plan(SamplingMode::DirectCrop, d, s);
        malformed.candidate.seam_indices.push(99);
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &malformed,
        ...
        let seam = SelectedSeam {
            axis: SeamAxis::X,
            positions: vec![0, 1, 2, 3, 0, 1, 2, 3],
            normalized_cost_milli: 100,
        };
        let (domain, d, s) = fixture_with(DomainRoute::GraphCutPeriodicClosure, vec![seam]);
        let mut seamed = plan(SamplingMode::RepeatX, d, s);
        seamed.candidate.period_pixels = Some([4, 2]);
        seamed.candidate.seam_indices = vec![0];
        seamed.maximum_seam_cost_milli = 150;
        let output = synthesize_slot_material(
            SlotSynthesisRequest {
                plan: &seamed,
                domain: &domain,
        ...
        for row in 0..8 {
            assert_eq!(
                output.correspondence.pixel(0, row)[0],
                0.5 + f32::from((row % 4) as u16),
        ...
        let wrong_axis = SelectedSeam {
            axis: SeamAxis::Y,
            positions: vec![1; 8],
            normalized_cost_milli: 10,
        };
        let (wrong_domain, d, s) =
            fixture_with(DomainRoute::GraphCutPeriodicClosure, vec![wrong_axis]);
        let mut wrong = plan(SamplingMode::RepeatX, d, s);
        wrong.candidate.seam_indices = vec![0];
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &wrong,
        ...
        let short_path = SelectedSeam {
            axis: SeamAxis::X,
            positions: vec![1; 7],
            normalized_cost_milli: 10,
        };
        let (short_domain, d, s) =
            fixture_with(DomainRoute::GraphCutPeriodicClosure, vec![short_path]);
        let mut short = plan(SamplingMode::RepeatX, d, s);
        short.candidate.seam_indices = vec![0];
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &short,
        ...
        let (domain, d, s) = fixture();
        let mut narrow = plan(SamplingMode::ThreeSliceCap, d, s);
        narrow.slot_physical_size = [0.4, 1.0];
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &narrow,
        ...
        let (domain, d, s) = fixture();
        let mut shallow = plan(SamplingMode::NineSlicePanel, d, s);
        shallow.slot_physical_size = [1.0, 0.4];
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &shallow,
        ...
        let (domain, d, s) = fixture();
        let mut reduced = plan(SamplingMode::NineSlicePanel, d, s);
        reduced.sampling_policy.scale = 0.5;
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
                    plan: &reduced,
        ...
        synthesized.slice_geometry = SliceGeometry::Three {
            leading_cap_pixels: 2,
            trailing_cap_pixels: 2,
            center: SliceCenterPolicy::Synthesize,
        };
        ...
        assert!(
            output
                ...
                .to_row_major()
                .iter()
                .all(|value| value.0 == 1.0)
        );
        let (direct, d, s) = fixture();
        let mut unavailable = plan(SamplingMode::ThreeSliceCap, d, s);
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            SlotSynthesisError::InsufficientSynthesizedCenter
        );

        // Nine-slice center stretch is explicit, localized, and carries visible user authorization.
        let (domain, d, s) = fixture();
        let mut panel = plan(SamplingMode::NineSlicePanel, d, s);
        panel.slot_physical_size = [2.0, 1.0];
        panel.slice_geometry = SliceGeometry::Nine {
            left_pixels: 2,
            right_pixels: 2,
            top_pixels: 2,
            bottom_pixels: 2,
        ...
        panel.stretch_override = StretchOverrideProvenance::UserOverride {
        ...
        let (domain, d, s) = fixture();
        let p = plan(SamplingMode::DirectCrop, d, s);
        let limited = SlotSynthesisLimits {
            max_pixels: 4,
            ..SlotSynthesisLimits::default()
        };
        assert_eq!(
            synthesize_slot_material(
                SlotSynthesisRequest {
            ...
            .unwrap_err(),
            SlotSynthesisError::ResourceLimitExceeded
        );
    }
}
