#![doc = "Deterministic compilation of a source texture into a pinned template sheet."]

use hot_trimmer_domain::{
    PixelBounds, PixelSize, SourceFraming, SourceFramingMode,
    StructuralProfile as TemplateProfile, TemplateDefinition, TemplateSlot,
};
use hot_trimmer_render_core::{
    NormalConvention, ProfileKind, StructuralProfile, StructuralProfileRequest,
...
pub fn compile_template_sheet(
    ...
    render_continuous_source(&mut rgba8, request);
...
fn render_continuous_source(output: &mut [u8], request: SheetCompileRequest<'_>) {
...
fn framed_coordinates(u: f64, v: f64, request: SheetCompileRequest<'_>) -> (f64, f64) {
    match request.source_framing.mode {
        SourceFramingMode::Stretch => (u, v),
        SourceFramingMode::Repeat => (u * 2.0, v * 2.0),
        SourceFramingMode::Cover => {
            let source_aspect = f64::from(request.source_width) / f64::from(request.source_height);
            let sheet_aspect = f64::from(request.sheet_size.width) / f64::from(request.sheet_size.height);
...
fn composite_slot(
    ...
    for local_y in 0..bounds.height {
        for local_x in 0..bounds.width {
            let local = usize::try_from(u64::from(local_y) * u64::from(bounds.width) + u64::from(local_x))
                .map_err(|_| SheetCompileError::OutputTooLarge)?;
            let offset = pixel_offset(request.sheet_size.width, bounds.x + local_x, bounds.y + local_y);
            let height_value = (0.5 + f64::from(structural.height_f32[local])).clamp(0.0, 1.0) as f32;
            ...
            for channel in 0..3 {
                base_color[offset + channel] =
                    (f64::from(base_color[offset + channel]) * shade).round().clamp(0.0, 255.0) as u8;
            }
            let cavity = (0.5 - f64::from(height_value)).max(0.0);
            let highlight = (f64::from(height_value) - 0.5).max(0.0);
            let rough = (145.0 + cavity * 90.0 - highlight * 24.0).round().clamp(0.0, 255.0) as u8;
            ...
            region_id[offset..offset + 4]
                .copy_from_slice(&[slot.id_color.0[0], slot.id_color.0[1], slot.id_color.0[2], 255]);
    ...
    for local_y in 0..bounds.height {
        for local_x in 0..bounds.width {
            let (u, v, opacity) = if let Some(radial) = slot.radial_parameters {
                radial_coordinates(local_x, local_y, bounds, radial.center_x, radial.center_y,
                    radial.inner_radius, radial.outer_radius)
            } else {
            ...
            let source_offset = pixel_offset(request.source_width, source_x, source_y);
            let output_offset = pixel_offset(request.sheet_size.width, bounds.x + local_x, bounds.y + local_y);
            let shade = 0.84 + f64::from(structural.height_f32[(local_y * bounds.width + local_x) as usize]) * 0.16;
            output[output_offset] = (f64::from(request.source_rgba8[source_offset]) * shade).round() as u8;
            output[output_offset + 1] = (f64::from(request.source_rgba8[source_offset + 1]) * shade).round() as u8;
            output[output_offset + 2] = (f64::from(request.source_rgba8[source_offset + 2]) * shade).round() as u8;
...
fn scaled_bounds(slot: &TemplateSlot, template: &TemplateDefinition, sheet: PixelSize) -> PixelBounds {
    ...
    let right = scale(slot.allocation.x + slot.allocation.width, template.canonical_width, sheet.width);
    let bottom = scale(slot.allocation.y + slot.allocation.height, template.canonical_height, sheet.height);
    PixelBounds { x, y, width: right - x, height: bottom - y }
}

fn radial_coordinates(
    x: u32, y: u32, bounds: PixelBounds, center_x: f64, center_y: f64, inner: f64, outer: f64,
) -> (f64, f64, u8) {
    ...
    if radius < inner || radius > outer { return (0.0, 0.0, 0); }
...
fn pixel_offset(width: u32, x: u32, y: u32) -> usize { ((y * width + x) * 4) as usize }

fn pixel_bytes(width: u32, height: u32) -> Result<usize, SheetCompileError> {
    usize::try_from(u64::from(width) * u64::from(height) * 4).map_err(|_| SheetCompileError::OutputTooLarge)
}
...
mod tests {
    ...
    fn request(framing: SourceFraming) -> SheetCompileRequest<'static> {
        ...
        let source = Box::leak(Box::new(vec![
            0, 10, 20, 255, 30, 40, 50, 255, 60, 70, 80, 255, 90, 100, 110, 255,
            120, 130, 140, 255, 150, 160, 170, 255, 180, 190, 200, 255, 210, 220, 230, 255,
        ]));
        SheetCompileRequest {
            source_rgba8: source,
            source_width: 4,
            source_height: 2,
            template,
            sheet_size: PixelSize { width: 512, height: 512 },
    ...
    fn compiles_pbr_and_exact_manifest_id_maps() {
        ...
        let expected_bytes = usize::try_from(u64::from(output.width) * u64::from(output.height) * 4)
        ...
        let offset = pixel_offset(output.width, bounds.x + bounds.width / 2, bounds.y + bounds.height / 2);
