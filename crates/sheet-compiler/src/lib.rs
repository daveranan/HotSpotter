...
use hot_trimmer_render_core::{
    NormalConvention, ProfileKind, RegionLayerRenderRequest, RenderCancellationToken, RenderError,
    SampleSpace, StructuralProfile, StructuralProfileRequest, compile_structural_profile,
...
pub fn compile_template_sheet_with_source_layers(
    ...
    if request.source_width == 0 || request.source_height == 0 || !request.sheet_size.is_nonzero() {
...
fn composite_source_layer(
    ...
    for local_y in 0..bounds.height {
        for local_x in 0..bounds.width {
            ...
            let alpha = (f64::from(source[source_offset + 3]) / 255.0 * opacity).clamp(0.0, 1.0);
            for channel in 0..3 {
...
fn composite_slot(
    ...
    for local_y in 0..bounds.height {
        for local_x in 0..bounds.width {
            let local =
                usize::try_from(u64::from(local_y) * u64::from(bounds.width) + u64::from(local_x))
...
mod tests {
    ...
    fn selected_region_source_layer_changes_compiled_pixels_deterministically() {
        ...
        let repeated = compile_template_sheet_with_source_layers(request, &bindings, &cancellation)
