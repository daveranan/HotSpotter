...
fn default_minimum_aspect_milli() -> u16 {
    125
}
fn default_maximum_aspect_milli() -> u16 {
    8_000
}
...
impl HierarchicalLayoutRecipe {
    pub fn mixed_hierarchy_default() -> Self {
        Self {
            schema_version: 1,
            macro_style: MacroStyle::MixedHierarchy,
            recursive_policy: RecursivePolicy::Cascade,
            target_region_min: 29,
            target_region_max: 36,
            large_share_milli: 580,
            medium_share_milli: 200,
            small_share_milli: 80,
            strip_share_milli: 110,
            radial_share_milli: 30,
            macro_parent_count: 4,
            protected_parent_count: 2,
            subdividable_parent_count: 2,
            hierarchy_depth: 3,
            scale_falloff_milli: 500,
            allowed_split_ratios: vec![
                SplitRatio::Half,
                SplitRatio::OneThird,
                SplitRatio::TwoThird,
            ],
            alignment_strength_milli: 900,
            variation_milli: 80,
            horizontal_strip_weight_milli: 550,
            vertical_strip_weight_milli: 450,
            strip_thickness_ladder: vec![1, 1, 2, 2, 3, 4],
            radial_count: 2,
            radial_min_diameter: 6,
            radial_max_diameter: 10,
    ...
    fn validate(&self) -> Result<(), PartitionError> {
        let shares = u32::from(self.large_share_milli)
            + u32::from(self.medium_share_milli)
            + u32::from(self.small_share_milli)
            + u32::from(self.strip_share_milli)
            + u32::from(self.radial_share_milli);
        if self.schema_version == 0 || shares != 1_000 {
            return Err(PartitionError::InvalidHierarchicalShares {
                total_milli: shares,
            });
        }
        if self.target_region_min == 0
            || self.target_region_min > self.target_region_max
            || self.target_region_max < 24
            || self.target_region_max > MAX_PARTITION_REGIONS
            || self.hierarchy_depth == 0
            || self.scale_falloff_milli == 0
            || self.scale_falloff_milli >= 1_000
            || self
                .protected_parent_count
                .saturating_add(self.subdividable_parent_count)
                > self.macro_parent_count
            || self.allowed_split_ratios.is_empty()
            || self.alignment_strength_milli > 1_000
            || self.variation_milli > 1_000
            || u32::from(self.horizontal_strip_weight_milli)
                + u32::from(self.vertical_strip_weight_milli)
                != 1_000
            || (self.strip_share_milli > 0
                && self.strip_thickness_ladder.iter().any(|value| *value == 0))
            || self.radial_count > 4
            || (self.radial_count > 0
                && (self.radial_min_diameter == 0
                    || self.radial_min_diameter > self.radial_max_diameter))
            || self.major_aspects.is_empty()
            || self.medium_aspects.is_empty()
            || self.detail_aspects.is_empty()
        {
...
impl Default for CompositionProfile {
    fn default() -> Self {
        Self {
            ...
            broad_panels: FamilyQuota {
                count: 0,
                area_share_milli: 0,
                minimum_width: 8,
                minimum_height: 8,
                maximum_width: MAX_LOGICAL_GRID_EDGE,
                maximum_height: MAX_LOGICAL_GRID_EDGE,
                minimum_aspect_milli: 250,
                maximum_aspect_milli: 4_000,
                subdivision_budget: 0,
            },
            medium_blocks: FamilyQuota {
                count: 0,
                area_share_milli: 0,
                minimum_width: 4,
                minimum_height: 4,
                maximum_width: MAX_LOGICAL_GRID_EDGE,
                maximum_height: MAX_LOGICAL_GRID_EDGE,
                minimum_aspect_milli: 200,
                maximum_aspect_milli: 5_000,
                subdivision_budget: 0,
            },
            horizontal_strips: StripQuota {
                count: 0,
                minimum_thickness: 1,
                maximum_thickness: MAX_LOGICAL_GRID_EDGE,
            },
            vertical_strips: StripQuota {
                count: 0,
                minimum_thickness: 1,
                maximum_thickness: MAX_LOGICAL_GRID_EDGE,
            },
            small_details: FamilyQuota {
                count: 0,
                area_share_milli: 0,
                minimum_width: 1,
                minimum_height: 1,
                maximum_width: MAX_LOGICAL_GRID_EDGE,
                maximum_height: MAX_LOGICAL_GRID_EDGE,
                minimum_aspect_milli: 125,
                maximum_aspect_milli: 8_000,
                subdivision_budget: 0,
            },
            micro_strips: StripQuota {
                count: 0,
                minimum_thickness: 1,
                maximum_thickness: 2,
            },
            radial_reservations: RadialQuota {
                count: 0,
                allocation_min_diameter: 1,
                allocation_max_diameter: 64,
            },
        }
    }
}
...
impl Default for FamilyQuota {
    fn default() -> Self {
        Self {
            ...
            maximum_width: MAX_LOGICAL_GRID_EDGE,
            maximum_height: MAX_LOGICAL_GRID_EDGE,
            minimum_aspect_milli: 1,
            maximum_aspect_milli: 65_535,
            subdivision_budget: 0,
        }
    }
}

fn default_maximum_family_extent() -> u32 {
    MAX_LOGICAL_GRID_EDGE
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct StripQuota {
    pub count: u32,
    pub minimum_thickness: u32,
    pub maximum_thickness: u32,
}

impl Default for StripQuota {
    fn default() -> Self {
        Self {
            count: 0,
            minimum_thickness: 1,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RadialQuota {
    pub count: u32,
    pub allocation_min_diameter: u32,
    pub allocation_max_diameter: u32,
}

impl Default for RadialQuota {
    fn default() -> Self {
        Self {
            count: 0,
            allocation_min_diameter: 1,
            allocation_max_diameter: 1,
        }
    }
}
...
pub enum PartitionError {
    ...
    ImpossibleTarget,
    #[error(
        "composition quota for {family:?} cannot be satisfied: {reason}. Suggested correction: {suggestion}"
    )]
    ImpossibleFamilyQuota {
        family: PartitionFamily,
        reason: String,
        suggestion: String,
    },
    #[error(
        "composition quotas request {requested} regions but the target is {target}. Suggested correction: increase target count or reduce family counts"
    )]
    QuotaExceedsTarget { requested: u32, target: u32 },
    #[error(
        "partition work/depth limit prevents this target. Suggested correction: raise the limit or reduce target count"
    )]
    WorkLimit,
...
impl LogicalGridSpec {
    pub const DEFAULT: Self = Self {
        schema_version: LOGICAL_GRID_SCHEMA_VERSION,
        width: 64,
        height: 64,
    };
    ...
    pub fn validate(self) -> Result<(), PartitionError> {
        if self.schema_version == 0
            || self.width == 0
            || self.height == 0
            || self.width > MAX_LOGICAL_GRID_EDGE
            || self.height > MAX_LOGICAL_GRID_EDGE
        {
...
impl SourceFrame {
    ...
    pub fn compute_identity(&self) -> DocumentHash {
        ...
        DocumentHash(
            Sha256::digest(serde_json::to_vec(&copy).expect("source frame serializable")).into(),
        )
    }
    ...
    pub fn region_bounds(&self, grid: LogicalGridSpec, rect: GridRect) -> crate::NormalizedBounds {
        ...
        let right = f64::from(source_x[(rect.x + rect.width) as usize])
            / f64::from(self.oriented_dimensions.width);
        let bottom = f64::from(source_y[(rect.y + rect.height) as usize])
            / f64::from(self.oriented_dimensions.height);
...
impl RegionSourceOverride {
    ...
    pub fn compute_identity(&self) -> DocumentHash {
        ...
        copy.identity = DocumentHash([0; 32]);
        DocumentHash(
            Sha256::digest(serde_json::to_vec(&copy).expect("source override serializable")).into(),
        )
    }
}
...
impl PartitionRecipe {
    ...
    pub fn validate(&self) -> Result<(), PartitionError> {
        ...
        if self.schema_version == 0
            ...
            || self.target_region_count == 0
            || self.target_region_count > MAX_PARTITION_REGIONS
        {
            return Err(PartitionError::InvalidTarget);
        }
        if self.minimum_logical_width == 0
            || self.minimum_logical_height == 0
            || self.minimum_aspect_milli == 0
            || self.minimum_aspect_milli > self.maximum_aspect_milli
            || self.work_limit < self.target_region_count
            || self.depth_limit == 0
        {
        ...
        if capacity < self.target_region_count {
            return Err(PartitionError::ImpossibleTarget);
        }
        ...
        let subdivision_floor = u32::from(profile.broad_panels.subdivision_budget)
            .saturating_mul(profile.broad_panels.count)
            .saturating_add(
                u32::from(profile.medium_blocks.subdivision_budget)
                    .saturating_mul(profile.medium_blocks.count),
            )
            .saturating_add(
                u32::from(profile.small_details.subdivision_budget)
                    .saturating_mul(profile.small_details.count),
            );
        let requested_floor = quotas
            .saturating_add(subdivision_floor)
            .saturating_add(u32::from(quotas > 0));
        if requested_floor > self.target_region_count {
            return Err(PartitionError::QuotaExceedsTarget {
                requested: requested_floor,
                target: self.target_region_count,
            });
        }
        ...
        if requested_area > 1_000 {
            return Err(PartitionError::ImpossibleFamilyQuota {
                family: PartitionFamily::BroadPanel,
                reason: "panel area shares exceed the complete frame".into(),
                suggestion: "reduce combined panel, block, and detail area share to 100% or less"
                    .into(),
            });
        }
        for (family, quota) in [
            (PartitionFamily::BroadPanel, profile.broad_panels),
            (PartitionFamily::MediumBlock, profile.medium_blocks),
            (PartitionFamily::SmallDetail, profile.small_details),
        ] {
            if quota.count > 0
                && (quota.minimum_width == 0
                    || quota.minimum_height == 0
                    || quota.maximum_width < quota.minimum_width
                    || quota.maximum_height < quota.minimum_height
                    || quota.minimum_aspect_milli == 0
                    || quota.minimum_aspect_milli > quota.maximum_aspect_milli)
            {
                return Err(PartitionError::ImpossibleFamilyQuota {
                    family,
                    reason: "dimensions or aspect bounds are invalid".into(),
                    suggestion: "use positive dimensions and an ordered aspect range".into(),
                });
            }
        }
        for (family, quota) in [
            (PartitionFamily::HorizontalStrip, profile.horizontal_strips),
            (PartitionFamily::VerticalStrip, profile.vertical_strips),
            (PartitionFamily::MicroStrip, profile.micro_strips),
        ] {
            if quota.count > 0
                && (quota.minimum_thickness == 0
                    || quota.minimum_thickness > quota.maximum_thickness)
            {
                return Err(PartitionError::ImpossibleFamilyQuota {
                    family,
                    reason: "strip thickness bounds are invalid".into(),
                    suggestion: "use a positive minimum no greater than maximum".into(),
                });
            }
        }
        let radial = profile.radial_reservations;
        if radial.count > 0
            && (radial.allocation_min_diameter == 0
                || radial.allocation_min_diameter > radial.allocation_max_diameter)
        {
            return Err(PartitionError::ImpossibleFamilyQuota {
                family: PartitionFamily::RadialReservation,
                reason: "allocation diameter bounds are invalid".into(),
                suggestion: "use a positive minimum no greater than maximum".into(),
            });
        }
        Ok(())
...
) -> Result<Vec<PartitionRegion>, PartitionError> {
    if recipe.hierarchical.is_some() {
        return generate_hierarchical_partition(recipe);
    }
    generate_legacy_partition(recipe)
}

/// Version-2 compatibility generator retained for saved projects and exact-count fixtures.
fn generate_legacy_partition(
    recipe: &PartitionRecipe,
) -> Result<Vec<PartitionRegion>, PartitionError> {
    recipe.validate()?;
    let mut leaves = vec![PartitionLeaf::remainder(
        GridRect {
            x: 0,
            y: 0,
            width: recipe.grid.width,
            height: recipe.grid.height,
        },
        0,
        true,
    )];
    ...
    while leaves.len() < recipe.target_region_count as usize {
        if leaves.len() as u32 >= recipe.work_limit {
            return Err(PartitionError::WorkLimit);
        }
        let index = leaves
            .iter()
            .enumerate()
            .filter(|(_, leaf)| leaf.depth < recipe.depth_limit && can_split_leaf(**leaf, recipe))
            .max_by_key(|(index, leaf)| {
                (
                    leaf.subdivision_budget > 0,
                    u64::from(leaf.rect.width) * u64::from(leaf.rect.height),
                    u64::from(leaf.rect.width.max(leaf.rect.height)),
                    u64::MAX - *index as u64,
                )
            })
            .map(|(index, _)| index)
            .ok_or(PartitionError::WorkLimit)?;
        ...
        let (family_minimum_width, family_minimum_height) = match family {
            PartitionFamily::BroadPanel => (
                recipe.composition.broad_panels.minimum_width,
                recipe.composition.broad_panels.minimum_height,
            ),
            PartitionFamily::MediumBlock => (
                recipe.composition.medium_blocks.minimum_width,
                recipe.composition.medium_blocks.minimum_height,
            ),
            PartitionFamily::SmallDetail => (
                recipe.composition.small_details.minimum_width,
                recipe.composition.small_details.minimum_height,
            ),
            _ => (1, 1),
        };
        let can_vertical = rect.width
            >= recipe
                .minimum_logical_width
                .max(family_minimum_width)
                .saturating_mul(2);
        let can_horizontal = rect.height
            >= recipe
                .minimum_logical_height
                .max(family_minimum_height)
                .saturating_mul(2);
        if !can_vertical && !can_horizontal {
            return Err(PartitionError::ImpossibleTarget);
        }
        let prefer_vertical =
            if recipe.horizontal_split_bias_milli == recipe.vertical_split_bias_milli {
                (recipe.seed.wrapping_add(leaves.len() as u64) & 1) == 0
            } else {
                recipe.vertical_split_bias_milli > recipe.horizontal_split_bias_milli
            };
        ...
        if recipe.variance_milli > 0 {
            let span = ((extent / 2).saturating_mul(u32::from(recipe.variance_milli).min(1000))
                / 1000)
                .max(1);
            let delta =
                (recipe.seed.wrapping_add(leaves.len() as u64) % u64::from(span * 2 + 1)) as u32;
            split = split.saturating_sub(span).saturating_add(delta);
        }
        let family_minimum = if vertical {
            family_minimum_width
        } else {
            family_minimum_height
        };
        let minimum = (if vertical {
            recipe.minimum_logical_width
        } else {
            recipe.minimum_logical_height
        })
        .max(family_minimum);
        ...
        if family == PartitionFamily::Remainder {
            leaves.extend([
                PartitionLeaf::remainder(first, depth + 1, leaf.fill_remainder),
                PartitionLeaf::remainder(second, depth + 1, leaf.fill_remainder),
            ]);
        } else {
            leaves.extend([
                PartitionLeaf {
                    ...
                    family,
                    depth: depth + 1,
                    subdivision_budget: leaf.subdivision_budget.saturating_sub(1),
    ...
    Ok(leaves
        .into_iter()
        .enumerate()
        .map(|(index, leaf)| PartitionRegion {
            id: region_id(recipe, leaf.rect, index as u32),
            grid_rect: leaf.rect,
            family: leaf.family,
            lineage: PartitionLineage::default(),
        })
        .collect())
}
...
#[allow(dead_code)]
struct HierarchicalZoneRect {
    rect: GridRect,
    kind: HierarchicalZoneKind,
}
...
impl BasisGrid {
    fn new(grid: LogicalGridSpec) -> Result<Self, PartitionError> {
        if grid.width < 16 || grid.height < 16 || grid.width != grid.height {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: "curated hotspot basis requires a square grid of at least 16 cells".into(),
            });
        }
        ...
        if cell_w == 0 || cell_h == 0 {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: "grid is too small for the canonical hotspot basis".into(),
            });
        }
        Ok(Self {
            width: grid.width,
...
fn transform_basis_rect(
    ...
    match transform {
        ...
        SymmetryTransform::MirrorX => GridRect {
            x: grid.width - rect.x - rect.width,
            ..rect
        },
        SymmetryTransform::MirrorY => GridRect {
            y: grid.height - rect.y - rect.height,
            ..rect
        },
        SymmetryTransform::Rotate180 => GridRect {
            x: grid.width - rect.x - rect.width,
            y: grid.height - rect.y - rect.height,
            ..rect
        },
        SymmetryTransform::Rotate90 => GridRect {
            x: grid.height - rect.y - rect.height,
            y: rect.x,
            width: rect.height,
            height: rect.width,
        },
        SymmetryTransform::Rotate270 => GridRect {
            x: rect.y,
            y: grid.width - rect.x - rect.width,
            width: rect.height,
            height: rect.width,
        },
        SymmetryTransform::MirrorDiagonal => GridRect {
            x: rect.y,
            y: rect.x,
            width: rect.height,
            height: rect.width,
        },
        SymmetryTransform::MirrorAntiDiagonal => GridRect {
            x: grid.height - rect.y - rect.height,
            y: grid.width - rect.x - rect.width,
            width: rect.height,
            height: rect.width,
...
fn strip_ladder_for(extent: u32) -> Vec<u32> {
    ...
    if extent >= 4 {
        return vec![extent / 2, 1, extent - extent / 2 - 1];
    }
    vec![1; extent as usize]
}

fn build_hotspot_basis_inventory(
    grid: LogicalGridSpec,
    hierarchy: &HierarchicalLayoutRecipe,
) -> Result<Vec<RegionDemand>, PartitionError> {
    let basis = BasisGrid::new(grid)?;
    let mut demands = Vec::new();
    let mut next_id = 0_u32;
    let mut push = |width_cells,
                    height_cells,
                    tier,
                    role,
                    pair_group,
    ...
    push(
        ...
        true,
        1,
        ZonePreference::HorizontalFamily,
    );
    push(
        basis.cell_w * 2,
        basis.cell_h * 4,
    ...
    for thickness in strip_ladder_for(basis.cell_h) {
        push(
            ...
            thickness,
            SizeTier::Strip,
            RegionRole::HorizontalStrip,
            None,
            Some(7),
            true,
            1,
            ZonePreference::HorizontalLadder,
        );
    }
    for thickness in strip_ladder_for(basis.cell_w) {
        push(
            ...
            basis.cell_h * 3,
            SizeTier::Strip,
            RegionRole::VerticalStrip,
            None,
            Some(8),
            true,
            1,
            ZonePreference::VerticalLadder,
        );
    }
    if hierarchy.radial_count > 0 {
        push(
            ...
            Some(9),
            true,
            hierarchy.radial_count,
            ZonePreference::RadialCluster,
        );
    }
...
fn validate_region_demands(demands: &[RegionDemand]) -> Result<(), PartitionError> {
    ...
    for demand in demands.iter().filter(|demand| demand.required) {
        if let Some(pair_group) = demand.pair_group {
            ...
            if demand.width_cells >= demand.height_cells {
                entry.0 = entry.0.saturating_add(demand.multiplicity);
            } else {
                entry.1 = entry.1.saturating_add(demand.multiplicity);
            }
        }
        if demand.role == RegionRole::Radial && demand.width_cells != demand.height_cells {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: "radial demand is not square".into(),
            });
        }
    }
    for (pair_group, (wide, tall)) in pair_counts {
        if wide == 0 || tall == 0 || wide != tall {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: format!(
                    "orientation pair group {pair_group} is incomplete ({wide} wide / {tall} tall)"
                ),
            });
        }
    }
...
fn generate_hierarchical_partition(
    recipe: &PartitionRecipe,
) -> Result<Vec<PartitionRegion>, PartitionError> {
    recipe.grid.validate()?;
    let hierarchy = recipe
        .hierarchical
        .as_ref()
        .ok_or(PartitionError::InvalidHierarchicalRecipe)?;
    hierarchy.validate()?;
    ...
    if leaves.len() < hierarchy.target_region_min as usize
        || leaves.len() > hierarchy.target_region_max as usize
    {
        return Err(PartitionError::InvalidHotspotBasis {
            reason: format!(
                "basis produced {} regions outside requested {}..={}",
                leaves.len(),
                hierarchy.target_region_min,
                hierarchy.target_region_max
            ),
        });
    }
...
fn place_hotspot_basis_inventory(
    recipe: &PartitionRecipe,
    hierarchy: &HierarchicalLayoutRecipe,
    demands: &[RegionDemand],
) -> Result<Vec<HierarchicalLeaf>, PartitionError> {
    ...
    for (ordinal, (rect, role)) in quadrants.into_iter().zip(roles).enumerate() {
        place_quadrant_role(
            ...
            ordinal as u32,
            hierarchy,
            &mut radial_remaining,
            &mut leaves,
        );
    }
    while radial_remaining > 0 {
        let added = replace_first_matching(
            &mut leaves,
            |leaf| {
                leaf.family == PartitionFamily::MediumBlock
                    && leaf.rect.width == leaf.rect.height
                    && leaf.rect.width >= basis.cell_w * 2
            },
...
fn quadrant_roles_for(hierarchy: &HierarchicalLayoutRecipe, seed: u64) -> [QuadrantRole; 4] {
    let mut roles = match hierarchy.macro_style {
        MacroStyle::MixedHierarchy => [
            QuadrantRole::MajorSquare,
            QuadrantRole::HorizontalFamily,
            QuadrantRole::VerticalFamily,
            QuadrantRole::DetailBasis,
        ],
        MacroStyle::PanelCascade => [
            QuadrantRole::MajorSquare,
            QuadrantRole::DetailBasis,
            QuadrantRole::VerticalFamily,
            QuadrantRole::HorizontalFamily,
        ],
        MacroStyle::HorizontalTrims => [
            QuadrantRole::HorizontalFamily,
            QuadrantRole::MajorSquare,
            QuadrantRole::DetailBasis,
            QuadrantRole::VerticalFamily,
        ],
        MacroStyle::VerticalTrims => [
            QuadrantRole::VerticalFamily,
            QuadrantRole::DetailBasis,
            QuadrantRole::MajorSquare,
            QuadrantRole::HorizontalFamily,
        ],
        MacroStyle::FacadeHalving => [
            QuadrantRole::MajorSquare,
            QuadrantRole::VerticalFamily,
            QuadrantRole::HorizontalFamily,
            QuadrantRole::DetailBasis,
        ],
        MacroStyle::ClassicSourceHotspot | MacroStyle::ClassicHotspotBasis => [
            QuadrantRole::VerticalFamily,
            QuadrantRole::HorizontalFamily,
            QuadrantRole::DetailBasis,
            QuadrantRole::MajorSquare,
        ],
        MacroStyle::MechanicalRadial => [
            QuadrantRole::MajorSquare,
            QuadrantRole::VerticalFamily,
            QuadrantRole::HorizontalFamily,
            QuadrantRole::DetailBasis,
...
fn place_quadrant_role(
    ...
    match role {
        QuadrantRole::MajorSquare => push_leaf(
            leaves,
            rect,
            PartitionFamily::BroadPanel,
            Some(ordinal),
        ...
        QuadrantRole::DetailBasis => {
            place_detail_quadrant(rect, ordinal, hierarchy, radial_remaining, leaves)
        }
    }
}
...
fn place_detail_quadrant(
    ...
    let half_w = rect.width / 2;
    let horizontal_ladder = if hierarchy.strip_thickness_ladder.is_empty() {
        strip_ladder_for(quarter_h)
    } else {
        normalized_ladder(&hierarchy.strip_thickness_ladder, quarter_h)
    };
    ...
    for thickness in horizontal_ladder {
        push_leaf(
            ...
            PartitionFamily::HorizontalStrip,
            None,
            1,
            false,
            HierarchyZone::HorizontalLadder,
            false,
        );
        y += thickness;
    }
    let vertical_ladder = if hierarchy.strip_thickness_ladder.is_empty() {
        strip_ladder_for(quarter_w)
    } else {
        normalized_ladder(&hierarchy.strip_thickness_ladder, quarter_w)
    };
    ...
    for thickness in vertical_ladder {
        push_leaf(
            ...
            PartitionFamily::VerticalStrip,
            None,
            1,
            false,
            HierarchyZone::VerticalLadder,
            false,
        );
        x += thickness;
    }
    ...
    push_leaf(
        ...
        2,
        false,
        radial_zone(radial_b),
        false,
    );
    split_small_cell_into_wide_pair(
        GridRect {
            x: rect.x + quarter_w,
            y: rect.y + half_w,
            width: quarter_w,
    ...
    split_small_cell_into_tall_pair(
        GridRect {
            x: rect.x + half_w,
            y: rect.y + half_w,
            width: quarter_w,
...
fn normalized_ladder(requested: &[u32], extent: u32) -> Vec<u32> {
    ...
    for value in requested.iter().take(desired_segments.saturating_sub(1)) {
        ...
        let thickness = (*value)
            .max(1)
            .min(remaining.saturating_sub(reserved_tail).max(1));
...
fn radial_zone(family: PartitionFamily) -> HierarchyZone {
    if family == PartitionFamily::RadialReservation {
        HierarchyZone::Radial
    } else {
        HierarchyZone::DetailHost
    }
}
...
fn split_small_cell_into_wide_pair(
    rect: GridRect,
    leaves: &mut Vec<HierarchicalLeaf>,
    parent_ordinal: Option<u32>,
) {
...
fn split_small_cell_into_tall_pair(
    rect: GridRect,
    leaves: &mut Vec<HierarchicalLeaf>,
    parent_ordinal: Option<u32>,
) {
...
fn split_medium_square_for_radials(
    leaf: HierarchicalLeaf,
    radial_remaining: &mut u32,
) -> Vec<HierarchicalLeaf> {
    let rect = leaf.rect;
    let half_w = rect.width / 2;
    let half_h = rect.height / 2;
    ...
    for (x, y, width, height) in [
        ...
        output.push(HierarchicalLeaf {
            ...
            family,
            lineage: PartitionLineage {
                parent_ordinal: leaf.lineage.parent_ordinal,
                host_rect: Some(rect),
                depth: 2,
                protected_parent: false,
                zone: radial_zone(family),
            },
...
fn apply_symmetry_to_leaves(
    leaves: &mut [HierarchicalLeaf],
    grid: LogicalGridSpec,
    ...
    for leaf in leaves {
        ...
        leaf.lineage.host_rect = leaf
            .lineage
            .host_rect
            .map(|rect| transform_basis_rect(rect, grid, symmetry));
    }
}

fn apply_basis_complexity(
    leaves: &mut Vec<HierarchicalLeaf>,
    recipe: &PartitionRecipe,
    hierarchy: &HierarchicalLayoutRecipe,
) {
    let depth_floor = 24_usize
        .saturating_add(usize::from(hierarchy.hierarchy_depth.saturating_sub(2)).saturating_mul(3));
    let desired = (hierarchy.target_region_min as usize)
        .max(depth_floor)
        .min(hierarchy.target_region_max as usize);
    if leaves.len() >= desired {
        return;
    }
    ...
    split_detail_squares_until(
        leaves,
        desired.min(recipe.target_region_count as usize),
        hierarchy.target_region_max as usize,
    );
    if leaves.len() >= desired {
        return;
    }
    split_micro_pairs_until(
        leaves,
        desired
            .min(recipe.target_region_count as usize)
            .min(hierarchy.target_region_max as usize),
    );
}

fn split_medium_square_pair(leaves: &mut Vec<HierarchicalLeaf>) {
    let Some(first_index) = leaves.iter().position(|leaf| {
        leaf.family == PartitionFamily::MediumBlock
            && leaf.rect.width == leaf.rect.height
            && leaf.rect.width >= 4
    }) else {
    ...
    let Some(second_index) = leaves.iter().position(|leaf| {
        leaf.family == PartitionFamily::MediumBlock
            && leaf.rect.width == leaf.rect.height
            && leaf.rect.width >= 4
    }) else {
    ...
    let split_horizontal = |leaf: HierarchicalLeaf| {
        ...
        let lineage = PartitionLineage {
            host_rect: Some(leaf.rect),
            depth: leaf.lineage.depth.saturating_add(1),
            zone: HierarchyZone::DetailHost,
            ..leaf.lineage
        };
        [
            HierarchicalLeaf {
                ...
                family: PartitionFamily::MediumBlock,
                lineage,
                splittable: true,
            },
            HierarchicalLeaf {
...
fn split_normal_small_square(leaves: &mut Vec<HierarchicalLeaf>) {
    let Some(index) = leaves.iter().position(|leaf| {
        leaf.family == PartitionFamily::SmallDetail
            && leaf.rect.width == leaf.rect.height
            && leaf.rect.width >= 4
    }) else {
        return;
    ...
    leaves.extend([
        HierarchicalLeaf {
            ...
            family: PartitionFamily::SmallDetail,
            lineage,
            splittable: true,
        },
        HierarchicalLeaf {
            rect: GridRect {
                x: leaf.rect.x + half_w,
                y: leaf.rect.y,
                width: leaf.rect.width - half_w,
                height: half_h,
            },
            family: PartitionFamily::SmallDetail,
            lineage,
        ...
        HierarchicalLeaf {
            rect: GridRect {
                x: leaf.rect.x,
                y: leaf.rect.y + half_h,
                width: half_w,
                height: leaf.rect.height - half_h,
            },
            family: PartitionFamily::SmallDetail,
            lineage,
            splittable: true,
...
fn split_micro_pairs_until(leaves: &mut Vec<HierarchicalLeaf>, desired: usize) {
    while leaves.len() + 1 < desired {
        let Some(wide_index) = leaves.iter().position(|leaf| {
            leaf.family == PartitionFamily::SmallDetail
                && leaf.rect.width >= leaf.rect.height.saturating_mul(2)
                && leaf.rect.height >= 2
        }) else {
            break;
        };
        let wide = leaves.remove(wide_index);
        let Some(tall_index) = leaves.iter().position(|leaf| {
            leaf.family == PartitionFamily::SmallDetail
                && leaf.rect.height >= leaf.rect.width.saturating_mul(2)
                && leaf.rect.width >= 2
        }) else {
            leaves.push(wide);
            break;
...
fn split_leaf_horizontal(leaf: HierarchicalLeaf, family: PartitionFamily) -> [HierarchicalLeaf; 2] {
    ...
    [
        ...
        HierarchicalLeaf {
            ...
            family,
            lineage,
            splittable: leaf.rect.height - half >= 2,
        },
    ]
}
...
fn split_leaf_vertical(leaf: HierarchicalLeaf, family: PartitionFamily) -> [HierarchicalLeaf; 2] {
    ...
    [
        ...
        HierarchicalLeaf {
            ...
            family,
            lineage,
            splittable: leaf.rect.width - half >= 2,
        },
    ]
}

fn validate_hotspot_basis(
    grid: LogicalGridSpec,
    hierarchy: &HierarchicalLayoutRecipe,
    demands: &[RegionDemand],
    leaves: &[HierarchicalLeaf],
    ...
    for leaf in leaves {
        if leaf.family == PartitionFamily::RadialReservation && leaf.rect.width != leaf.rect.height
        {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: format!(
                    "radial region {}x{} is not square",
                    leaf.rect.width, leaf.rect.height
                ),
            });
        }
        *counts
            .entry((leaf.rect.width, leaf.rect.height))
            .or_default() += 1;
    }
    let basis = BasisGrid::new(grid)?;
    let required = [
    ...
    for (width, height, label) in required {
        if counts.get(&(width, height)).copied().unwrap_or(0) == 0 {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: format!("missing required {label} {width}x{height}"),
            });
        }
    }
    for (wide, tall, label) in [
        (
            (basis.cell_w * 4, basis.cell_h * 2),
            (basis.cell_w * 2, basis.cell_h * 4),
            "macro pair",
        ),
        (
            (basis.cell_w * 2, basis.cell_h),
            (basis.cell_w, basis.cell_h * 2),
            "medium pair",
        ),
        (
            (basis.cell_w, (basis.cell_h / 2).max(1)),
            ((basis.cell_w / 2).max(1), basis.cell_h),
            "small pair",
        ),
    ] {
        ...
        if wide_count == 0 || tall_count == 0 || wide_count != tall_count {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: format!("{label} incomplete ({wide_count} wide / {tall_count} tall)"),
            });
        }
    }
    if !leaves
        .iter()
        .any(|leaf| leaf.family == PartitionFamily::HorizontalStrip)
        || !leaves
            .iter()
            .any(|leaf| leaf.family == PartitionFamily::VerticalStrip)
    {
        return Err(PartitionError::InvalidHotspotBasis {
            reason: "horizontal and vertical strip ladders must both be present".into(),
        });
    }
    if hierarchy.radial_count > 0 {
        let radial_count = leaves
            .iter()
            .filter(|leaf| leaf.family == PartitionFamily::RadialReservation)
            .count() as u32;
        if radial_count != hierarchy.radial_count {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: format!(
                    "expected {} radial slots, found {radial_count}",
                    hierarchy.radial_count
                ),
            });
        }
...
fn validate_exact_cover(
    ...
    for leaf in leaves {
        if leaf.rect.width == 0
            || leaf.rect.height == 0
            || leaf.rect.x + leaf.rect.width > grid.width
            || leaf.rect.y + leaf.rect.height > grid.height
        {
            return Err(PartitionError::InvalidHotspotBasis {
                reason: format!("region is out of bounds: {:?}", leaf.rect),
            });
        }
    ...
    if cells.iter().any(|value| *value != 1) {
        return Err(PartitionError::InvalidHotspotBasis {
            reason: "regions must cover every logical cell exactly once".into(),
        });
    }
    Ok(())
}
...
#[allow(dead_code)]
fn hierarchical_macro_zones(
    grid: LogicalGridSpec,
    hierarchy: &HierarchicalLayoutRecipe,
) -> Result<Vec<HierarchicalZoneRect>, PartitionError> {
    let root = GridRect {
        x: 0,
        y: 0,
        width: grid.width,
    ...
    if hierarchy.macro_style == MacroStyle::ClassicSourceHotspot {
        ...
        return Ok(vec![
            HierarchicalZoneRect {
                ...
                kind: HierarchicalZoneKind::VerticalLadder,
            },
            HierarchicalZoneRect {
                rect: GridRect {
                    x: 0,
                    y: quarter_y,
                    width: half_x,
                    height: upper_half - quarter_y,
                },
                kind: HierarchicalZoneKind::HorizontalLadder,
            },
            HierarchicalZoneRect {
                rect: GridRect {
                    x: half_x,
                    y: 0,
                    width: grid.width - half_x,
                    height: upper_half,
                },
                kind: HierarchicalZoneKind::Cascade,
            },
            HierarchicalZoneRect {
                rect: GridRect {
                    x: 0,
                    y: upper_half,
                    width: grid.width,
                    height: grid.height - upper_half,
                },
                kind: HierarchicalZoneKind::Panels,
            },
        ]);
    }
    ...
    if hierarchy.radial_count > 0 && hierarchy.radial_share_milli > 0 {
        let requested_width = ((u64::from(grid.width) * u64::from(hierarchy.radial_share_milli)
            + 999)
            / 1_000) as u32;
        let width = requested_width
            .max(hierarchy.radial_min_diameter)
            .min(remainder.width.saturating_sub(2).max(1));
        let radial = GridRect {
            x: remainder.x + remainder.width - width,
            y: remainder.y,
            width,
            height: remainder.height,
        };
        remainder.width -= width;
        zones.push(HierarchicalZoneRect {
            rect: radial,
            kind: HierarchicalZoneKind::Radial,
        });
    }
    if hierarchy.strip_share_milli > 0 {
        ...
        if vertical_dominant {
            let width = ((u64::from(remainder.width) * u64::from(hierarchy.strip_share_milli)
                + 999)
                / 1_000) as u32;
            ...
            let height = ((u64::from(remainder.height) * u64::from(hierarchy.strip_share_milli)
                + 999)
                / 1_000) as u32;
            let height = height.max(1).min(remainder.height.saturating_sub(2).max(1));
            let strip = GridRect {
                x: remainder.x,
                y: remainder.y + remainder.height - height,
                width: remainder.width,
                height,
            };
            remainder.height -= height;
            if hierarchy.horizontal_strip_weight_milli > 0
                && hierarchy.vertical_strip_weight_milli > 0
                && strip.width >= 4
            {
                let horizontal_width = ((u64::from(strip.width)
                    * u64::from(hierarchy.horizontal_strip_weight_milli))
                    / 1_000) as u32;
                ...
                zones.push(HierarchicalZoneRect {
                    rect: GridRect {
                        x: strip.x + horizontal_width,
                        width: strip.width - horizontal_width,
                        ..strip
                    },
                    kind: HierarchicalZoneKind::VerticalLadder,
                });
...
fn generate_panel_zone(
    ...
    while parents.len() < count as usize {
        let index = parents
            ...
            .max_by_key(|(index, value)| {
                (
                    u64::from(value.width) * u64::from(value.height),
                    usize::MAX - *index,
                )
            })
            .map(|(index, _)| index)
        ...
        let Some((first, second)) =
            best_hierarchical_split(parent, &context.hierarchy.major_aspects, context, 0)
        else {
            parents.push(parent);
            break;
        };
    ...
    let preserve_classic_lower_panels = context.hierarchy.macro_style
        == MacroStyle::ClassicSourceHotspot
        ...
        let should_subdivide = !preserve_classic_lower_panels
            ...
            && index
                < context
                    .hierarchy
                    .protected_parent_count
                    .saturating_add(context.hierarchy.subdividable_parent_count)
                    as usize;
        if should_subdivide {
            ...
            leaves.push(HierarchicalLeaf {
                ...
                family: PartitionFamily::BroadPanel,
                lineage: PartitionLineage {
                    parent_ordinal: Some(parent_ordinal),
                    host_rect: Some(parent),
                    depth: 0,
                    protected_parent: true,
                    zone: HierarchyZone::MacroPanel,
                },
                splittable: false,
            });
        }
...
#[allow(dead_code)]
fn cascade_subdivide(
    mut continuation: GridRect,
    parent_ordinal: u32,
    context: &mut HierarchicalContext<'_>,
    leaves: &mut Vec<HierarchicalLeaf>,
) -> Result<(), PartitionError> {
    ...
    for level in 1..=context.hierarchy.hierarchy_depth {
        let palette = if level == 1 {
            &context.hierarchy.medium_aspects
        } else {
            &context.hierarchy.detail_aspects
        };
        let immediate_host = continuation;
        let Some((first, second)) = best_hierarchical_split(continuation, palette, context, level)
        else {
            break;
        };
        let (terminal, next) = if u64::from(first.width) * u64::from(first.height)
            <= u64::from(second.width) * u64::from(second.height)
        {
            (first, second)
        } else {
            (second, first)
        ...
        leaves.push(HierarchicalLeaf {
            ...
            lineage: PartitionLineage {
                parent_ordinal: Some(parent_ordinal),
                host_rect: Some(if last { immediate_host } else { macro_host }),
                depth: level,
                protected_parent: false,
                zone: if last {
                    HierarchyZone::DetailHost
...
fn balanced_subdivide(
    ...
    let Some((first, second)) =
        best_hierarchical_split(rect, &context.hierarchy.medium_aspects, context, 1)
    else {
        leaves.push(HierarchicalLeaf {
            rect,
            family: PartitionFamily::MediumBlock,
...
fn generate_strip_ladder(
    ...
    while offset < extent {
        ...
        leaves.push(HierarchicalLeaf {
            rect: strip,
            family: if thickness == 1 && index >= ladder.len() {
                PartitionFamily::MicroStrip
            } else if horizontal {
                PartitionFamily::HorizontalStrip
            } else {
                PartitionFamily::VerticalStrip
...
fn generate_radial_zone(
    ...
    for ordinal in 0..context.hierarchy.radial_count {
        let index = remainders
            .iter()
            .enumerate()
            .max_by_key(|(_, value)| u64::from(value.width) * u64::from(value.height))
            .map(|(index, _)| index)
            .ok_or(PartitionError::InvalidHierarchicalRecipe)?;
        ...
        let available = host
            .width
            .min(host.height)
            .min(context.hierarchy.radial_max_diameter);
        if available < context.hierarchy.radial_min_diameter {
            return Err(PartitionError::InvalidHierarchicalRecipe);
        }
        let span =
            context.hierarchy.radial_max_diameter - context.hierarchy.radial_min_diameter + 1;
        let diameter = context.hierarchy.radial_min_diameter
            + ((context.recipe.seed + u64::from(ordinal)) % u64::from(span)) as u32;
        ...
        leaves.push(HierarchicalLeaf {
            ...
            lineage: PartitionLineage {
                depth: 1,
                zone: HierarchyZone::Radial,
                ..PartitionLineage::default()
            },
            splittable: false,
        });
        if diameter < host.width {
            remainders.push(GridRect {
                x: host.x + diameter,
                width: host.width - diameter,
                ..host
            });
        }
        if diameter < host.height {
            remainders.push(GridRect {
                y: host.y + diameter,
                width: diameter,
                height: host.height - diameter,
                ..host
            });
        }
    }
    leaves.extend(remainders.into_iter().map(|value| HierarchicalLeaf {
        rect: value,
        family: PartitionFamily::Remainder,
        lineage: PartitionLineage {
            zone: HierarchyZone::Radial,
...
#[allow(dead_code)]
fn constrained_hierarchical_cleanup(
    leaves: &mut Vec<HierarchicalLeaf>,
    context: &mut HierarchicalContext<'_>,
) {
    while leaves.len() < context.hierarchy.target_region_min as usize
        && leaves.len() < context.hierarchy.target_region_max as usize
    {
        ...
        let leaf = leaves.remove(index);
        let palette = if leaf.family == PartitionFamily::MediumBlock {
            &context.hierarchy.detail_aspects
        } else {
            &context.hierarchy.medium_aspects
        };
        let Some((first, second)) = best_hierarchical_split(
            leaf.rect,
            palette,
            context,
            leaf.lineage.depth.saturating_add(1),
        ) else {
            leaves.push(HierarchicalLeaf {
                splittable: false,
                ..leaf
        ...
        let family = if leaf.family == PartitionFamily::MediumBlock {
            PartitionFamily::SmallDetail
        } else {
            leaf.family
        };
        let lineage = PartitionLineage {
            host_rect: Some(leaf.rect),
            depth: leaf.lineage.depth.saturating_add(1),
            zone: if family == PartitionFamily::SmallDetail {
                HierarchyZone::DetailHost
            } else {
                leaf.lineage.zone
            },
        ...
        leaves.extend([
            HierarchicalLeaf {
                ...
                family,
                lineage,
                splittable: first.width >= 2 || first.height >= 2,
            },
            HierarchicalLeaf {
                rect: second,
                family,
                lineage,
                splittable: second.width >= 2 || second.height >= 2,
            },
        ]);
    }
}
...
#[allow(dead_code)]
fn best_hierarchical_split(
    rect: GridRect,
    palette: &[AspectClass],
    context: &mut HierarchicalContext<'_>,
    level: u8,
) -> Option<(GridRect, GridRect)> {
    ...
    for vertical in [true, false] {
        ...
        for ratio in &context.hierarchy.allowed_split_ratios {
            let (numerator, denominator) = match ratio {
                SplitRatio::Half => (1, 2),
                SplitRatio::OneThird => (1, 3),
                SplitRatio::TwoThird => (2, 3),
            };
            let base = extent.saturating_mul(numerator) / denominator;
            let variation_span =
                extent.saturating_mul(u32::from(context.hierarchy.variation_milli)) / 2_000;
            let variation = if variation_span == 0 {
                0_i32
            } else {
                ...
                ((context
                    .recipe
                    .seed
                    .wrapping_add(u64::from(context.split_ordinal))
                    .wrapping_add(u64::from(level))
                    % u64::from(range)) as i32)
                    - variation_span as i32
            };
            let cut = (base as i32 + variation).clamp(1, extent as i32 - 1) as u32;
            let (first, second) = if vertical {
                (
                    GridRect { width: cut, ..rect },
                    GridRect {
                        x: rect.x + cut,
            ...
            if [first, second].iter().any(|value| {
                value.width < context.recipe.minimum_logical_width
                    || value.height < context.recipe.minimum_logical_height
            }) {
                continue;
            }
            let aspect_error = i64::from(nearest_aspect_error(first, palette))
                + i64::from(nearest_aspect_error(second, palette));
            ...
            let desired_milli = if level == 0 {
                context.hierarchy.large_share_milli.clamp(100, 900)
            } else {
                context.hierarchy.scale_falloff_milli
            };
            let desired = total_area * u64::from(desired_milli) / 1_000;
            let area_error = first_area.abs_diff(desired) as i64;
            let tiny_penalty = [first, second]
                .iter()
                .map(|value| {
                    if value.width.min(value.height) <= 1 {
                        20_000
                    } else if value.width.min(value.height) <= 2 {
                        2_000
                    } else {
                        0
                    }
            ...
            let aligned = if vertical {
                context.cuts_x.contains(&absolute_cut)
            } else {
                context.cuts_y.contains(&absolute_cut)
            };
            let alignment_bonus = if aligned {
                i64::from(context.hierarchy.alignment_strength_milli) * 20
            } else {
                0
            };
    ...
    if vertical {
        context.cuts_x.insert(rect.x + cut);
    } else {
        context.cuts_y.insert(rect.y + cut);
    }
    ...
    palette
        ...
        .map(|class| {
            let target = match class {
                AspectClass::Square => 1_000,
                AspectClass::Wide2 => 2_000,
                AspectClass::Tall2 => 500,
                AspectClass::Wide4 => 4_000,
                AspectClass::Tall4 => 250,
                AspectClass::Wide8 => 8_000,
                AspectClass::Tall8 => 125,
                AspectClass::Wide16 => 16_000,
                AspectClass::Tall16 => 62,
            };
...
fn can_split_leaf(leaf: PartitionLeaf, recipe: &PartitionRecipe) -> bool {
    if leaf.family == PartitionFamily::Remainder {
        return leaf.fill_remainder
            && (leaf.rect.width >= recipe.minimum_logical_width.saturating_mul(2)
                || leaf.rect.height >= recipe.minimum_logical_height.saturating_mul(2));
    }
    if leaf.subdivision_budget == 0 {
        return false;
    }
    let quota = family_quota(leaf.family, &recipe.composition);
    quota.is_none_or(|value| {
        leaf.rect.width >= value.minimum_width.saturating_mul(2)
...
fn reserve_composition(
    recipe: &PartitionRecipe,
    leaves: &mut Vec<PartitionLeaf>,
) -> Result<(), PartitionError> {
    ...
    for (family, count) in requests {
        for ordinal in 0..count {
            let index = leaves
                ...
                .enumerate()
                .filter(|(_, leaf)| {
                    leaf.family == PartitionFamily::Remainder && leaf.fill_remainder
                })
                .max_by_key(|(_, leaf)| u64::from(leaf.rect.width) * u64::from(leaf.rect.height))
                .map(|(index, _)| index)
                .ok_or_else(|| PartitionError::ImpossibleFamilyQuota {
                    family,
                    reason: "earlier reservations consume the remaining frame".into(),
                    suggestion: "reduce reserved family counts or increase Count".into(),
                })?;
            let leaf = leaves.remove(index);
            let rect = leaf.rect;
            let depth = leaf.depth;
            let thickness = match family {
                PartitionFamily::HorizontalStrip => Some(varied_strip_thickness(
                    profile.horizontal_strips,
                    recipe.seed,
                    ordinal,
                )),
                PartitionFamily::VerticalStrip => Some(varied_strip_thickness(
                    profile.vertical_strips,
                    recipe.seed,
                    ordinal,
                )),
                PartitionFamily::MicroStrip => Some(varied_strip_thickness(
                    profile.micro_strips,
                    recipe.seed,
                    ordinal,
                )),
                _ => None,
            };
            let (reserved, remainders) =
                reserve_family_rect(recipe, rect, family, thickness, ordinal).ok_or_else(|| {
                    PartitionError::ImpossibleFamilyQuota {
                        family,
                        reason: "no shared boundary can create an eligible reserved rectangle"
                            .into(),
                        suggestion:
                            "relax family dimensions/aspect/thickness or reduce requested families"
                                .into(),
                    }
                })?;
            let subdivision_budget =
                family_quota(family, profile).map_or(0, |quota| quota.subdivision_budget);
            ...
            leaves.extend(
                remainders
                    .into_iter()
                    .map(|remainder| PartitionLeaf::remainder(remainder, depth + 1, true)),
            );
        }
    }
    Ok(())
...
fn varied_strip_thickness(quota: StripQuota, seed: u64, ordinal: u32) -> u32 {
    let span = quota
        .maximum_thickness
        .saturating_sub(quota.minimum_thickness)
        .saturating_add(1);
    quota
        .minimum_thickness
        .saturating_add((seed.wrapping_add(u64::from(ordinal)) % u64::from(span.max(1))) as u32)
}

fn reserve_family_rect(
    recipe: &PartitionRecipe,
    rect: GridRect,
    family: PartitionFamily,
    thickness: Option<u32>,
    ordinal: u32,
) -> Option<(GridRect, Vec<GridRect>)> {
    let profile = &recipe.composition;
    let (width, height) = match family {
        PartitionFamily::HorizontalStrip => (
            rect.width,
            thickness?.clamp(1, rect.height.saturating_sub(1)),
        ),
        PartitionFamily::VerticalStrip => (
            thickness?.clamp(1, rect.width.saturating_sub(1)),
            rect.height,
        ),
        PartitionFamily::MicroStrip if ordinal % 2 == 0 => (
            rect.width,
            thickness?.clamp(1, rect.height.saturating_sub(1)),
        ),
        PartitionFamily::MicroStrip => (
        ...
        PartitionFamily::RadialReservation => {
            ...
            let columns = (1..=requested_slots)
                .find(|value| (*value).saturating_mul(*value) >= requested_slots)
                .unwrap_or(requested_slots);
            let desired = (recipe.grid.width.min(recipe.grid.height) / columns).max(1);
            let available = rect
                .width
                .min(rect.height)
                .min(quota.allocation_max_diameter);
            if available < quota.allocation_min_diameter {
                return None;
            }
        ...
        | PartitionFamily::SmallDetail => {
            ...
            let desired_area = if quota.area_share_milli > 0 && quota.count > 0 {
                total_area.saturating_mul(u64::from(quota.area_share_milli))
                    / 1_000
                    / u64::from(quota.count)
            } else {
                let divisor = if family == PartitionFamily::SmallDetail {
                    4
                } else {
                    2
                };
            ...
            (quota.minimum_width..=quota.maximum_width.min(rect.width))
                .flat_map(|width| {
                    (quota.minimum_height..=quota.maximum_height.min(rect.height))
    ...
    let reserved = GridRect {
        x: rect.x,
        y: rect.y,
        width,
        height,
    };
    if !family_matches(family, reserved, profile) {
        return None;
    }
...
fn family_matches(family: PartitionFamily, rect: GridRect, profile: &CompositionProfile) -> bool {
    ...
    let block = |quota: FamilyQuota| {
        rect.width >= quota.minimum_width
            && rect.height >= quota.minimum_height
            && rect.width <= quota.maximum_width
            && rect.height <= quota.maximum_height
            && aspect_milli >= quota.minimum_aspect_milli
            && aspect_milli <= quota.maximum_aspect_milli
    };
    match family {
        ...
        PartitionFamily::HorizontalStrip => {
            rect.height >= profile.horizontal_strips.minimum_thickness
                && rect.height <= profile.horizontal_strips.maximum_thickness
        }
        PartitionFamily::VerticalStrip => {
            rect.width >= profile.vertical_strips.minimum_thickness
                && rect.width <= profile.vertical_strips.maximum_thickness
        }
        PartitionFamily::MicroStrip => {
            rect.width.min(rect.height) >= profile.micro_strips.minimum_thickness
                && rect.width.min(rect.height) <= profile.micro_strips.maximum_thickness
        }
        PartitionFamily::RadialReservation => {
            rect.width.min(rect.height) >= profile.radial_reservations.allocation_min_diameter
                && rect.width.min(rect.height)
                    <= profile.radial_reservations.allocation_max_diameter
        }
...
pub fn resolve_boundaries(start: u32, extent: u32, cells: u32) -> Vec<u32> {
    (0..=cells)
        .map(|index| {
            (f64::from(start) + f64::from(index) * f64::from(extent) / f64::from(cells)).round()
                as u32
        })
        .collect()
}
...
mod tests {
    ...
    fn dim_count(regions: &[PartitionRegion], width: u32, height: u32) -> usize {
        regions
            .iter()
            .filter(|region| region.grid_rect.width == width && region.grid_rect.height == height)
            .count()
    }
    ...
    fn assert_hotspot_basis_inventory(regions: &[PartitionRegion], expected_radials: usize) {
        for (width, height, label) in [
            ...
            assert!(
                dim_count(regions, width, height) > 0,
                "missing {label} {width}x{height}"
            );
        }
        ...
        assert!(
            regions
                .iter()
                .any(|region| region.family == PartitionFamily::HorizontalStrip),
            "missing horizontal strip ladder"
        );
        assert!(
            regions
                .iter()
                .any(|region| region.family == PartitionFamily::VerticalStrip),
            "missing vertical strip ladder"
        );
        let radial = regions
            .iter()
            .filter(|region| region.family == PartitionFamily::RadialReservation)
            .collect::<Vec<_>>();
        assert_eq!(radial.len(), expected_radials, "radial count");
        assert!(
            radial
                .iter()
                .all(|region| region.grid_rect.width == region.grid_rect.height),
            "radial allocation regions must be square"
        );
    }
    ...
    fn source_frame_partition_counts_are_not_template_contracts() {
        for target in [16, 63, 103] {
            assert_partition(LogicalGridSpec::DEFAULT, target);
        }
    }
    ...
    fn hierarchical_default_has_lineage_ladders_radials_and_soft_count() {
        ...
        assert_eq!(
            first
                .iter()
                .filter(|region| region.lineage.protected_parent
                    && region.family == PartitionFamily::BroadPanel)
                .count(),
            3
        );
        assert!(
            first
                .iter()
                .any(|region| region.family == PartitionFamily::MediumBlock
                    && region.lineage.parent_ordinal.is_some())
        );
        assert!(first.iter().any(
            |region| region.family == PartitionFamily::SmallDetail && region.lineage.depth >= 2
        ));
        ...
        assert!(
            radial
                .iter()
                .all(|region| region.grid_rect.width == region.grid_rect.height
                    && region.lineage.zone == HierarchyZone::Radial)
        );
        let mut cells = vec![0_u8; (recipe.grid.width * recipe.grid.height) as usize];
        for region in first {
            for y in region.grid_rect.y..region.grid_rect.y + region.grid_rect.height {
                for x in region.grid_rect.x..region.grid_rect.x + region.grid_rect.width {
                    cells[(y * recipe.grid.width + x) as usize] += 1;
                }
            }
        }
        assert!(cells.iter().all(|value| *value == 1));
    ...
    fn hierarchical_product_presets_generate_inside_their_soft_ranges() {
        let presets = [
            ...
            ("panel-cascade", {
                ...
                value.macro_style = MacroStyle::PanelCascade;
                value.large_share_milli = 600;
                value.medium_share_milli = 180;
                value.small_share_milli = 60;
                value.strip_share_milli = 120;
                value.radial_share_milli = 40;
                value.target_region_min = 29;
                value.target_region_max = 36;
                value.radial_count = 2;
                value
            }),
            ("horizontal-trims", {
                let mut value = HierarchicalLayoutRecipe::mixed_hierarchy_default();
                value.macro_style = MacroStyle::HorizontalTrims;
                value.recursive_policy = RecursivePolicy::Balanced;
                value.large_share_milli = 460;
                value.medium_share_milli = 160;
                value.small_share_milli = 60;
                value.strip_share_milli = 280;
                value.radial_share_milli = 40;
                value.target_region_min = 29;
                value.target_region_max = 38;
                value.horizontal_strip_weight_milli = 800;
                value.vertical_strip_weight_milli = 200;
                value.strip_thickness_ladder = vec![1, 1, 1, 2, 2, 3, 4, 6];
                value.radial_count = 2;
                value
            }),
            ("facade-halving", {
                let mut value = HierarchicalLayoutRecipe::mixed_hierarchy_default();
                value.macro_style = MacroStyle::FacadeHalving;
                value.recursive_policy = RecursivePolicy::Balanced;
                value.large_share_milli = 680;
                value.medium_share_milli = 200;
                value.small_share_milli = 40;
                value.strip_share_milli = 40;
                value.radial_share_milli = 40;
                value.target_region_min = 24;
                value.target_region_max = 30;
                value.hierarchy_depth = 2;
                value.allowed_split_ratios = vec![SplitRatio::Half];
                value.alignment_strength_milli = 1_000;
                value.variation_milli = 0;
                value.horizontal_strip_weight_milli = 1_000;
                value.vertical_strip_weight_milli = 0;
                value.radial_count = 2;
                value
            }),
            ("classic", {
                let mut value = HierarchicalLayoutRecipe::mixed_hierarchy_default();
                value.macro_style = MacroStyle::ClassicHotspotBasis;
                value.large_share_milli = 540;
                value.medium_share_milli = 180;
                value.small_share_milli = 60;
                value.strip_share_milli = 180;
                value.radial_share_milli = 40;
                value.target_region_min = 24;
                value.target_region_max = 24;
                value.horizontal_strip_weight_milli = 545;
                value.vertical_strip_weight_milli = 455;
                value.radial_count = 2;
                value.variation_milli = 0;
                value
            }),
            ("mechanical", {
                let mut value = HierarchicalLayoutRecipe::mixed_hierarchy_default();
                value.macro_style = MacroStyle::MechanicalRadial;
                value.recursive_policy = RecursivePolicy::Balanced;
                value.large_share_milli = 460;
                value.medium_share_milli = 180;
                value.small_share_milli = 100;
                value.strip_share_milli = 140;
                value.radial_share_milli = 120;
                value.target_region_min = 32;
                value.target_region_max = 40;
                value.radial_count = 4;
                value.radial_max_diameter = 12;
                value
            }),
        ];
        for (name, hierarchy) in presets {
            let mut recipe = PartitionRecipe::default_for(
                LogicalGridSpec::DEFAULT,
                hierarchy.target_region_max,
                5,
            );
            recipe.recipe_version = 3;
            ...
            assert!(
                (hierarchy.target_region_min..=hierarchy.target_region_max)
                    .contains(&(regions.len() as u32)),
                "{name} soft count"
            );
    ...
    fn hierarchical_recipe_migrates_legacy_json_and_rejects_unified_share_errors() {
        assert_eq!(
            serde_json::to_value(SplitRatio::TwoThird).unwrap(),
            serde_json::json!("two_third")
        );
        assert_eq!(
            serde_json::from_value::<SplitRatio>(serde_json::json!("two_thirds")).unwrap(),
            SplitRatio::TwoThird,
            "plural preview payloads remain readable during migration"
        );
        ...
        let decoded: PartitionRecipe =
            serde_json::from_value(json).expect("version-2 recipe remains readable");
        assert!(decoded.hierarchical.is_none());
        assert_eq!(
            generate_partition(&decoded)
                .expect("legacy generator remains available")
                .len(),
            24
        );
        ...
        assert!(matches!(
            invalid.validate(),
            Err(PartitionError::InvalidHierarchicalShares { total_milli: 999 })
        ));
    }
    ...
    fn classic_hotspot_basis_has_exact_required_inventory() {
        ...
        assert_eq!(
            dim_count(&regions, 16, 16),
            4,
            "classic basis carries four 16x16 material variants"
        );
    ...
    fn basis_rotations_mirroring_and_radial_variants_preserve_roles() {
        for style in [
            ...
            recipe.hierarchical = Some(hierarchy);
            let regions =
                generate_partition(&recipe).unwrap_or_else(|error| panic!("{style:?}: {error}"));
            assert_hotspot_basis_inventory(&regions, 2);
        }
    ...
    fn generated_for_golden(name: &str) -> Vec<PartitionRegion> {
        ...
        let mut recipe =
            PartitionRecipe::default_for(LogicalGridSpec::DEFAULT, hierarchy.target_region_max, 0);
        recipe.recipe_version = 3;
        recipe.hierarchical = Some(hierarchy);
        generate_partition(&recipe).unwrap_or_else(|error| panic!("{name}: {error}"))
    }
    ...
    fn topology_signature(
        ...
        transform: SymmetryTransform,
    ) -> BTreeSet<(PartitionFamily, u32, u32, u32, u32)> {
        regions
            .iter()
            .map(|region| {
                let rect =
                    transform_basis_rect(region.grid_rect, LogicalGridSpec::DEFAULT, transform);
    ...
    fn visible_layout_families_are_not_just_symmetry_variants() {
        ...
        let transforms = [
            ...
            SymmetryTransform::MirrorDiagonal,
            SymmetryTransform::MirrorAntiDiagonal,
        ];
        for name in [
            "panel-cascade",
            "horizontal-trim-sheet",
            "facade-halving",
            "classic-source-hotspot",
            "mechanical-radial",
        ] {
            let other =
                topology_signature(&generated_for_golden(name), SymmetryTransform::Identity);
            assert!(
                transforms
                    .iter()
                    .all(|transform| topology_signature(&diagonal, *transform) != other),
                "{name} collapsed to a rotated or mirrored Diagonal Cascade"
            );
        }
    }
    ...
    fn active_hierarchical_controls_affect_topology() {
        ...
        let layout_for = |hierarchy: HierarchicalLayoutRecipe, seed: u64| {
            ...
            recipe.hierarchical = Some(hierarchy);
            topology_signature(
                &generate_partition(&recipe).expect("control layout"),
                SymmetryTransform::Identity,
            )
        };
    ...
    fn hierarchy_svg(name: &str, regions: &[PartitionRegion]) -> String {
        let mut svg = format!(
            "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"768\" height=\"768\" viewBox=\"0 0 64 64\" aria-label=\"{name} hierarchical layout\">\n<rect width=\"64\" height=\"64\" fill=\"#10141b\"/>\n"
        );
        for (index, region) in regions.iter().enumerate() {
            let (fill, code) = match region.family {
                PartitionFamily::BroadPanel => ("#385a7c", "L"),
                PartitionFamily::MediumBlock => ("#557d67", "M"),
                PartitionFamily::SmallDetail => ("#9b6c58", "D"),
                PartitionFamily::HorizontalStrip => ("#876a9e", "H"),
                PartitionFamily::VerticalStrip => ("#8b884b", "V"),
                PartitionFamily::MicroStrip => ("#b1778f", "u"),
                PartitionFamily::RadialReservation => ("#b0713d", "R"),
                PartitionFamily::Remainder => ("#343b46", "C"),
            };
    ...
    fn hierarchical_layout_goldens_match_reviewed_line_and_id_fixtures() {
        ...
        let directory = if update {
            manifest_directory.join("../../target/hierarchical-goldens")
        } else {
        ...
        for name in [
            ...
            let expected = std::fs::read_to_string(&path).unwrap_or_else(|_| {
                panic!(
                    "missing golden {}; run with UPDATE_HIERARCHICAL_GOLDENS=1",
                    path.display()
                )
            });
            assert_eq!(
    ...
    fn boundary_tables_are_shared_and_reconstruct_a_frame() {
        ...
        assert_eq!(xs[0], 1_000);
        assert_eq!(*xs.last().unwrap(), 5_000);
    ...
    fn intentional_source_partition_composition_quotas_are_exact_and_deterministic() {
        ...
        for (family, expected) in [
            (PartitionFamily::BroadPanel, 2),
            (PartitionFamily::MediumBlock, 2),
            (PartitionFamily::HorizontalStrip, 3),
            (PartitionFamily::VerticalStrip, 3),
            (PartitionFamily::SmallDetail, 4),
            (PartitionFamily::RadialReservation, 2),
        ] {
            assert_eq!(
                first
                    .iter()
                    .filter(|region| region.family == family)
                    .count(),
                expected
            );
        }
        assert!(
            first
                .iter()
                .filter(|region| region.family == PartitionFamily::HorizontalStrip)
                .all(|region| region.grid_rect.height
                    >= recipe.composition.horizontal_strips.minimum_thickness)
        );
        assert!(
            first
                .iter()
                .filter(|region| region.family == PartitionFamily::BroadPanel)
                .all(|region| region.grid_rect.width
                    >= recipe.composition.broad_panels.minimum_width
    ...
    fn intentional_source_partition_rejects_unsatisfied_quotas_without_partial_layout() {
        ...
        assert!(matches!(
            generate_partition(&recipe),
            Err(PartitionError::QuotaExceedsTarget { .. })
        ));
    }
    ...
    fn radial_slot_reservations_are_feasible_on_the_default_grid() {
        ...
        recipe
            ...
            .radial_reservations
            .allocation_max_diameter = 64;
        let regions = generate_partition(&recipe)
            .expect("four radial allocation slots leave legal remainder");
        let radial = regions
            .iter()
            .filter(|region| region.family == PartitionFamily::RadialReservation)
            .collect::<Vec<_>>();
    ...
    fn product_default_composition_contains_major_medium_band_trim_and_detail_regions() {
        ...
        recipe.composition.broad_panels = FamilyQuota {
            count: 4,
            area_share_milli: 480,
            minimum_width: 10,
            minimum_height: 9,
            maximum_width: 64,
            maximum_height: 64,
        ...
        recipe.composition.medium_blocks = FamilyQuota {
            count: 6,
            area_share_milli: 200,
            minimum_width: 4,
            minimum_height: 4,
            maximum_width: 32,
            maximum_height: 32,
        ...
        recipe.composition.horizontal_strips = StripQuota {
            count: 7,
            minimum_thickness: 1,
            maximum_thickness: 3,
        };
        recipe.composition.vertical_strips = StripQuota {
            count: 5,
            minimum_thickness: 1,
            maximum_thickness: 2,
        };
        recipe.composition.small_details = FamilyQuota {
            count: 4,
            area_share_milli: 60,
            minimum_width: 1,
            minimum_height: 1,
        ...
        recipe.composition.micro_strips = StripQuota {
            count: 4,
            minimum_thickness: 1,
            maximum_thickness: 1,
        };
        ...
        for (family, minimum) in [
            (PartitionFamily::BroadPanel, 4),
            (PartitionFamily::MediumBlock, 6),
            (PartitionFamily::HorizontalStrip, 7),
            (PartitionFamily::VerticalStrip, 5),
            (PartitionFamily::SmallDetail, 4),
            (PartitionFamily::MicroStrip, 4),
        ] {
            assert!(
                regions
                    .iter()
        ...
        assert!(
            regions
                .iter()
                .filter(|region| region.family == PartitionFamily::BroadPanel)
                .all(|region| u64::from(region.grid_rect.width)
                    * u64::from(region.grid_rect.height)
                    >= 64)
        );
        assert!(
            regions
                ...
                .any(|region| region.grid_rect.width.min(region.grid_rect.height) == 1),
            "one-cell trims remain legal and visible"
        );
    }
    ...
    fn six_reference_template_recipes_generate_exact_cover_layouts() {
        struct Reference {
            ...
            vertical_bias: u16,
            variance: u16,
            broad: (u32, u16, u32, u32),
            medium: (u32, u16),
            horizontal: u32,
            vertical: u32,
            detail: (u32, u16, u32, u32),
            micro: u32,
        }
        let references = [
            Reference {
                name: "sparse-structural",
                target: 20,
                horizontal_bias: 350,
                vertical_bias: 650,
                variance: 80,
        ...
        for reference in references {
            let mut recipe =
                PartitionRecipe::default_for(LogicalGridSpec::DEFAULT, reference.target, 0);
            ...
            recipe.composition.broad_panels = FamilyQuota {
                count: reference.broad.0,
                area_share_milli: reference.broad.1,
                minimum_width: reference.broad.2,
                minimum_height: reference.broad.3,
                maximum_width: 64,
                maximum_height: 64,
                minimum_aspect_milli: 250,
                maximum_aspect_milli: 4_000,
            ...
            recipe.composition.medium_blocks = FamilyQuota {
                count: reference.medium.0,
                area_share_milli: reference.medium.1,
                minimum_width: 4,
                minimum_height: 4,
                maximum_width: 32,
                maximum_height: 32,
            ...
            recipe.composition.horizontal_strips = StripQuota {
                count: reference.horizontal,
                minimum_thickness: 1,
                maximum_thickness: 3,
            };
            recipe.composition.vertical_strips = StripQuota {
                count: reference.vertical,
                minimum_thickness: 1,
                maximum_thickness: 2,
            };
            recipe.composition.small_details = FamilyQuota {
                count: reference.detail.0,
                area_share_milli: reference.detail.1,
                minimum_width: 1,
                minimum_height: 1,
                maximum_width: reference.detail.2,
                maximum_height: reference.detail.3,
                minimum_aspect_milli: 125,
                maximum_aspect_milli: 8_000,
                subdivision_budget: u16::from(reference.detail.0 > 0),
            };
            recipe.composition.micro_strips = StripQuota {
                count: reference.micro,
                minimum_thickness: 1,
                maximum_thickness: if reference.name == "sparse-structural" {
                    1
                } else {
                    2
                },
            ...
            let regions = generate_partition(&recipe)
                .unwrap_or_else(|error| panic!("{} must generate: {error}", reference.name));
            assert_eq!(
                regions.len(),
                reference.target as usize,
                "{} reaches its declared Count",
                reference.name
            );
            for (family, expected) in [
                (PartitionFamily::BroadPanel, reference.broad.0),
                (PartitionFamily::MediumBlock, reference.medium.0),
                (PartitionFamily::HorizontalStrip, reference.horizontal),
                (PartitionFamily::VerticalStrip, reference.vertical),
                (PartitionFamily::SmallDetail, reference.detail.0),
                (PartitionFamily::MicroStrip, reference.micro),
            ] {
                assert_eq!(
                    regions
                        .iter()
            ...
            if reference.horizontal >= 3 {
                let thicknesses = regions
                    .iter()
                    .filter(|region| region.family == PartitionFamily::HorizontalStrip)
                    .map(|region| region.grid_rect.height)
                    .collect::<BTreeSet<_>>();
                assert!(
                    thicknesses.len() >= 2,
                    "{} uses the configured horizontal thickness range",
                    reference.name
                );
            }
            let mut cells = vec![0_u8; (recipe.grid.width * recipe.grid.height) as usize];
            ...
            assert!(
                cells.iter().all(|value| *value == 1),
                "{} remains an exact cover",
                reference.name
            );
        }
    }
    ...
    fn macro_composition_fixtures_preserve_large_zones_and_localize_detail() {
        ...
        let dense_broad = generate_partition(&denser)
            .expect("remainder accepts denser detail")
            .into_iter()
            .filter(|region| region.family == PartitionFamily::BroadPanel)
            .map(|region| region.grid_rect)
            .collect::<Vec<_>>();
        assert_eq!(
            dense_broad, broad,
            "increasing Count must not recursively cut protected broad panels"
        );
        ...
        let horizontal_regions =
            generate_partition(&horizontal).expect("horizontal-band-heavy fixture");
        assert_eq!(
            horizontal_regions
                .iter()
                .filter(|region| region.family == PartitionFamily::HorizontalStrip)
        ...
        assert_eq!(
            vertical_regions
                .iter()
                .filter(|region| region.family == PartitionFamily::VerticalStrip)
                .count(),
            9
        );
        ...
        let mixed_regions =
            generate_partition(&mixed).expect("mixed intentional hierarchy fixture");
        assert_eq!(mixed_regions.len(), 32);
        assert!(
            mixed_regions
                .iter()
                .any(|region| region.family == PartitionFamily::BroadPanel)
        );
        assert!(
            mixed_regions
                .iter()
                .any(|region| region.family == PartitionFamily::SmallDetail)
        );
    }
}
