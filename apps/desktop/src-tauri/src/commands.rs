...
mod removed_legacy {
    ...
    use hot_trimmer_project_store::{
        LayoutCommand, ProjectStore, SourceChannel, SourceInput, SourceOwnership,
        SourceSetSnapshot, StoreError, StoredLayout, StoredSource,
    };
    ...
    pub async fn apply_patch_command(
        ...
        run_blocking(move || {
            let mut guard = shared.lock().map_err(|_| session_poisoned())?;
            let warning = guard
                .apply_patch_and_refresh_recovery(&request.command, request.coalescing_group)?;
    ...
    fn generate_template_layout(
        ...
        let definition = TemplateRegistry::built_in()
            ...
            .filter(|definition| {
                definition.identity.compatibility_key == identity.compatibility_key
            })
            .cloned()
            .ok_or_else(|| {
    ...
    pub async fn apply_layout_command(
        ...
        run_blocking(move || {
            let mut guard = shared.lock().map_err(|_| session_poisoned())?;
            let warning = guard
                .apply_layout_and_refresh_recovery(&request.command, request.coalescing_group)?;
    ...
    #[tauri::command]
    #[allow(clippy::needless_pass_by_value)]
    pub fn fit_patch_polygon(
        request: PolygonAssistRequest,
    ) -> Result<PatchGeometry, UserFacingError> {
        validate_protocol(request.protocol_version)?;
        let assistance =
    ...
    pub async fn generate_patch_preview(
        ...
        let result_job = job.clone();
        let result = run_blocking(move || {
            render_patch_preview(&patch, &source, request.max_edge, &job, &app)
        })
        .await;
        if let Ok(mut current) = jobs.lock()
            && current
    ...
    fn inspect_stored(source: &StoredSource) -> Result<InspectedImage, UserFacingError> {
        ...
        match input.ownership {
            SourceOwnership::OwnedCopy => inspect_bytes_with_policy(
                input.owned_bytes.clone().ok_or_else(|| {
                    store_error(StoreError::InvalidData("owned source bytes".into()))
                })?,
                DecodeLimits::default(),
                policy,
            )
    ...
    mod tests {
        ...
        fn rust_requests_match_the_phase_three_layout_contract_fixture() {
            ...
            let generate: GenerateLayoutRequest =
                serde_json::from_value(fixture["generateRequest"].clone())
        ...
        fn cancelled_and_stale_layout_solves_cannot_publish() {
            ...
            let error = validate_layout_publish(&stale, true, 4, 5)
                .expect_err("stale solve cannot publish");
        ...
        fn mixed_history_snapshots_keep_patches_layout_and_availability_coherent() {
            ...
            let make_session = |name: &str| {
                let mut store =
                    ProjectStore::create(&root.join(format!("{name}.hottrimmer")), name)
                        .expect("project");
